...
은행에서 고객의 재정 상태를 예측하는 업무가 있다고 상상해보자. 모형이 정확할수록 은행은 많은 돈을 벌겠지만 예측값이 대출 신청에 사용될 터이니 해당 예측을 한 합법적 이유를 설명해야 한다. 여러 모형을 실험한 결과 XGBoost가 구현한 그래디언트 부스팅 트리 정확도가 가장 높음을 알았다. XGBoost가 예측한 이유를 설명하긴 까다로워 보이므로 선형 모형으로 돌아가거나 XGBoost 모형을 해석할 수 있는 방안을 고민해봐야한다. 데이터 과학자는 정확도를 포기하고 싶지 않을 것이기에 후자를 시도하여 복잡한 XGBoost 모형(깊이가 6인 1,247개의 트리)을 해석하기로 결정했다.
