...
소득 예측 모형에 적용된 전역 평균(\|트리 SHAP\|) 방법. x축은 본질적으로 모형에서 변수가 "숨겨질" 때 모형 출력값 크기 변화의 평균이다.(이 모형의 경우 출력값은 로그 오즈 단위를 갖는다.) 자세한 내용은 [논문](https://github.com/slundberg/shap)을 참조하라. "숨김"은 모형에서 해당 변수를 적분하여 소거하는걸 의미한다. 변수를 숨겼을 때의 영향은 숨겨진 다른 변수에 따라 달라지므로 일관성과 정확성 유지를 위해 Shapley 값이 사용된다.
