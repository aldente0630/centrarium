...
여기서 XGBoost 모형이 잡아낸, 잠재적 소득에 대한 연령의 명확한 영향력을 볼 수 있다. 변수 값이 변함에 따라 모형 출력값 평균의 추이를 표시하는 기존 부분 종속성 도표와 달리 이러한 SHAP 종속성 도표는 상호 작용 효과를 나타낸다. 데이터셋의 많은 사람들이 20대지만 20대 점들의 수직적 산포를 통해 알 수 있듯이 그들의 연령이 그들 예측값에 영향을 미치는 정도는 서로 매우 다르다. 이건 다른 변수들이 연령 중요도에 영향을 미치고 있음을 의미한다. 어떤 변수가 영향력의 일부인지 살펴보기 위해 각 점을 교육 년수로 채색해보자. 교육 수준이 높을수록 20대 연령의 영향력은 줄지만 30대는 증대함을 알 수 있다.
