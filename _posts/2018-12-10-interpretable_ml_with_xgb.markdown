...
이득(Gini 중요도)과 같이 널리 사용되는 방식이 이렇게 명확하게 비일관적인 결과를 가져온다는 사실은 좀 놀랍다. 왜 이런 일이 발생하는지 더욱 잘 이해하기 위해 모형 A와 모형 B를 두고 이득 계산 방식을 살펴 보겠다. 단순화하기 위해 데이터셋의 25%가 각 리프에 속하고 각 모형의 데이터셋 레이블은 모형의 출력값과 정확히 일치한다고 가정하자.
