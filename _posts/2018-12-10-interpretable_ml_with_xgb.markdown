...
일관성이 유지되지 않으면 어떤 두 모형 간에 변수 기여도를 비교할 수 없다. *기여도가 더 높다고 모형이 해당 변수에 실제로 더 의존함을 의미하지 않는다.*
  
정확성이 유지되지 못하면 각 변수의 속성이 결합되어 전체 모델의 출력을 나타내는 방법을 알 수 없습니다. 메소드가 일관성을 잃을 수 있기 때문에 메소드가 완료된 후에는 속성을 표준화 할 수 없습니다.  
(번역 중)
