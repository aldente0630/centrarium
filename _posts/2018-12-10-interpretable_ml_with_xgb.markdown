...
소득 예측 모형에 적용된 전역 평균(\|트리 SHAP\|) 방법. x축은 본질적으로 모형에서 변수가 "숨겨질" 때 모형 출력값 크기 변화의 평균이다.(이 모형의 경우 출력값은 로그 오즈 단위를 갖는다.) 자세한 내용은 [논문](https://github.com/slundberg/shap)을 참조하라. "숨김"은 모형에서 해당 변수를 적분하여 없애는 걸 의미한다. 기능 숨기기의 영향은 숨겨진 다른 기능에 따라 변경되므로 Shapley 값을 사용하여 일관성과 정확성을 강화합니다.
