...
은행 속 데이터 과학자로서의 노력으로 돌아가면... 일관성과 정확성이 중요하다는 걸 알고 있다. 사실 방법이 일관성이 없다면 가장 높은 기여도를 가진 기능이 실제로 가장 중요하다는 보장이 없습니다. 그래서 우리는 은행의 업무와 관련이없는 두 가지 매우 간단한 트리 모델을 사용하여 각 방법의 일관성을 점검합니다.
