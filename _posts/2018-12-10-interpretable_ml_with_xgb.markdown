...
중요도의 첫 번째 정의는 모형에 대한 변수의 전역적인 영향력을 측정한다. 두 번째 정의는 단일 예측값에 대한 변수의 개별화된 영향력을 측정한다. 단순 트리 모형의 경우, *기침* 변수는 모형 B에서 명백히 더 중요하다. 전체적인 중요도와 *발열*과 *기침*이 모두 예일 때의 개별 예측값 모든 면에서 말이다.
