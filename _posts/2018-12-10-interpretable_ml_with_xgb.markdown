...
2. **적용 범위.** 스플릿 수의 연수 데이터 포인트 수에 따라 가중치가 적용된 모든 트리에서 각 변수가 데이터를 분할하는데 사용된 횟수.
3. **이득.** 분할을 위해 변수를 사용할 때 감소한 평균 훈련 손실.

이는 트리 기반 모델링 패키지에서 찾을 수 있는 중요도의 대표적인 척도이다. 가중치가 기본 옵션이었기 때문에 다른 두 가지 접근법과 차이가 있는지 확인해보았다.
