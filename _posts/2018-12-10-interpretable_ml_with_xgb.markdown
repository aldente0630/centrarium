...
견고한 이론적 토대에 빠르고 실용적인 알고리즘이 더해져 SHAP 값은 XGBoost 그래디언트 부스팅 머신 같은 트리 모형을 신뢰도 있게 해석하는 강력한 도구가 된다. 이 새로운 접근 방식으로 무장하고 은행의 XGBoost 모형을 해석하는 작업으로 돌아가보자.
  
![그림6](https://aldente0630.github.io/assets/interpretable_ml_with_xgb6.png)
소득 예측 모형에 적용된 전역 평균(\|트리 SHAP\|) 방법. x축은 본질적으로 모형에서 변수가 "숨겨질" 때 모형 출력값의 크기 변화의 평균이다(이 모델의 경우 출력에는 로그 - 오딧 단위가 있음). 자세한 내용은 [논문](https://github.com/slundberg/shap)을 참조하십시오. 그러나 "숨김"은 변수를 모델 밖으로 통합하는 것을 의미합니다. 기능 숨기기의 영향은 숨겨진 다른 기능에 따라 변경되므로 Shapley 값을 사용하여 일관성과 정확성을 강화합니다.
