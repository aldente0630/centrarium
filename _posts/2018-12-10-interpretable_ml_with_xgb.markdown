...
견고한 이론적 정당성과 빠른 실용적인 알고리즘의 결합으로 SHAP 값은 XGBoost의 그라데이션 부스팅 머신과 같은 트리 모델을 자신있게 해석하는 강력한 도구가됩니다. 이 새로운 접근 방식으로 무장 한 우리 은행 XGBoost 모델을 해석하는 작업으로 돌아갑니다.
  
![그림6](https://aldente0630.github.io/assets/interpretable_ml_with_xgb6.png)
소득 예측 모델에 적용되는 지구 평균 (| Tree SHAP |) 방법. X 축은 본질적으로 모델에서 피쳐가 "숨겨져"있을 때 모델 출력의 평균 크기 변화입니다 (이 모델의 경우 출력에는 로그 - 오딧 단위가 있음). 자세한 내용은 [논문](https://github.com/slundberg/shap)을 참조하십시오. 그러나 "숨김"은 변수를 모델 밖으로 통합하는 것을 의미합니다. 기능 숨기기의 영향은 숨겨진 다른 기능에 따라 변경되므로 Shapley 값을 사용하여 일관성과 정확성을 강화합니다.
  
*관계* 특징이 실제로 가장 중요하다는 것을 알 수 있으며 연령 특성이 뒤 따른다. SHAP 값은 일관성을 보장하므로 이득 또는 분할 수 계산 방법을 사용하기 전에 발견 한 모순의 종류에 대해 걱정할 필요가 없습니다. 그러나 이제 우리는 모든 사람에 대해 개별화 된 설명을 했으므로 막 대형 차트를 만드는 것 이상을 할 수 있습니다. 우리는 데이터 세트의 모든 고객에 대해 기능의 중요성을 계획 할 수 있습니다. [shap 파이썬 패키지](https://github.com/slundberg/shap)는 이것을 쉽게 만듭니다. 먼저 shap.TreeExplainer (model) .shap_values (X)를 호출하여 모든 예측을 설명하고 shap.summary_plot (shap_values, X)를 호출하여 다음 설명을 그립니다.

![그림7](https://aldente0630.github.io/assets/interpretable_ml_with_xgb7.png)
모든 고객은 각 행에 하나의 점이 있습니다. 점의 x 위치는 해당 기능이 고객에 대한 모델 예측에 미치는 영향이며 점의 색은 고객을위한 해당 기능의 가치를 나타냅니다. 행에 맞지 않는 점은 밀도를 나타냅니다 (이 예에서는 32,561 명의 고객이 있음). XGBoost 모델에는 물 리적 손실이 있기 때문에 x 축에는 log-odds 단위가 있습니다 (트리 SHAP는 모델의 여백 출력 변경을 설명합니다).
  
기능은 평균 (\| 트리 SHAP \|)으로 정렬되므로 연례로 $ 50K 이상을 만드는 가장 강력한 예측 인자로 *관계* 기능을 다시 볼 수 있습니다. 피처의 영향을 모든 샘플에 플로팅함으로써 우리는 중요한 아웃 라이어 효과를 볼 수 있습니다. 예를 들어, 자본 이득이 전 세계적으로 가장 중요한 특징은 아니지만, 일부 고객에게는 자본 이득이 가장 중요한 특징입니다. 기능적 가치에 의한 채색은 우리가 젊을수록 50,000 달러 이상을 버는 기회를 줄이는 반면, 고등 교육은 50,000 달러를 넘을 기회를 증가시키는 것과 같은 패턴을 보여줍니다.
  
우리는 여기에서 멈추고이 상영을 상사에게 보여줄 수 있지만 대신에 이러한 기능 중 일부를 조금 더 깊이 파고 들자. *나이* 기능에 대해 연령 SHAP 값 (로그 확률 변화)을 플로팅하여 연령 특성에 대해이를 수행 할 수 있습니다.
  
![그림8](https://aldente0630.github.io/assets/interpretable_ml_with_xgb8.png)
Y 축은 연령 기능이 매년 $ 50K 이상을 벌어 들일 확률을 얼마나 많이 바꿉니 까. X 축은 고객의 연령입니다. 각 점은 데이터 세트의 단일 고객을 나타냅니다.
  
여기서 우리는 XGBoost 모델에 포착 된 잠재력에 대한 연령의 명확한 영향을 봅니다. 피쳐의 값을 변경할 때 평균 모델 출력을 표시하는 기존의 부분 종속성 플롯과 달리 이러한 SHAP 의존도 플롯은 상호 작용 효과를 나타냅니다. 데이터 세트의 많은 사람들이 20 세이지만 그들의 나이가 그들의 예측에 얼마나 영향을 미치는지는 20 세에 점들의 수직적 인 분산으로 나타나는 것과 다릅니다. 이것은 다른 기능들이 연령의 중요성에 영향을 미치고 있음을 의미합니다. 어떤 효과가이 효과의 일부인지 알아보기 위해 우리는 점 수를 교육 년수로 표시하고 교육 수준이 높으면 20 대 연령의 영향을 줄이지 만 30 대는 증가시킵니다.
  
![그림9](https://aldente0630.github.io/assets/interpretable_ml_with_xgb9.png)
Y 축은 연령 기능이 매년 $ 50K 이상을 벌어 들일 확률을 얼마나 많이 바꿉니 까. X 축은 고객의 연령입니다. 교육 - Num은 고객이 완료 한 교육 연수입니다.
  
주당 근무 시간 수에 대한 또 다른 의존성 도표를 작성하면 약 50 시간 / 주로 더 평원을 일할 수있는 이점과 결혼 생활을한다면 여분의 근로가 높은 수입을 나타내는 경향이 적음을 알 수 있습니다.
...
이 간단한 워크 스루는 자신의 모델을 설계하고 배포 할 때 수행 할 수있는 프로세스를 반영하기위한 것입니다. [shap 패키지](https://github.com/slundberg/shap)는 pip를 통해 쉽게 설치할 수 있으며, 자신있게 모델을 탐색하는 데 도움이되기를 바랍니다. SHAP 상호 작용 값, 모델 불가지론 SHAP 값 추정 및 추가 시각화를 포함하여이 기사에서 다뤘던 것 이상을 포함합니다. 다양한 흥미로운 데이터 세트의 모든 기능을 보여주는 [노트북](https://slundberg.github.io/shap/notebooks/NHANES%20I%20Survival%20Model.html)을 사용할 수 있습니다. 예를 들어, XGBoost 사망률 모델을 설명하는 노트북의 건강 검진을 기반으로 사망 할 주요 이유를 확인할 수 있습니다. Python 이외의 언어의 경우, Tree SHAP가 코어 XGBoost 및 LightGBM 패키지에 직접 병합되었습니다.
