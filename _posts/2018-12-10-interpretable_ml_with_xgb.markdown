...
어떤 변수 속성 분석 방법을 다른 방법과 비교할 기준이 마땅치않다. 데이터 정리, 편향 탐지 등의 작업을 통해 각 방법의 최종 사용자 성능을 측정해볼 수 있다. 그러나 이러한 작업은 변수 속성 분석 방법의 품질에 대한 간접적 측정이 될 뿐이다. 대신 여기에서 좋은 변수 속성 분석 방법이 가져야할 두 가지 성질을 정의해보자.
