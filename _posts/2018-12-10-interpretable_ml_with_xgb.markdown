...
주당 근무 시간에 대해 또 다른 종속성 도표를 그려보자. 주당 50시간을 넘어가면 추가 근무의 이점이 크게 없어지며 결혼 생활을 할 경우 추가 근무가 높은 수입으로 이어지지 못하는걸 볼 수 있다.
