...
1. 무게. 지형지 물이 모든 트리에서 데이터를 분할하는 데 사용 된 횟수입니다.
2. 덮개. 지형지 물이 모든 트리에서 데이터를 분할하는 데 사용되는 스플릿 수의 연수 데이터 포인트 수에 따라 가중치가 적용된 횟수입니다.
3. 이득. 분할을 위해 피쳐를 사용할 때 평균 교육 손실 감소가 발생했습니다.
