...
XGBoost가 출력한 변수 중요도를 살펴보면 *연령*을 다른 변수를 압도하는 가장 중요한 수입 예측 변수로 꼽을 수 있다. 여기서 멈추고 관리자에게 *주당 근무 시간*과 *교육 수준*에 따라 나이가 가장 중요한 특징이라는 직관적 인 대답을 할 수 있습니다. 그러나 좋은 데이터 과학자로서 우리는 문서를보고 XGBoost에서 기능 중요성을 측정하는 세 가지 옵션이 있음을 확인합니다.
