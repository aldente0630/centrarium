...
여기서 XGBoost 모형이 잡아낸, 잠재적 소득에 대한 연령의 명확한 영향력을 볼 수 있다. 변수 값이 변함에 따라 모형 출력값 평균의 추이를 표시하는 기존 부분 종속성 도표와 달리 이러한 SHAP 의존도 도표는 상호 작용 효과를 나타낸다. 데이터셋의 많은 사람들이 20대지만 20대 점들의 수직적 산포를 통해 알 수 있듯이 그들의 연령이 그들의 예측값에 영향을 미치는 정도는 서로 매우 다르다. 이건 다른 변수들이 연령 중요도에 영향을 미치고 있음을 의미한다. 어떤 변수가 이런 영향력의 일부인지 살펴보기 위해 각 점을 교육 년수로 채색해보자. 교육 수준이 높을수록 20대 연령의 영향력은 줄지만 30대는 증대함을 알 수 있다.
...
주당 근무 시간에 대해 또 다른 의존성 도표를 작성해보자. 주당 약 50시간 이상이면 추가로 일할 때 이점이 크게 없으며 결혼 생활을 할 경우 추가 근로 시간이 높은 수입으로 드러나는 경향이 적음을 알 수 있다.
