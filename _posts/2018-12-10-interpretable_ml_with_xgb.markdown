...
**XGBoost에서 제공하는 세 가지 옵션 각각에 대해 기능 중요도 순서가 매우 다르다는 것을 알게되었습니다!** 커버 방법에있어서, 자본 이득 특징이 소득에 대해 가장 예측하는 것처럼 보이지만, 이득 방법에 대해서는 관계 상태 특성이 모든 것을 지배한다. 이는 어떤 방법이 가장 적합한 지 알지 못해도 기능 중요도를보고하기 위해 이러한 조치에 의존하는 것에 대해 우리를 매우 불편하게 만듭니다.
