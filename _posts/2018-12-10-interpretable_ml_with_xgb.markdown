...
견고한 이론적 토대에 빠르고 실용적인 알고리즘이 더해져 SHAP 값은 XGBoost 그래디언트 부스팅 머신 같은 트리 모형을 신뢰도 있게 해석하는 강력한 도구가 된다. 이 새로운 접근법으로 무장하고 은행의 XGBoost 모형을 해석하는 작업으로 돌아가보자.
  
![그림6](https://aldente0630.github.io/assets/interpretable_ml_with_xgb6.png)
소득 예측 모형에 적용된 전역 평균(\|트리 SHAP\|) 방법. x축은 본질적으로 모형에서 변수가 "숨겨질" 때 모형 출력값 크기 변화의 평균이다.(이 모형의 경우 출력값은 로그 오즈 단위를 갖는다.) 자세한 내용은 [논문](https://github.com/slundberg/shap)을 참조하라. "숨김"은 모형에서 해당 변수를 적분하여 소거하는걸 의미한다. 변수를 숨겼을 때 영향력은 숨겨진 다른 변수에 따라 달라지므로 일관성과 정확성 유지를 위해 Shapley 값이 사용된다.
  
실제로 *관계* 변수가 가장 중요하다는 사실을 알 수 있으며 연령 변수가 그 뒤를 잇는다. SHAP 값은 일관성을 보장하므로 이득과 분할 횟수 계산 방법을 사용하기 전에 발견 한 상호 에 대해 걱정할 필요가 없습니다. 그러나 이제 우리는 모든 사람에 대해 개별화 된 설명을 했으므로 막 대형 차트를 만드는 것 이상을 할 수 있습니다. 우리는 데이터 세트의 모든 고객에 대해 기능의 중요성을 계획 할 수 있습니다. [shap 파이썬 패키지](https://github.com/slundberg/shap)는 이것을 쉽게 만듭니다. 먼저 shap.TreeExplainer (model) .shap_values (X)를 호출하여 모든 예측을 설명하고 shap.summary_plot (shap_values, X)를 호출하여 다음 설명을 그립니다.
