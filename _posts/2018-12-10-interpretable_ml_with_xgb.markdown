...
모든 고객은 각 행마다 점 하나로 표현된다. 점의 x축 위치는 모형의 고객 예측에 해당 변수가 미치는 영향이며 점의 색은 고객의 해당 변수값을 나타낸다. 밀도가 드러나게끔 점들이 행에 다소 맞지 않게 쌓여있다.(이 예에 고객 32,561명이 있음) XGBoost 모형은 로지스틱 손실을 사용하므로 x축은 로그 오즈 단위를 갖는다.(트리 SHAP는 모형 마진 출력값의 변화를 설명한다).
  
변수를 평균 (\|트리 SHAP\|)로 정렬하면 연 $ 50K 이상으로 이끄는 가장 강력한 예측 변수가 *관계* 변수임을 다시 볼 수 있다. 피처의 영향을 모든 샘플에 플로팅함으로써 우리는 중요한 아웃 라이어 효과를 볼 수 있습니다. 예를 들어, 자본 이득이 전 세계적으로 가장 중요한 특징은 아니지만, 일부 고객에게는 자본 이득이 가장 중요한 특징입니다. 기능적 가치에 의한 채색은 우리가 젊을수록 50,000 달러 이상을 버는 기회를 줄이는 반면, 고등 교육은 50,000 달러를 넘을 기회를 증가시키는 것과 같은 패턴을 보여줍니다.
