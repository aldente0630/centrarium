...
일반적으로 트리 루트에 있는 변수가 리프 근처에서 분할한 변수보다 더 중요할거라고 기대한다(트리는 탐욕스럽게 구성되기 때문에). 그러나 이득 방법은 낮은 쪽 분할이 더 높은 중요도를 기여하게끔 편향되어있다. 편향은 비일관성으로 이어지고 *기침*이 더 중요해지면(따라서 루트에서 분할되면) 실제로 기여한 중요도는 하락한다. [treeinterpreter](https://github.com/andosa/treeinterpreter) 패키지가 사용하는 개별화된 Saabas 방식은 트리를 내려가며 예측의 차이를 계산하므로 트리의 낮은 쪽 분할에 대해 동일한 편향을 겪는다. 트리 깊이가 커질수록 이러한 편향은 더욱 커진다. 이와 다르게 트리 SHAP 방식은 트리에서의 위치에 의해 지정된 순서 만이 아니라, 피처의 모든 가능한 순서에 대한 예측의 차이를 평균화하는 것과 수학적으로 동일합니다.
