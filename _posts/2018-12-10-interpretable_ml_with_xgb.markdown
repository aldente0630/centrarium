...
모든 고객은 각 행마다 점 하나로 표현된다. 점의 x축 위치는 모형의 고객 예측에 해당 변수가 미치는 영향이며 점의 색은 고객의 해당 변수값을 나타낸다. 밀도가 드러나게끔 점들이 행에 다소 맞지 않게 쌓여있다.(이 예에서는 고객 32,561명이 있음) XGBoost 모형은 로지스틱 손실을 사용하기에 x축은 로그 오즈 단위를 갖는다.(트리 SHAP는 모형의 여백 출력 변경을 설명한다).
