...
1. **일관성.** 변수를 더 많이 사용하게끔 모형을 변경할 때마다 각 변수 속성으로 지정한 중요도가 감소하지 않아야한다.
2. **정확성.** 각 변수 중요도의 합계가 모형의 전체 중요도여야한다. 예를 들어 중요도가 R<sup>2</sup> 값으로 측정되는 경우 각 변수 속성의 합이 전체 모형 R<sup>2</sup>이 되어야 한다.
  
일관성이 유지되지 않으면 어떤 두 모형 간에 변수 속성 도를 비교할 수 없다. *더 높은 기여도를 지정했다고해서 모델이 실제로 해당 기능에 더 의존한다는 것을 의미하지는 않습니다.*
