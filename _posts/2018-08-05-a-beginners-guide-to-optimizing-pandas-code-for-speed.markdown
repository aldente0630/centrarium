...
판다스는 수학 연산에서 집계 및 문자열 함수(사용 가능한 함수의 광범위한 목록은 [판다스 문서](https://pandas.pydata.org/pandas-docs/stable/index.html)를 확인하라)에 이르기까지 다양한 벡터화 함수를 포함하고 있다. 내장 기능은 판다스 시리즈와 데이터프레임에서 작동하도록 최적화되어있다. 결과적으로 벡터화 판다스 함수를 사용하는 건 비슷한 목적을 달성하고자 손수 반복시키는 방식보다 거의 항상 바람직하다.
