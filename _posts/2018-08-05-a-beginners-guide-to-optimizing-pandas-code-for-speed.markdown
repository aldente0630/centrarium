...
def haversine_looping(df):
 ...
 위 함수를 실행하는 데 걸리는 시간을 알기 위해 `%timeit` 명령을 사용했다. `%timeit`은 [주피터 노트북](http://jupyter.org) 용으로 특별히 제작한 "[매직](http://ipython.readthedocs.io/en/stable/interactive/magics.html)" 명령이다. (모든 매직 명령은 명령을 한 줄에 적용할 경우 `%` 기호로 시작하고 전체 주피터 셀에 적용하려면 `%%`로 시작한다.) `%timeit`는 함수를 여러 번 실행하고 얻은 실행 시간 평균 및 표준 편차를 인쇄합니다. 물론 % timeit으로 얻은 런타임은 함수를 실행하는 모든 시스템에서 반드시 동일하지는 않습니다. 여전히 동일한 시스템 및 데이터 세트에서 여러 기능의 런타임을 비교하는 데 유용한 벤치마킹 도구를 제공 할 수 있습니다.
