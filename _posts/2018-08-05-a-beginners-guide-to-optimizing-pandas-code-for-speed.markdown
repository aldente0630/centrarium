...
판다스는 수학 연산에서 집계 및 문자열 함수(사용 가능한 함수의 광범위한 목록은 [판다스 문서](https://pandas.pydata.org/pandas-docs/stable/index.html)에서 확인하라)에 이르기까지 다양한 벡터화 함수를 포함하고 있다. 내장 함수는 판다스 시리즈와 데이터프레임에서 작동하게끔 최적화되어있다. 결과적으로 벡터화 판다스 함수를 사용하는건 비슷한 목적을 위해 손수 반복시키는 방법보다 거의 항상 바람직하다.
...
이 시점에서 우리는 하루라고 부를 수 있습니다. Pandas 시리즈를 벡터 라이 제이션하면 매일 계산할 때 압도적 인 다수의 최적화 요구 사항을 얻을 수 있습니다. 그러나 속도가 최우선 순위 인 경우 NumPy Python 라이브러리 형식으로 증원을 호출 할 수 있습니다.
  
[NumPy 라이브러리](http://www.numpy.org)는 "파이썬에서의 과학 컴퓨팅을위한 기본 패키지"라고 말하며 최적화 된 사전 컴파일 된 C 코드로 작업을 수행합니다. Pandas와 마찬가지로 NumPy는 배열 객체 (ndarrays라고 함)에서 작동합니다. 그러나 인덱싱, 데이터 유형 확인 등과 같은 Pandas 시리즈 작업으로 인해 많은 오버 헤드가 발생하지 않습니다. 결과적으로 NumPy 배열의 작업은 Pandas 시리즈 작업보다 훨씬 빠릅니다.
  
NumPy 배열은 Pandas 시리즈가 제공하는 추가 기능이 중요하지 않을 때 Pandas 시리즈 대신 사용할 수 있습니다. 예를 들어, Haversine 함수의 벡터화 된 구현은 실제로 위도 또는 경도 시리즈의 인덱스를 사용하지 않으므로 사용할 수있는 인덱스가 없어도 함수가 중단되지 않습니다. 이에 비해 인덱스로 값을 참조해야하는 DataFrame 조인과 같은 작업을 수행했다면 팬더 개체를 계속 사용하는 것이 좋습니다.
  
우리는 위도와 경도 배열을 Pandas 시리즈에서 NumPy 배열로 단순히 시리즈의 값 방법을 사용하여 변환합니다. 이 시리즈의 벡터화와 마찬가지로 NumPy 배열을 함수에 직접 전달하면 팬더가 전체 벡터에 함수를 적용합니다.
...
NumPy 배열에서 연산을 실행하면 또 다른 4 배의 개선이 이루어졌습니다. 결론적으로 우리는 NumPy로 벡터화를 통해 루핑을 통해 30 분의 1 초에서 3 분의 1 밀리 초로 런타임을 개선했습니다!
