...
판다스는 수학 연산에서 집계 및 문자열 함수(사용 가능한 함수의 광범위한 목록은 [판다스 문서](https://pandas.pydata.org/pandas-docs/stable/index.html)를 확인하라)에 이르기까지 다양한 벡터화 함수 모음을 포함한다. 내장 기능은 Pandas 시리즈 및 DataFrames에서 작동하도록 최적화되어 있습니다. 결과적으로, vectorized Pandas 함수를 사용하는 것은 맞춤 작성 루핑으로 유사한 목적을 달성하는 것보다 거의 항상 바람직합니다.
