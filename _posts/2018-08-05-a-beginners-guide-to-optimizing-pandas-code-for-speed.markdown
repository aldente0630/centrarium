...
함수 수행의 반복량 줄이는 방법을 이해하기 위해 판다스, 데이터프레임과 시리즈 기본 단위가 모두 배열 기반임을 알아두자. 기본 단위의 고유한 구조는 개별 값 (**스칼라**라고 함)을 순차적으로 사용하는 대신 전체 배열에서 작동하도록 설계한 내장 판다스 함수로 변환된다. **벡터화**는 전체 배열에서 작업을 실행하는 프로세스입니다.
