...
NumPy 라이브러리는 "파이썬에서의 과학 컴퓨팅을위한 기본 패키지"라고 말하며 최적화 된 사전 컴파일 된 C 코드로 작업을 수행합니다. Pandas와 마찬가지로 NumPy는 배열 객체 (ndarrays라고 함)에서 작동합니다. 그러나 인덱싱, 데이터 유형 확인 등과 같은 Pandas 시리즈 작업으로 인해 많은 오버 헤드가 발생하지 않습니다. 결과적으로 NumPy 배열의 작업은 Pandas 시리즈 작업보다 훨씬 빠릅니다.
