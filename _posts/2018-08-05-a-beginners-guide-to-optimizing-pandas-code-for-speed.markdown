...
판다스는 수학 연산에서 집계 및 문자열 함수(사용 가능한 함수의 광범위한 목록은 [판다스 문서](https://pandas.pydata.org/pandas-docs/stable/index.html)를 확인하라)에 이르기까지 다양한 벡터화 함수를 포함하고 있다. 내장 기능은 판다스 시리즈와 데이터프레임에서 작동하게 최적화되어있다. 결과적으로 벡터화 판다스 함수를 사용하는 건 비슷한 목적을 위해 손수 반복하는 방법보다 거의 항상 바람직하다.
  
지금까지는 Haversine 함수에 스칼라를 전달했다. 그러나 Haversine 함수 내에서 사용하는 모든 함수는 배열에서도 작동할 수 있다. 이렇게하면 거리 함수를 매우 간단하게 벡터화 할 수 있습니다. 각 스칼라 값에 위도와 경도를 전달하는 대신 전체 시리즈 (열)에 전달합니다. 이를 통해 팬더는 벡터화 된 기능에 사용할 수있는 모든 최적화 옵션을 활용할 수 있습니다. 특히 전체 배열에 대한 모든 계산을 동시에 수행하는 것이 좋습니다.
