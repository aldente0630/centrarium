...
함수 수행으로 반복되는 양을 줄이는 방법을 이해하려면 판다스, 데이터프레임 및 시리즈의 기본 단위가 모두 배열을 기반으로한다는 점을 상기하십시오. 기본 단위의 고유 한 구조는 개별 값 (**스칼라**라고 함)을 순차적으로 사용하는 대신 전체 배열에서 작동하도록 설계된 내장 팬더 함수로 변환됩니다. **벡터화**는 전체 배열에서 작업을 실행하는 프로세스입니다.
