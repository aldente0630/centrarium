...
Airbnb 서비스에서 데이터 제품은 항상 중요한 부분이다. 그러나 데이터 제품를 만드는데 비용이 많이 든다는 사실을 점차 깨닫게 되었다. 예를 들어 개인화 검색 순위로 게스트는 집을 더 쉽게 찾을 수 있고 스마트 가격 정책으로 호스트는 수요와 공급에 따라 더 경쟁력있는 가격을 책정할 수 있다. 그러나 이런 프로젝트들을 위해 데이터 과학 및 엔지니어링에 수많은 시간과 노력이 들어갔다.
  
최근에 Airbnb 기계학습 인프라가 발전하면서 새 기계 학습 모형을 제품화하여 배포하는 비용이 크게 절감되었다. 예를 들어 ML Infra팀은 고품질의, 검증되고, 재사용 가능한 변수를 사용자가 모형에 활용할 수 있게끔 범용 변수 저장소를 구축했다. 데이터 과학자는 여러 AutoML 도구를 작업흐름에 통합시켜 모형 선택 및 성능 벤치마킹 작업을 빠르게 했다.또한 ML Infra팀은 Jupyter 노트북을 Airflow 파이프라인으로 변환시키는 새로운 프레임워크를 만들었다.
  
이 문서에서는 LTV 모델링, 즉, Airbnb에 올라온 숙소 가치를 예측하는 특정 활용 사례를 통해 이런 도구들이 어떻게 함께 동작하며 모델링 절차를 어떻게 빠르게 만들고 전체적인 개발 비용을 어떻게 낮추는지 설명하겠다.
...
features: {
 ...
 ```
  
훈련 데이터를 구성하는데 다중형 변수가 필요한 경우 Zipline은 지능형 키값 조인을 자동으로 수행하고 뒷편으로 훈련 데이터를 채워넣는다. 숙소 LTV 모형을 위해 기존 Zipline 변수를 사용했고 나만의 변수를 일부 추가했다. 결론적으로 모형에 아래 변수를 포함해 150개 이상의 변수가 사용되었다.
