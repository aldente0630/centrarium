...
데이터 제품은 Airbnb 서비스에서 늘 중요한 부분이었다. 그러나 데이터 제품를 만드는데 비용이 많이 든다는 사실을 알게되었다. 예를 들어 개인화 검색 순위로 게스트는 집을 더 쉽게 찾을 수 있고 스마트 가격 정책으로 호스트는 수요와 공급에 따라 더 경쟁력있는 가격을 책정할 수 있다. 그러나 이런 프로젝트들을 위해 데이터 과학 및 엔지니어링에 수많은 시간과 노력이 들어갔다.
  
최근에 Airbnb 기계학습 인프라가 발전하면서 새로운 기계 학습 모형을 제품화하여 배포시키는 비용이 크게 절감되었다. 예를 들어 ML Infra팀은 고품질의, 검증되고, 재사용 가능한 변수를 사용자가 모형에 활용할 수 있게끔 범용 변수 저장소를 구축했다. 데이터 과학자는 여러 AutoML 도구를 작업흐름에 통합시켜 모형 선택 및 성능 벤치마킹 작업을 빠르게 했다.또한 ML Infra팀은 Jupyter 노트북을 Airflow 파이프라인으로 변환시키는 새로운 프레임워크를 만들었다.
