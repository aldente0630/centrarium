...
Airbnb 서비스에서 데이터 제품은 항상 중요한 부분이다. 그러나 데이터 제품를 만드는데 비용이 많이 든다는 사실을 깨닫게 되었다. 예를 들어 개인화 검색 순위로 게스트는 집을 더 쉽게 찾을 수 있고 스마트 가격 정책으로 호스트는 수요와 공급에 따라 더 경쟁력있는 가격을 책정할 수 있다. 그러나 이런 프로젝트들을 위해 데이터 과학 및 엔지니어링에 수많은 시간과 노력이 들어갔다.
