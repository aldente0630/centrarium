...
데이터가 희소해도 암시적 추천의 성능은 괜찮지만 상호작용 행렬을 좀 더 밀집되게 만들면 훨씬 좋아질 수 있다. 좋아요가 적어도 5번 이상 있는 모델만 데이터를 수집했다. 그러나 사용자 모두가 좋아요를 적어도 5번 이상 누른 건 아닐 수 있다. 좋아요를 누른 모델이 5개 미만인 사용자를 날려버리자. 해당 사용자를 날려버리면 일부 모델은 다시 좋아요 5개 미만으로 떨어질 수 있다. 수렴할 때까지 사용자와 모델 날리는 작업을 교대로 반복하자.
