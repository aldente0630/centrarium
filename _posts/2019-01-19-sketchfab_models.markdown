...
암시적 추천은 데이터가 희소해도 성능이 좋지만 상호 작용 행렬을 좀 더 밀집되게 만들면 훨씬 좋아질 수 있다. 좋아요가 적어도 5번 이상 있는 모델만 데이터를 수집했다. 그러나 사용자 모두가 좋아요를 적어도 5번 이상 한 건 아닐 수 있다. 좋아요를 한 모델이 5개 미만인 사용자를 날려버리자. 해당 사용자를 날려버리면 일부 모델이 또 좋아요 개수 5번 미만이 될 수 있으므로 수렴할 때까지 사용자와 모델 날리는 작업을 앞뒤로 반복하자.
