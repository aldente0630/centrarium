...
WARP 논문 저자는 이러한 절차를 통해 BPR이 수행하는 AUC의 최적화가 정밀도의 최적화로 바뀐다고 주장한다. 본 성질은 대부분의 추천 시스템 목표와 관련 있을 것이고 BPR 손실보다 WARP 손실을 최적화하는 편이 개인적으로도 좋았다. WARP는 두 개의 하이퍼 파라미터가 갖는다. 하나는 예측이 얼마나 틀렸는지 보고 SGD 업데이트 수행을 결정하는 마진 값이다. 논문에서는 마진 값이 1이므로 SGD 업데이트를 수행하기 위해서는 \\(p_{uj} > p_{ui} + 1\\)로 추정해야한다. 다른 하이퍼 파라미터는 현재 사용자를 중단하고 다음 사용자로 이동하기 전, 잘못된 예측을 얻기 위해 부정적 표본 추출을 얼마나 할 지 결정하는 지정 값이다. 
