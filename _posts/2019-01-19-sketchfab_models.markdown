...
조사 자료를 읽다보니 고전, [추천 시스템을 위한 행렬 분해 기법](https://datajobs.com/data-science-repo/Recommender-Systems-[Netflix].pdf)(pdf 링크) 내용 중에 "부가적 입력 원천" 절이 있었다. 여기에는 소위 "부가 정보"를 추천 시스템에 통합시키는 접근법이 나와있다. 아이디어는 상대적으로 단순했다.(행렬 분해에 대해 머리를 부여잡게 만드는 초기 장벽에 비하면 단순하다.) 정규 행렬 분해 모형에서 사용자 \\(u\\)는 사용자의 잠재 요인(이에 대한 배경 설명은 이전 게시물들을 봐라)을 담고 있는 단일 벡터 \\(\mathbf{x}_u\\)로 표현된다. 자, 해당 사용자에 대한 인구 통계학적 정보가 다음과 같이 있다.
