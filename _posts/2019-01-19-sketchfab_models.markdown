...
6.이 차이를 시그모이드에 전달하고 확률적 경사 하강법(SGD)을 통해 모형의 모든 파라미터를 업데이트하기 위한 가중치로 이용한다.  
...
다른 느린 방법과 비교하여 표본 추출 기반 접근법을 이용하기 때문에 저자는 이 모형이 상당히 빠르고 확장 가능하다는 사실을 보여준다. 또한, 저자들은 BPR이 ROC 곡선 면적(AUC)을 직접 최적화한다고 주장한다. 이는 바람직한 특성일 수 있다. 가장 중요한 건 계산 속도를 뚝 떨어뜨리지 않고 부가 정보를 쉽게 추가할 수 있다는 것이다.
