...
Birchbox에서 일하기 시작했을 때 암시적 피드백 행렬 분해 추천 시스템에 풍부한 사용자 정보를 통합시킬 수있는 방법을 모색해야만 했다. 나는 내가 무엇을 하고 있는지 전혀 몰랐다. 그래서 구글 검색을 많이 했다. 추천 시스템에는 주로 두 가지 패러다임, 즉 인구 통계학적 사용자 데이터가 있고 다른 유사 사용자를 찾기 위해 이 데이터를 사용하는 방식의 내용 기반 접근과 각 사용자가 상호작용 품목들을 평가한 내용이 데이터로 있는 "점수 기반" 접근이 있기에 이는 어려운 문제였다. 나는 두 접근 방식이 결합하길 원했다.
