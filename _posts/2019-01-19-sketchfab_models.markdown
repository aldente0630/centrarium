...
이 변수들 각각을 "속성-공간" \\(A\\)로 원-핫-인코딩할 수 있고 각각의 속성 \\(a\\)가 잠재 벡터 \\(\mathbf{s}_a\\)를 갖는다고 가정하자. 마지막으로 "총" 사용자 벡터는 원래 사용자 벡터 \\(\mathbf{x}_u\\)에 연관된 속성 벡터들을 더한 것이라고 가정하자. 만약 \\(N(u)\\)가 사용자 \\(u\\)에 속하는 속성 집합을 나타내면 총 사용자 벡터는
    
$$\mathbf{x}_u + \sigma_{a \in N(u)} \mathbf{s}_a$$
   
항목에 대한 부가 정보에 대해 동일한 가정 집합을 만들 수 있으며 이제 권장 사항으로 더 나은 결과를 얻을 수있을뿐만 아니라 벡터 및 결과적으로 측면 정보 벡터 간의 유사점을 배울 수 있습니다. 이에 대한 일반적인 개요는 [Dia & Co](https://making.dia.com)의 기술 블로그에 게시 한 [게시물](https://making.dia.com/embedding-everything-for-anything2anything-recommendations-fca7f58f53ff)을 참조하십시오.

좋습니다. 접근법이 명확합니다. 아마도 마지막 게시물의 암시 적 피드백 목표 함수에이를 더하고 해결할 것입니다. 맞습니까? 글쎄, 나는 수학을 돌렸고 불행하게도 이것은 확장 할 수 없다. 추가 정보 집합 벡터를 사용하면 마지막 게시물의 ALS (Alternating Least Squares)가 3 방향 교대 문제가됩니다. ALS 최적화에는 데이터의 희소성을 이용하여 계산을 확장하는 특별한 트릭이있었습니다. 아아,이 트릭은 ALS 단계에서 사이드 정보 벡터를 해결할 때 사용할 수 없습니다.
