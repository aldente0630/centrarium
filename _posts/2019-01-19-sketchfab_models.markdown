...
이 변수들 각각을 "속성-공간" \\(A\\)로 원-핫-인코딩할 수 있고 각각의 속성 \\(a\\)가 잠재 벡터 \\(\mathbf{s}_a\\)를 갖는다고 가정하자. 마지막으로 우리는 "총"사용자 벡터가 원래 사용자 벡터 \\(\mathbf{x}_u\\)라는 가정을합니다
유

  각 속성 벡터에 더하여. \\(N(u)\\)가 사용자 \\(u\\)에 속하는 속성 집합을 나타내면 총 사용자 벡터는
