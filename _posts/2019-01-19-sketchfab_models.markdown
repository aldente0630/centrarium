...
이 변수들 각각을 "속성-공간" \\(A\\)로 원-핫-인코딩할 수 있고 각각의 속성 \\(a\\)가 잠재 벡터 \\(\mathbf{s}_a\\)를 갖는다고 가정하자. 마지막으로 우리는 "총"사용자 벡터가 원래 사용자 벡터 \ textbf {x} _ {u} x라는 가정을합니다
유

  각 속성 벡터에 더하여. N (u) N (u)가 사용자 uu에 속하는 속성 집합을 나타내면 총 사용자 벡터는

\ textbf {x} _ {u} + \ sum \ limits_ {a \ in N (u)} \ textbf {s} _ {a}
엑스
유
...
항목에 대한 부가 정보에 대해 동일한 가정 집합을 만들 수 있으며 이제 권장 사항으로 더 나은 결과를 얻을 수있을뿐만 아니라 벡터 및 결과적으로 측면 정보 벡터 간의 유사점을 배울 수 있습니다. 이에 대한 일반적인 개요는 Dia & Co의 기술 블로그에 게시 한 게시물을 참조하십시오.
