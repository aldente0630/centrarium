...
WARP 논문 저자는 이러한 절차를 통해 BPR이 하는 AUC의 최적화에서 정밀도의 최적화로 바뀐다고 주장한다. 이 사항은 대부분의 추천 시스템 목표와 관련 있을 것이며 개인적으로 BPR 손실보다 WARP 손실 최적화하는 편이 보통 좋았다. WARP는 두 개의 하이퍼 파라미터가 갖는다. 하나는 예측이 얼마나 틀렸는지 보고 SGD 업데이트 수행을 결정하는 마진 값이다. 논문에서는 마진 값이 1이므로 SGD 업데이트를 수행하기 위해서는 \\(p_{uj} > p_{ui} + 1\\)로 추정해야한다. 다른 하이퍼 파라미터는 현재 사용자를 중단하고 다음 사용자로 이동하기 전, 잘못된 예측을 얻기 위해 부정적 표본 추출을 몇 번이나 할 지 결정하는 지정 값이다. 
  
해당 표본 추출 방법을 사용하면 을 시작할 때 훈련되지 않은 모델에 대한 예측이 잘못되기 쉽기 때문에 WARP가 빠르게 실행됩니다. 그러나 훈련을 마친 후에 WARP는 예측이 잘못되어 업데이트 할 때까지 많은 항목을 샘플링하고 예측해야하기 때문에 느리게 실행됩니다. 그래도 모델이 잘못 예측하게 만드는 것이 어렵다면 긍정적 인 문제와 같을 것입니다.
