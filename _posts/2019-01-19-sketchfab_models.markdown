...
Sketchfab이 사람들이 태그를 더 많이 사용하도록 권장한다고 가정 해 봅시다. 이는 사용자에게 매력적인 동안 구조화 된 데이터를 무료로 만들 수 있기 때문에 회사에 유리합니다. Sketchfab은 이미지와 함께 태그를 제안함으로써 이러한 행동을 조장 할 수 있습니다. 우리가 할 수있는 한 가지 방법은 모델을 가져 와서 현재 존재하지 않는 태그를 제안하는 것입니다. 여기에는 모델과 가장 유사한 태그 벡터를 찾은 다음 이미 존재하는 태그를 필터링하는 작업이 포함됩니다.
...
게시물이 길었다. 수행해야할 학습도 많았고 최적화시킬 파라미터도 많았다. 성능이 정말 뛰어난가 말해보면, 지난 게시물의 ALS 모형을 이기기 어려웠다고 강조하고 싶다. 해당 모형은 최적화시킬 파라미터가 더 적고 최적 하이퍼 파라미터에서 모형이 좀 벗어나도 훨씬 "용인되는" 편이다. SGD는 그와 반대로 학습률이 작다면 *쓸모없는* 결과만 줄 것이다. 그러나 탐색에 충분한 시간을 들인다면 ALS를 능가하는 것도 가능하다. 또한 부가 정보를 녹일 수 있는 기능은 새로운 유형의 추천을 만들고 콜드 스타트 문제를 해결할 때 요긴하다. 따라서 시도해볼 모형 후보에 추가하는 걸 추천한다.
