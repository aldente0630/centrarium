...
이제 그만한 것이지, WRMF 모델을 훈련 시켜서 Sketchfab 모델을 마침내 추천 할 수 있습니다!
  
첫 번째 단계는 데이터를로드하고이를 "항목 수"로 크기 "사용자 수"의 상호 작용 행렬로 변환하는 것입니다. 데이터는 현재 사용자가 Sketchfab 웹 사이트에서 "좋아하는"모델을 나타내는 각 행과 함께 csv로 저장됩니다. 첫 번째 열은 모델 이름이고 두 번째 열은 고유 한 모델 ID (mid)이며 세 번째 열은 익명 사용자 ID (uid)입니다.
