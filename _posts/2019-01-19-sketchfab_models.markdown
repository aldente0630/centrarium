...
첫 번째 단계는 데이터를 불러와서 "사용자 수" 곱하기 "항목 수" 크기의 상호 작용 행렬로 변환하는 것이다. 데이터는 사용자가 Sketchfab 웹사이트에서 "좋아하는" 모델을 나타내는 각 행으로 현재 csv형태에 맞게 저장되었다. 첫 번째 열은 모델 이름이고 두 번째 열은 고유 한 모델 ID (mid)이며 세 번째 열은 익명 사용자 ID (uid)입니다.
