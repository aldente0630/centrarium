...
Birchbox에서 일하기 시작했을 때 암시적 피드백 행렬 분해 추천 시스템에 풍부한 고객 정보를 통합시킬 수있는 방법을 모색해야만 했다. 나는 내가 무엇을 하고 있는지 전혀 몰랐다. 그래서 구글 검색을 많이 했다. 이는 추천 시스템의 두 가지 패러다임, 즉 인구 통계 학적 고객 데이터가 있고 다른 유사 고객을 찾기 위해이 데이터를 사용할 때와 같은 콘텐츠 기반 접근 방식과 데이터가있을 때와 같은 "등급 기반"접근 방식의 두 가지 패러다임이 공통적으로 있기 때문에 어려운 문제였습니다. 각 사용자가 상호 작용 한 각 항목을 평가 한 내용 나는이 두 가지 방법과 결혼하기를 바랐다.
