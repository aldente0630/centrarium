...
LightFM은 위와 동일한 방법을 사용하여 부가 정보를 녹인다. 총 "사용자 벡터"를 사용자 관련 부가 정보 벡터(사용자 "변수"라고 함)의 합산으로 가정하며 품목 또한 비슷하게 다룬다. 위에서 \\(\mathbf{x}_u\\)와 \\(\mathbf{s}_a\\), 잠재 벡터의 두 종류를 가정했다. LightFM은 모든 것을 부가 정보 또는 변수로 취급한다. 사용자 \\(u\\)에 대해 하나의 특정 사용자 벡터를 구하기 위해서는 해당 사용자에 대해 단일한 변수로 원-핫-인코딩을 해줘야한다.
