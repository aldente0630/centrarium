...
암시적 피드백에 대해 순위를 학습하기 위한 고전적 방법은 논문 [BPR: 베이즈 개인화 순위 매기기](https://arxiv.org/ftp/arxiv/papers/1205/1205.2618.pdf)(pdf 링크)에 나와있다. 암시적 피드백, 행렬 분해 모든 면에서 대단한 Steffen Rendle이 이 논문의 주저자이다. *긍정적 품목*과 *부정적 품목*을 각기 표본 추출하고 쌍 별로 비교를 수행하는게 아이디어의 핵심이다. 이 예에서 데이터셋은 사용자가 웹사이트에서 다양한 항목을 클릭한 횟수로 구성된다. BPR은 (단순화된 형태로는) 다음과 같이 진행된다.
  
1.무작위로 사용자 \\(u\\)를 선택한 다음 사용자가 클릭한 임의의 품목 \\(i\\)를 선택한다. 이것이 *긍정적* 품목이다.  
2.사용자가 품목 \\(i\\)보다 *적은* 횟수로 클릭한 품목 \\(j\\)를 임의로 선택한다(여기에는 클릭한 적이 없는 품목도 포함됨). 이것이 *부정적* 품목이다.  
3.사용자 \\(u\\)와 긍정적 품목 \\(i\\)에 대한 "점수", \\(p_{ui}\\)를 예측하는 방정식을 계산한다. 행렬 분해의 경우 방정식은 \\(\mathbf{x}\_{u} \cdot \mathbf{y}\_{i}\\)일 수 있다.  
4.사용자 \\(u\\)와 부정적 품목 \\(j\\)에 대한 점수, \\(p_{uj}\\)를 예측한다.  
5.긍정적 품목과 부정적 품목 점수 간의 차이 \\(x_{uij} = p_{ui} - p_{uj}\\)를 구한다.  
6.이 차이를 시그모이드에 전달하고 확률적 경사 하강법(SGD)을 통해 모형의 모든 파라미터 업데이트하기 위한 가중치로 이용한다.  
  
이 방법론을 처음 봤을때 매우 급진적으로 보였다. 특히 예측하려는 점수 실제 값에 신경을 쓰지 않는 점 말이다. 신경 쓰는 건 사용자가 별로 클릭하지 않은  품목 대비 어떤 품목을 보다 자주 클릭했다고 순위를 매기는 부분이다. 따라서 모형은 "순위 매기는 법"을 배우게 된다. :)
  
다른 느린 방법과 비교하여 표본 추출 기반 접근법을 이용하기 때문에 저자는 이 모형이 상당히 빠르고 확장 가능하다는 사실을 보여준다. 또한, 저자들은 BPR이 ROC 곡선 아래 면적(AUC)을 직접 최적화한다고 주장한다. 이는 바람직한 특성일 수 있다. 가장 중요한 건 계산 속도를 뚝 떨어뜨리지 않고 부가 정보를 쉽게 추가할 수 있다는 것이다.
