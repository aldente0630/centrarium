...
이제 일부 모델들과 그와 연관된 추천들을 시각화해서 추천 시스템이 얼마나 잘 동작하는지 느껴보도록 하자. 모델의 섬네일을 가져 오기 위해 Sketchfab API에 간단히 질의하면 된다. 아래는 품목 유사도, 색인 그리고 색인 - `mid` 매퍼를 사용하여 추천의 섬네일 URL 목록을 반환하는 도우미 함수이다. 첫 번째 추천은 코사인 유사도가 1인 관계로 항상 본 모델 자신임을 유의하라.
...
Birchbox에서 일하기 시작했을 때 암묵적인 피드백 행렬 인수 분해 권장 시스템에 풍부한 고객 정보를 통합 할 수있는 방법을 모색해야했습니다. 나는 내가 무엇을하고 있는지 전혀 몰랐다. 그래서 나는 많은 구글 검색을했다. 이는 추천 시스템의 두 가지 패러다임, 즉 인구 통계 학적 고객 데이터가 있고 다른 유사 고객을 찾기 위해이 데이터를 사용할 때와 같은 콘텐츠 기반 접근 방식과 데이터가있을 때와 같은 "등급 기반"접근 방식의 두 가지 패러다임이 공통적으로 있기 때문에 어려운 문제였습니다. 각 사용자가 상호 작용 한 각 항목을 평가 한 내용 나는이 두 가지 방법과 결혼하기를 바랐다.
  
고전 설문 기사, Recommender Systems에 대한 매트릭스 요소 화 기법 (pdf 링크)을 읽으면서 "추가 입력 소스"섹션이 나왔습니다. 여기에는 소위 "부가 정보"를 추천 시스템에 통합하는 접근법이 포함되어 있습니다. 아이디어는 상대적으로 간단했다. (행렬 인수 분해에 대한 머리를 감싸는 초기 장애물과 관련하여 간단하다.) 정규 행렬 인수 분해 모델에서 사용자 uu가 단일 벡터 \ textbf {x} _ {u} x로 표현된다고 가정 해 봅시다
유
