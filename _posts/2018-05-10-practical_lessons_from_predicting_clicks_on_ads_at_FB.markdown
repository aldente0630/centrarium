...
위 결과에서 좌표 별 학습률을 적용한 SGD가 제일 높은 예측 정확도를 보였고 최저 성능을 기록한 가중치 별 학습률 경우보다 5% NE가 낮았다. 이 결과는 논문[^3]의 결론과 일치한다. 가중치 별 제곱근 또는 상수 학습률을 적용한 SGD의 NE는 비슷하거나 조금 높은 수준이다. 다른 두 체계는 훨씬 안좋다. 전역 학습률은 각 변수 별 훈련 샘플 수의 불균형으로 주로 실패한다. 각 훈련 샘플은 서로 다른 변수들로 구성될 수 있으므로 많이 사용하는 일부 변수는 다른 변수보다 훨씬 많은 훈련 샘플을 취한다. 전역 학습률 체계 하에서는 인스턴스 수가 적은 지형지 물에 대한 학습률이 너무 빨리 감소하고 최적의 가중치로 수렴하지 못합니다. 무게 당 학습률 체계는이 문제를 해결하지만 모든 기능의 학습 속도가 너무 빠르기 때문에 여전히 실패합니다. 모델이 최적 이하의 지점으로 수렴되는 곳에서는 훈련이 너무 일찍 종료됩니다. 이것은 왜이 계획이 모든 선택 중에서 최악의 성능을 갖는지를 설명합니다.
