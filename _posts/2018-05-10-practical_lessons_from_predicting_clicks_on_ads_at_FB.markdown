...
모델의 트리가 많을수록 예측에 필요한 시간이 길어집니다. 이 부분에서 우리는 추정 정확도에 부스트 된 나무의 수의 효과를 연구합니다.
  
우리는 나무 수를 1에서 2,000까지 다양하게하고 하루 종일 데이터 모델을 훈련시키고 다음날 예측 성능을 테스트합니다. 우리는 각 나무에 12 개가 넘는 나뭇잎이 없도록 제한합니다. 이전 실험과 유사하게 우리는 정규화 된 엔트로피를 평가 척도로 사용합니다. 실험 결과는 그림 5에 나와 있습니다. 정규화 된 엔트로피는 부스트 된 트리의 수를 늘리면 줄어 듭니다. 그러나 나무를 추가함으로써 얻을 수있는 이익은 수익을 감소시킵니다. 거의 모든 북동 개선은 최초의 500 그루 나무에서 비롯된 것입니다. 마지막 1, 000 그루의 나무가 0.1 % 이하로 NE를 감소시킵니다. 또한, 우리는 서브 모델 2에 대한 표준화 된 엔트로피가 1,000 그루의 나무 후에 퇴보하기 시작한다는 것을 알 수 있습니다. 이 현상의 원인은 오버 피팅입니다. 서브 모델 2에 대한 학습 데이터는 서브 모델 0 및 1에서의 학습 데이터보다 4 배 작기 때문에.

![그림 5](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB8.PNG)
**그림 5: 부스트 트리 수에 대한 실험 결과입니다. 다른 시리즈는 다른 서브 모델에 해당합니다. X 축은 부스팅 트리 수입니다. Y 축은 정규화 된 엔트로피입니다.** 
  
### 5.2 부스팅 변수 중요도
  
기능 수는 추정 정확도와 계산 성능 간의 절충에 영향을 줄 수있는 또 다른 모델 특성입니다. 기능 수의 영향을 더 잘 이해하기 위해 먼저 각 기능에 기능 중요성을 적용합니다.
