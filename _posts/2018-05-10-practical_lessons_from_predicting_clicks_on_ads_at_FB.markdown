...
이 절은 그림 1의 묘사처럼 부스팅 결정 트리와 확률론적 희소 선형 분류기를 연결한 하이브리드 모형 구조를 제시한다. 3.1절은 의사 결정 트리가 확률론적 선형 분류기의 정확도를 상당히 향상시키는 매우 강력한 입력 변수 변환임을 설명한다. 3.2절은 더 세밀한 훈련 데이터가보다 정확한 예측을 유도하는 방법을 보여줍니다. 이것은 선형 분류기를 훈련시키는 온라인 학습 방법을 사용하려는 동기를 부여합니다. 3.3 절에서는 확률 론적 선형 분류 자의 두 가지 패밀리에 대한 온라인 학습 변형을 비교합니다.
