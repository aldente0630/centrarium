...
엄격하고 통제 된 실험을 수행하기 위해 2013 년 4 분기 중 임의의 한 주를 선택하여 오프라인 교육 데이터를 준비했습니다. 다른 조건에서 동일한 교육 및 테스트 데이터를 유지하기 위해 오프라인 교육 데이터를 준비했습니다. 온라인으로 관찰. 저장된 오프라인 데이터를 교육 및 테스트로 분할하여 온라인 교육 및 예측을위한 스트리밍 데이터를 시뮬레이션합니다. 동일한 교육 / 테스트 데이터가이 문서의 모든 실험에 대한 테스트 베드로 사용됩니다.
  
평가 척도 : 우리는 기계 학습 모델에 대한 요인의 영향에 가장 관심이 있기 때문에 이익과 수익에 직접적으로 관련된 지표 대신 예측 정확도를 사용합니다. 이 작업에서는 표준화 엔트로피 (NE)와 교정을 주요 평가 척도로 사용합니다.
