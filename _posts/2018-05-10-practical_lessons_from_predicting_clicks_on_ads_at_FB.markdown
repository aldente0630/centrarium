...
클래스 불균형은 많은 연구자가 탐구했고 학습 모형 성과에 중대한 영향을 미치는 것으로 나타났다. 클래스 불균형 문제를 해결하기 위해 음의 레이블에 대한 다운 샘플링을 사용해보았다. 학습한 모형 예측 정확도를 테스트하기 위해 다양한 음의 레이블 다운 샘플링 비율을 실험적으로 실험합니다. 비율은 {0.1, 0.01, 0.001, 0.0001}로 다양합니다. 실험 결과는 그림 11과 같습니다.
