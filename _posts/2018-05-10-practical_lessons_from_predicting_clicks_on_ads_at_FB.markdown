...
클릭 예측 시스템은 종종 동적 환경에 배포됩니다. 여기서 데이터 분포는 시간이 지남에 따라 변합니다. 우리는 예측 성능에 대한 데이터 신선도 교육의 효과를 연구합니다. 이를 위해 우리는 특정한 날에 모델을 훈련시키고 연속적으로 그것을 테스트합니다. 우리는 강화 된 의사 결정 트리 모델과 트리 변형 된 입력 피쳐를 가진 logisitic 회귀 모델에 대해이 실험을 실행합니다.
  
이 실험에서는 하루 동안 데이터를 학습하고 6 일 연속으로 평가하고 각각에 대해 정규화 된 엔트로피를 계산합니다. 결과는 그림 2에 나와 있습니다.
  
![표 2](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB3.PNG)
  
**표 2: 수 일간의 훈련과 시험 사이의 지연의 함수로서의 예측 정확도. 정확도는 최악의 결과를 기준으로 정규화 된 엔트로피로 표시되며, 나무 전용 모델의 경우 6 일 지연됩니다.**
  
트레이닝과 테스트 세트 간의 지연이 증가함에 따라 예측 정확도가 두 모델 모두에서 분명히 저하됩니다. 두 모델 모두 주간 교육에서 매일 교육으로 이동하여 NE를 약 1 %까지 줄일 수 있음을 알 수 있습니다.
  
이러한 결과는 매일 재 훈련을받을만한 가치가 있음을 나타냅니다. 한 가지 옵션은 일일이 반복적으로 모델을 재 훈련하는 것입니다. 부스트 된 의사 결정 트리를 재 훈련하는 데 필요한 시간은 훈련 예 수, 트리 수, 각 트리의 리프 수, CPU, 메모리 등과 같은 요소에 따라 다릅니다. 부스팅 모델을 만드는 데 24 시간 이상 걸릴 수 있습니다 단일 코어 CPU로 수억 개의 인스턴스를 가진 수백 개의 나무가 있습니다. 실제적인 경우 전체 교육 세트를 보유하기 위해 많은 양의 메모리가있는 멀티 코어 시스템에서 충분한 동시성을 통해 몇 시간 내에 교육을 수행 할 수 있습니다. 다음 섹션에서는 대안을 고려합니다. 증가 된 의사 결정 트리는 매일 또는 며칠마다 훈련 될 수 있지만 선형 분류기는 온라인 학습의 약간의 풍미를 사용하여 거의 실시간으로 훈련 될 수 있습니다.
