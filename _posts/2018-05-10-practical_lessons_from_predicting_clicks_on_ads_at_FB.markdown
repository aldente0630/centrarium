...
*조정*은 평균 예측 CTR과 관측된 CTR 간의 비율이다. 즉, 실제로 관측된 클릭 수에 대한 예측 클릭 수 비율이다. 정확하고 잘 보정된 CTR 예측은 온라인 입찰 및 경매 성공에 필수이므로 조정은 매우 중요한 척도이다. 정 값이 1보다 작을수록 모델이 더 좋습니다. 우리는 교정에서 교정이 단지 사소한 경우에만보고합니다.
