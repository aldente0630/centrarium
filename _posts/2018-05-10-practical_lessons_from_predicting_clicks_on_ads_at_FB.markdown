...
\\(\mathbf{x}=(\mathbf{e}_{i_1}, ..., \mathbf{e})\\)의 함수로 주어진다. 여기서 \\(\mathbf{e_i}\\)는 i번째 단위 벡터이고 \\(i_1, ... , i_n\\)은 n개의 범주형 입력 변수이다. 클릭함 또는 클릭하지 않음을 나타내는 이진 레이블 \\(y_i \in \\{-1, +1\\}\\)을 훈련 단계에서 갖고 있다고 가정한다.
