...
데이터 분포가 시간이 흐름에 따라 변하는 동적인 환경에 클릭 예측 시스템을 종종 배포한다. 예측 성능에 대한 훈련 데이터 신선도 효과를 알아보았다. 이를 위해 특정일에 모형을 훈련시키고 후속 기간에 테스트를 했다. 부스팅 결정 트리 모형과 트리 변환한 입력 변수를 적용한 로지스틱 회귀 모형에 대해 실험을 실행했다.
  
실험은 하루 동안 데이터를 학습하고 6일 연속으로 평가하며 각 날짜에 대해 정규화 엔트로피를 계산했다. 결과는 그림 2에 나와있다.
