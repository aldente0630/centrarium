...
2장은 실험 설정에 대한 개요로 시작한다. 3장에서는 여러 가지 확률론적 선형 분류기와 다양한 온라인 학습 알고리즘을 평가한다. 변수 변환과 데이터 신선도 영향을 평가하기 위해 선형 분류기 관련 내용을 계속 진행할 것이다. 꽤 작은 크기의 모형을 만들면서 데이터 신선도와 온라인 학습 관련한 실제 결과에 영감을 특히 얻어 온라인 학습 계층을 통합한 모형 아키텍처를 제시할 것이다. 4장에서는 온라인 학습 계층의 핵심 구성 요소이자 실시간 훈련 데이터의 실시간 스트림을 생성시킬 수 있는 인프라 속 실험적인 부분, 즉 온라인 접합부에 대해 설명한다.
  
마지막으로 메모리와 정확도를 교환하여 계산 시간을 산출하고 엄청난 양의 훈련 데이터를 다룰 수 있는 방법을 제시한다. 5장은 방대한 규모의 응용 프로그램을 위해 메모리 및 대기 시간을 유지시키는 실용적인 방법을 설명하고 6장에서는 훈련 데이터 양과 정확도 사이의 균형 관계를 조사한다.
...
이 섹션에서는 온라인 학습을 통해 선형 분류기를 학습하는 데 사용되는 실시간 학습 데이터를 생성하는 실험 시스템을 소개합니다. 중요한 작업은 온라인 방식으로 교육 입력 (광고 노출)에 레이블 (클릭 / 클릭하지 않음)을 결합하는 것이므로이 시스템을 "온라인 소목 장이"라고합니다. 유사한 인프라가 예를 들어 Google 광고 시스템에서 스트림 학습에 사용됩니다 [1]. 온라인 조이너는 Scribe [10]라는 인프라에 실시간 교육 데이터 스트림을 출력합니다. 긍정적 인 레이블 (클릭)은 잘 정의되어 있지만 사용자가 누를 수있는 "클릭하지 않음"버튼은 없습니다. 따라서 사용자가 광고를 본 후 일정 기간 동안 충분히 긴 시간 후에 광고를 클릭하지 않으면 노출 수가 음수 인 클릭이없는 것으로 간주됩니다. 대기 시간 창의 길이는 조심스럽게 조정해야합니다.
  
너무 긴 대기 창을 사용하면 실시간 교육 데이터가 지연되고 클릭 신호를 기다리는 동안 버퍼링 노출에 할당 된 메모리가 증가합니다. 시간이 너무 짧으면 해당 노출이 플러시되고 클릭되지 않은 것으로 분류되어 클릭 수 중 일부가 손실됩니다. 이는 '클릭률'에 부정적인 영향을줍니다. 즉 클릭에 성공적으로 연결된 모든 클릭의 비율입니다. 결과적으로, 온라인 소목 장이 시스템은 최신 성 및 클릭 적용 범위 사이의 균형을 유지해야합니다.
  
전체 클릭 커버리지가 없다는 것은 실시간 훈련 세트가 편향 될 것이라는 것을 의미합니다. 경험적 클릭률은 실제 진실보다 다소 낮습니다. 이는 대기 시간이 충분히 길면 클릭하지 않은 것으로 표시된 라벨의 일부가 클릭으로 라벨이 지정 되었기 때문입니다. 그러나 실제로는 관리 가능한 메모리 요구 사항을 초래하는 대기 창 크기로 백분율의 소수점 이하로 편향되는 것을 쉽게 줄일 수 있다는 것을 발견했습니다. 또한이 작은 바이어스를 측정하고 수정할 수 있습니다. 창 크기와 효율에 대한 더 많은 연구는 [6]에서 찾을 수 있습니다. 온라인 소목 장이는 요청 ID를 결합 술어의 기본 구성 요소로 사용하여 광고 노출 및 광고 클릭에 대한 분산 스트림 - 스트림 결합을 수행하도록 설계되었습니다. 요청 ID는 사용자가 노출 된 콘텐츠의 새로 고침을 트리거하는 Facebook에서 작업을 수행 할 때마다 생성됩니다. 필연적 인 온라인 학습을위한 도식적 인 데이터 및 모델 흐름은 그림 4에 나와 있습니다. 사용자가 Facebook을 방문하고 후보 광고에 대한 요청이 행거에 요청되면 초기 데이터 스트림이 생성됩니다. 광고는 사용자 기기로 다시 전달되며 각 광고와 해당 노출 순위에 사용 된 관련 기능이 노출 스트림에 추가됩니다. 사용자가 광고를 클릭하면 해당 클릭이 클릭 스트림에 추가됩니다. Stream-to-Stream Join을 실현하기 위해서, 시스템은 First-InFirst-Out 큐를 버퍼 윈도우로서 구성하는 HashQueue와, 라벨의 임프레션에의 고속 랜덤 액세스를위한 해시 맵을 사용합니다. 일반적으로 HashQueue는 키 - 값 쌍에 대해 세 가지 종류의 작업을 수행합니다. enqueue, dequeue 및 lookup입니다. 예를 들어 항목을 대기열에 추가하려면 항목을 대기열 앞에 추가하고 해시지도에서 대기열의 항목을 가리키는 값으로 키를 만듭니다. 전체 결합 창이 만료 된 후에 만 레이블이 지정된 인상이 교육 스트림에 방출됩니다. 조인되지 않은 클릭은 음수로 표시된 예제로 방출됩니다.
  
이 실험 설정에서 트레이너는 교육 스트림에서 지속적으로 학습하고 새 모델을 정기적으로 Ranker에 게시합니다. 이는 궁극적으로 피쳐 분포 또는 모델 성능의 변화를 짧은 시간에 포착, 학습 및 교정 할 수있는 기계 학습 모델을위한 엄격한 폐쇄 루프를 형성합니다.
  
실시간 교육 데이터 생성 시스템을 실험 할 때 중요한 고려 사항 중 하나는 온라인 학습 시스템을 손상시킬 수있는 변형에 대한 보호 메커니즘을 구축해야한다는 것입니다. 간단한 예를 들어 보겠습니다. 일부 데이터 인프라 문제로 인해 클릭 스트림이 부실 해지면 온라인 조이너는 경험적 CTR이 매우 작거나 0 인 교육 데이터를 생성합니다. 따라서 실시간 트레이너는 클릭 빈도가 매우 낮거나 0에 가깝게 예측하기 시작합니다. 광고의 예상 가치는 예상 클릭 확률에 따라 달라 지므로 클릭률 (CTR)이 매우 낮다고 잘못 예측하면 광고 노출 수가 줄어들 수 있습니다. 이상 탐지 메커니즘이 도움이 될 수 있습니다. 예를 들어 실시간 교육 데이터 배포가 갑자기 변경되면 온라인 조이너와 자동으로 연결을 끊을 수 있습니다.
