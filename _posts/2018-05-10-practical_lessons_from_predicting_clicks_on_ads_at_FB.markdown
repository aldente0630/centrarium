...
문맥 적 특징이 없으면 예측 정확도가 4.5 % 감소합니다. 반대로 문맥 적 특징이 없으면 예측 정확도가 1 % 미만으로 떨어집니다.
  
콜드 스타트 문제를 처리하려면 상황 별 기능이 매우 중요하다는 사실을 알아야합니다. 신규 사용자 및 광고의 경우 클릭률 예측에있어 문맥 적 기능이 필수적입니다.
  
다음 단계에서는 연속 된 주간의 기록 기능 만 또는 상황 별 기능을 사용하여 훈련 된 모델을 평가하여 데이터 최신성에 대한 기능 종속성을 테스트합니다. 결과는 그림 9에 나와 있습니다.
  
![그림 9](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB12.PNG)
**그림 9: 다른 유형의 기능에 대한 데이터 빈도 결과입니다. X 축은 평가 날짜이고 y 축은 정규화 된 엔트로피입니다.** 
  
그림에서 볼 수 있듯이 상황 별 기능이있는 모델은 과거 기능보다 데이터 신선도에 더 많이 의존합니다. 역사적 특징은 오랜 시간 축적 된 사용자 행동을 묘사하기 때문에 우리의 직감과 일치합니다.
