...
데이터 분포가 시간이 흐름에 따라 변하는 동적인 환경에 클릭 예측 시스템을 종종 배포한다. 예측 성능에 대한 훈련 데이터 신선도 효과를 알아보았다. 이를 위해 우리는 특정한 날에 모델을 훈련시키고 연속적으로 그것을 테스트합니다. 우리는 강화 된 의사 결정 트리 모델과 트리 변형 된 입력 피쳐를 가진 logisitic 회귀 모델에 대해이 실험을 실행합니다.
