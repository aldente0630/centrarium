...
2장은 실험 설정에 대한 개요로 시작한다. 3장에서는 여러 가지 확률론적 선형 분류기와 다양한 온라인 학습 알고리즘을 평가한다. 변수 변환과 데이터 신선도 영향을 평가하기 위해 선형 분류기 관련 내용을 계속 진행할 것이다. 꽤 작은 크기의 모형을 만들면서 데이터 신선도와 온라인 학습 관련한 실제 결과에 영감을 특히 얻어 온라인 학습 계층을 통합한 모형 아키텍처를 제시할 것이다. 4장에서는 온라인 학습 계층의 핵심 구성 요소이자 실시간 훈련 데이터의 실시간 스트림을 생성시킬 수 있는 인프라 속 실험적인 부분, 즉 온라인 조이너에 대해 설명한다.
  
마지막으로 메모리와 정확도를 교환하여 계산 시간을 산출하고 대규모 훈련 데이터를 다룰 수 있는 방법을 제시한다. 5장은 방대한 규모의 응용 프로그램을 위해 메모리 및 대기 시간을 유지시키는 실용적인 방법을 설명하고 6장에서는 훈련 데이터 양과 정확도 사이의 균형 관계를 조사한다.
  
## 2. 실험 설정
  
엄격하고 통제된 실험을 수행하기 위해 2013년 4분기 중 한 주를 임의로 선택하여 오프라인 훈련 데이터 준비를 했다. 동일한 훈련 및 테스트 데이터를 상이한 조건에서 가져가기 위해 온라인에서 관측한 것과 비슷하게 오프라인 훈련 데이터를 준비했다. 저장한 오프라인 데이터를 훈련과 테스트 용으로 분할하여 온라인 교육과 예측을 위한 스트리밍 데이터로 시뮬레이션했다. 이 논문의 모든 실험에 동일한 훈련/테스트 데이터를 테스트베드로 이용했다.
  
**평가 척도:** 기계 학습 모형에 영향을 주는 요인에 대해 가장 큰 관심이 있기에 이익과 수익에 직접 관련된 지표 대신 예측 정확도를 사용한다. 본 작업에서는 정규화 엔트로피(NE)와 보정을 주요 평가 척도로 사용한다.
...
훈련과 테스트 세트 간 지연 일수가 늘면서 두 모형 모두 예측 정확도가 명백히 떨어진다. 두 모형 모두 주 단위 훈련 대비 일 단위 훈련할 경우 NE가 약 1%까지 감소함을 알 수 있다.
...
변수 중요도 측정을 목적으로 특정 변수로 인해 손실이 얼마나 누적 감소하는지 잡아내는 통계량 *부스팅 변수 중요도*를 사용했다. 각 트리 노드를 구성할 때 제곱 오차를 최대로 감소시키는 최적 변수를 선택하고 분할한다. 한 변수가 여러 트리에서 사용될 수 있으므로 특정 변수에 대한 부스팅 피쳐 중요도는 모든 트리에서 각 변수가 감소시킨 양을 합하여 결정한다.
...
**그림 6: 부스팅 변수 중요도. X축은 변수 개수에 해당한다. 좌측 y 주축은 변수 중요도를 로그 척도로 그리고 우측 y 보조축은 누적 변수 중요도를 표시한다.** 
...
**그림 7: 상위 변수들로 모형을 부스팅한 결과. 좌측 y 주축에 눈금을 그렸고 우측 y 보조축에 표준화 엔트로피를 표시했다.** 
...
![그림 9](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB12.PNG)
...
Facebook 광고 노출 데이터의 하루 종일에는 엄청난 양의 인스턴스가 포함될 수 있습니다. 우리는 비밀 번호이므로 실제 번호를 밝힐 수 없습니다. 그러나 하루 분량의 데이터 중 적은 수의 인스턴스가 수억 개가 될 수 있습니다. 교육 비용을 관리하는 데 사용되는 일반적인 기술은 교육 데이터의 양을 줄이는 것입니다. 이 섹션에서는 다운 샘플링 데이터, 균일 서브 샘플링 및 음수 다운 샘플링을위한 두 가지 기술을 평가합니다. 각각의 경우에 우리는 600 나무를 가진 부스트 된 나무 모델 세트를 훈련시키고 교정과 정규화 엔트로피 모두를 사용하여이를 평가합니다.

### 6.1 균등 서브샘플링
  
교육 행의 균일 한 서브 샘플링은 구현하기 쉽고 결과 모델을 서브 샘플링 된 학습 데이터와 서브 샘플링되지 않은 테스트 데이터 모두에서 수정없이 사용할 수 있기 때문에 데이터 볼륨을 줄이기위한 유 용한 접근 방법입니다. 이 부분에서는 대략 기하 급수적으로 증가하는 서브 샘플링 비율을 평가합니다. 각 비율에 대해 기본 데이터 집합에서 해당 비율로 샘플링 된 부스트 트리 모델을 학습합니다. 서브 샘플링 비율은 {0.001, 0.01, 0.1, 0.5, 1}로 다양합니다.
  
데이터 볼륨의 결과는 그림 10에 나와 있습니다. 데이터가 많을수록 성능이 향상됩니다. 또한 데이터 볼륨은 예측 정확도 측면에서 수익 감소를 나타냅니다. 데이터의 10 % 만 사용함으로써 정규화 된 엔트로피는 전체 교육 데이터 세트에 비해 성능이 1 % 감소합니다. 이 샘플링 속도로 보정하면 성능이 저하되지 않습니다.

![그림 10](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB13.PNG)
**그림 10: 데이터 볼륨에 대한 실험 결과입니다. X 축은 트레이닝 인스턴스의 수에 해당합니다. 왼쪽 1 차 y 축에 보정을 그리고 표준화 된 엔트로피는 오른쪽 2 차 y 축에 표시됩니다.** 

### 6.2 음의 레이블에 대한 다운 샘플링
  
수업 불균형은 많은 연구자들에 의해 연구되었으며 학습 된 모델의 성과에 중요한 영향을 미치는 것으로 나타났습니다. 이 파트에서는 클래스 불균형 문제를 해결하기 위해 음수 다운 샘플링을 사용합니다. 학습 된 모델의 예측 정확도를 테스트하기 위해 다양한 음의 다운 샘플링 속도를 실험적으로 실험합니다. 비율은 {0.1, 0.01, 0.001, 0.0001}로 다양합니다. 실험 결과는 그림 11과 같습니다.
  
![그림 11](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB14.PNG)
**그림 11: 음수 다운 샘플링의 실험 결과입니다. X 축은 서로 다른 음의 다운 샘플링 속도에 해당합니다. 왼쪽 1 차 y 축에 보정을 그리고 표준화 된 엔트로피는 오른쪽 2 차 y 축에 표시됩니다.** 
  
결과에서 음의 다운 샘플링 속도는 훈련 된 모델의 성능에 중요한 영향을 미친다는 것을 알 수 있습니다. 음의 다운 샘플링 속도를 0.025로 설정하면 최상의 성능을 얻을 수 있습니다.

### 6.3 모형 재보정
  
음수 다운 샘플링은 교육 속도를 높이고 모델 성능을 향상시킬 수 있습니다. 음수 다운 샘플링을 사용하는 데이터 세트에서 모델을 학습하면 모델은 또한 다운 샘플링 공간에서 예측을 보정합니다. 예를 들어, 샘플링 전 평균 CTR이 0.1 %이고 0.01 음수 다운 샘플링을 수행하면 경험적 CTR은 대략 10 %가됩니다. 실시간 트래픽 실험을 위해 모델을 다시 조정하고 \\q = {q \over p + (1 - p) / w}\\)로 0.1 % 예측으로 돌아 가야합니다. 여기서 \\(p\\)는 다운 샘플링 공간에서의 예측이고 \\(w\\)는 음의 다운 샘플링 속도입니다.

## 7. 논의
  
우리는 페이스 북 광고 데이터를 실험하여 몇 가지 실용적인 교훈을 제시했습니다. 이것은 클릭 예측을위한 유망한 하이브리드 모델 아키텍처에 영감을주었습니다.
  
* 데이터 신선도가 중요합니다. 적어도 일일 재교육을받을 가치가 있습니다. 이 백서에서는 다양한 온라인 학습 계획에 대해 더 자세히 논의하고 논의했습니다. 또한 실시간 교육 데이터를 생성 할 수있는 인프라를 제시했습니다.
  
* 부스트 된 결정 트리를 사용하여 실수 값 입력 특성을 변환하면 확률 론적 선형 분류기의 예측 정확도가 크게 향상됩니다. 이는 증가 된 의사 결정 트리와 스파 스 선형 분류기를 연결하는 하이브리드 모델 아키텍처에 동기를 부여합니다.

* 최고의 온라인 학습 방법 : 좌표 별 학습 속도가있는 LR. BOPR과 성능이 비슷하며 학습중인 다른 모든 LR SGD 체계보다 뛰어납니다. (표 4,도 12)
  
대규모 기계 학습 응용 프로그램에 메모리와 대기 시간을 유지하는 기술을 설명했습니다.
  
* 우리는 증가 된 의사 결정 트리의 수와 정확성 간의 균형을 제시했습니다. 계산 및 메모리를 유지하기 위해 트리의 수를 작게 유지하는 것이 유리합니다.
  
* 부스트 된 의사 결정 트리는 기능 중요성을 통해 기능 선택을 수행하는 편리한 방법을 제공합니다. 능동적 인 기능의 수를 적극적으로 줄이는 동시에 예상 정확도를 보통 수준으로 낮출 수 있습니다.

* 우리는 역사적 특징을 문맥 특징과 결합하여 사용하는 효과를 분석했다. 기록이있는 광고 및 사용자의 경우이 기능은 컨텍스트 기능보다 뛰어난 예측 성능을 제공합니다.
  
마지막으로, 우리는 훈련 데이터를 서브 샘플링하는 방법을 일률적으로 또한보다 흥미롭게도 편향된 방식으로 논의 할 것이고 여기서 오직 부정적 예만이 서브 샘플링된다.
