...
위의 결과에서 좌표 별 학습 속도가있는 SGD가 가장 정확한 예측 정확도를 얻었으며 NE는 가중치 학습 속도를 사용할 때보 다 5 % 더 낮았으며 최악의 결과를 보였습니다. 이 결과는 [8]의 결론과 일치합니다. 가중치 제곱근 및 일정한 학습률을 가진 SGD는 NE와 비슷하고 약간 악화됩니다. 다른 두 가지 계획은 이전 버전보다 심각합니다. 글로벌 학습률은 주로 각 기능에 대한 교육 인스턴스 수의 불균형으로 인해 실패합니다.
각 교육 인스턴스는 서로 다른 기능으로 구성 될 수 있으므로 일부 인기있는 기능은 다른 기능보다 훨씬 많은 교육 인스턴스를 수신합니다. 글로벌 학습률 체계 하에서는 인스턴스 수가 적은 지형지 물에 대한 학습률이 너무 빨리 감소하고 최적의 가중치로 수렴하지 못합니다. 무게 당 학습률 체계는이 문제를 해결하지만 모든 기능의 학습 속도가 너무 빠르기 때문에 여전히 실패합니다. 모델이 최적 이하의 지점으로 수렴되는 곳에서는 훈련이 너무 일찍 종료됩니다. 이것은 왜이 계획이 모든 선택 중에서 최악의 성능을 갖는지를 설명합니다.
  
흥미로운 점은 평균에 대한 BOPR 업데이트 수식 (3)은 LR에 대한 SGD의 좌표 별 학습률 버전과 가장 유사하다는 점입니다. BOPR의 효과적인 학습률은 각 좌표에 따라 다르며 각 좌표에 연관된 가중치의 사후 변화와 모델이 예측 한 바에 따른 레이블의 "놀라움"에 달려있다.
  
우리는 훈련 된 LR의 예측 성능을 좌표 당 SGD와 BOPR과 비교하기위한 실험을 수행합니다. 우리는 LR 및 BOPR 모델을 동일한 교육 데이터로 교육하고 다음 날 예측 성능을 평가합니다. 그 결과를 표 3에 나타낸다.
