...
이 절에서 우리는 하이브리드 모형 구조를 제시한다. 그림 1에서 설명한 바와 같이 부스팅 결정 트리와 확률 론적 희소 선형 분류기의 연결. 3.1 절에서 우리는 의사 결정 트리가 매우 강력한 입력 특징 변환임을 보여 주며, 확률 론적 선형 분류 자. 3.2 절에서는 더 세밀한 훈련 데이터가보다 정확한 예측을 유도하는 방법을 보여줍니다. 이것은 선형 분류기를 훈련시키는 온라인 학습 방법을 사용하려는 동기를 부여합니다. 3.3 절에서는 확률 론적 선형 분류 자의 두 가지 패밀리에 대한 온라인 학습 변형을 비교합니다.
