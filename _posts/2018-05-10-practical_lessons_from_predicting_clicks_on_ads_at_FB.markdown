...
다음 단계로 연속 된 주간의 이력 변수만 또는 맥락 변수만 사용하여 훈련시킨 모형을 평가하여 데이터 신선도에 대한 변수 종속성을 테스트했다. 결과는 그림 9에 나와있다.
  
![그림 9](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB12.PNG)
**그림 9: 다른 유형의 기능에 대한 데이터  결과입니다. X 축은 평가 날짜이고 y 축은 정규화 된 엔트로피입니다.** 
  
그림에서 볼 수 있듯이 맥락 변수를 사용한 모형이 이력 변수를 사용한 것보다 데이터 신선도에 더 많이 의존한다. 이력 변수는 사용자 행동을 오랜 기간 축적하여 표현하기에 이 결과가 직관적으로 옳음을 알 수 있다.
