...
ROC 아래 면적(AUC) 또한 보정을 고려하지 않고 순위 품질을 측정하는데 매우 좋은 척도이다. 실제 환경에서는 잠재적 인 미달 게재 또는 초과 게재를 피하기 위해 최적 순서만 지정하는 대신 예측이 정확할 것으로 기대합니다. NE는 예측의 우수성을 측정하고 보정을 암시 적으로 반영합니다. 예를 들어, 모델이 2 배의 과대 예측을하고 보정을 위해 글로벌 배율 0.5를 적용하면 AUC가 동일하게 유지 되더라도 해당 NE가 향상됩니다. 이러한 측정 기준에 대한 심도있는 연구는 ...를 참조하십시오.
