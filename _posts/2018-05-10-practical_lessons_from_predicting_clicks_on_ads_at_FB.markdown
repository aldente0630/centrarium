...
정규화 엔트로피 또는 보다 정확하게 정규화 크로스 엔트로피는 노출 당 평균 로그 손실을 모형이 모든 노출에 대한 배경 클릭률 (CTR)을 예측 한 경우 평균 로그 손실이 될 것으로 나눈 값과 같습니다. 즉, 배경 CTR의 엔트로피로 정규화 된 예측 로그 손실입니다. 백그라운드 CTR은 교육 데이터 세트의 평균 경험적 CTR입니다. 메트릭을 정규화 된 로그 손실이라고하는 것이 더 이해하기 쉽습니다. 값이 낮을수록 모델에 의한 예측이 더 좋습니다. 이 정규화의 이유는 배경 CTR이 0 또는 1에 가까울수록 로그 손실을 더 쉽게 달성 할 수 있기 때문입니다. 배경 CTR의 엔트로피로 나누면 NE는 배경 CTR에 둔감합니다.
