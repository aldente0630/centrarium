...
Varian 또는 Edelman 등이 쓴 2007년 논문은 Google과 Yahoo!가 개척한 클릭마다 입찰하고 지불하는 경매 시스템을 설명한다. 같은 해 Microsoft는 동일한 경매 모형을 기반으로 스폰서 검색 시장을 구축했다. 광고 입찰의 효율성은 클릭 예측의 정확도 및 조정에 달려있다. 클릭 예측 시스템은 견고하고 적응력이 있어야하며 대량의 데이터를 학습할 수 있어야한다. 본 논문의 목표는 이런 요구 사항을 염두에 두고 실제 데이터로 수행한 실험에서 얻은 통찰을 공유하는 일이다.
...
정규화 된 엔트로피 또는보다 정확하게, 정규화 된 교차로 지수는 노출 당 평균 로그 손실을 모델이 모든 노출에 대한 배경 클릭률 (CTR)을 예측 한 경우 평균 로그 손실이 될 것으로 나눈 값과 같습니다. 즉, 배경 CTR의 엔트로피로 정규화 된 예측 로그 손실입니다. 백그라운드 CTR은 교육 데이터 세트의 평균 경험적 CTR입니다. 메트릭을 정규화 된 로그 손실이라고하는 것이 더 이해하기 쉽습니다. 값이 낮을수록 모델에 의한 예측이 더 좋습니다. 이 정규화의 이유는 배경 CTR이 0 또는 1에 가까울수록 로그 손실을 더 쉽게 달성 할 수 있기 때문입니다. 배경 CTR의 엔트로피로 나누면 NE는 배경 CTR에 둔감합니다.
  
주어진 훈련 데이터 세트가 레이블 yi ∈ {-1, +1}과 클릭 확률 pi (i = 1, 2, ... N)의 N 개의 예제. 경험적 평균 CTR은 p


NE는 기본적으로 상대 정보를 계산하는 구성 요소입니다. 이득 (RIG) 및 RIG = 1 - NE
