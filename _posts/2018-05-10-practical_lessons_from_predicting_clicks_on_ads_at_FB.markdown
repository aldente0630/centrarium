...
변수 중요도를 측정하기 위해 특정 변수로 인해 손실이 누적으로 얼마나 감소하는지 담아내는 통계량 *부스팅 변수 중요도*를 사용했다. 각 트리 노드를 구성할 때 제곱 오차를 최대로 감소시키는 최적 변수를 선택하고 분할한다. 한 변수가 여러 트리에서 사용될 수 있으므로 특정 변수에 대한 부스팅 피쳐 중요도는 모든 트리에서 각 변수가 감소시킨 양을 합하여 결정한다.
  
일반적으로 변수 일부가 설명력 대부분의 설명력에 기여하는 반면 나머지 기능에는 한계가 있습니다. 그림 6에서 누적 피쳐 중요도 대 피쳐 수를 플로팅 할 때 이와 동일한 패턴을 볼 수 있습니다.
