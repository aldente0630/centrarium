...
변수 개수는 추정 정확도와 계산 성능 간 절충에 영향을 줄 수 있는 또 다른 모형 특징이다. 변수 개수의 영향을 더 잘 이해하기 위해 각 변수마다 변수 중요도를 먼저 산출해보았다.
  
변수 중요도를 측정하기 위해 특정 변수로 손실이 얼마나 누적 감소하는지 잡아내는 통계량 *부스팅 변수 중요도*를 사용했다. 각 트리 노드를 구성할 때 제곱 오차를 최대로 감소시키는 최적 변수를 선택하고 분할한다. 한 변수가 여러 트리에서 사용될 수 있으므로 특정 변수에 대한 부스팅 피쳐 중요도는 모든 트리에서 각 변수가 감소시킨 양을 합하여 결정한다.
...
위 결과에서 상위 10개 변수가 전체 변수 중요도의 약 절반을 차지하는 반면 마지막 300개 변수은 변수 중요도가 1% 미만임을 알 수 있다. 이 결과를 바탕으로 상위 10, 20, 50, 100 및 200 기능 만 유지하고 성능이 어떻게 영향을 받는지 평가합니다. 실험 결과는 그림 7에 나와 있습니다. 그림에서 표준화 된 엔트로피는 더 많은 기능을 포함 할 때와 비슷한 감소하는 반환 특성을 나타냅니다.
