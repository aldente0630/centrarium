...
데이터 볼륨의 결과는 그림 10에 나와 있습니다. 데이터가 많을수록 성능이 향상됩니다. 또한 데이터 볼륨은 예측 정확도 측면에서 수익 감소를 나타냅니다. 데이터의 10 % 만 사용함으로써 정규화 된 엔트로피는 전체 교육 데이터셋에 비해 성능이 1 % 감소합니다. 이 샘플링 속도로 보정하면 성능이 저하되지 않습니다.
...
음수 다운 샘플링은 교육 속도를 높이고 모델 성능을 향상시킬 수 있습니다. 음수 다운 샘플링을 사용하는 데이터셋에서 모델을 학습하면 모델은 또한 다운 샘플링 공간에서 예측을 보정합니다. 예를 들어, 샘플링 전 평균 CTR이 0.1 %이고 0.01 음수 다운 샘플링을 수행하면 경험적 CTR은 대략 10 %가됩니다. 실시간 트래픽 실험을 위해 모델을 다시 조정하고 \\(q = {q \over p + (1 - p) / w}\\)로 0.1 % 예측으로 돌아 가야합니다. 여기서 \\(p\\)는 다운 샘플링 공간에서의 예측이고 \\(w\\)는 음의 다운 샘플링 속도입니다.
