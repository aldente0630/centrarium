...
\\(\mathbf{x}=(\mathbf{e}_{i_1}, \ldots, \mathbf{e}_{i_n})\\)의 함수로 주어진다. 여기서 \\(\mathbf{e_i}\\)는 i번째 단위 벡터이고 \\(i_1, ... , i_n\\)은 n개의 범주형 입력 변수이다. 트레이닝 단계에서 우리는 또한 클릭 또는 클릭을 나타내는 이진 레이블 y∈ {+1, -1}을 가정합니다.
