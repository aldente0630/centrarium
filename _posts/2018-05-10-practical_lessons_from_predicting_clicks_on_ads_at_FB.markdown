...
여기서 \\(g\\)는 0이 아닌 모든 성분의 로그 우도에 관한 기울기이고 \\(g(s) := \[y(y + 1) / 2 - y \cdot sigmoid(s)]\\)로 구할 수 있다. 식 (3)은  신뢰 불확실성 \\(\sigma\\)이 보폭 \\(\eta_{i_j}\\)을 자동으로 제어하는, (6) 같은 형태의 평균 벡터 \\(\mu\\)에 관한 좌표 별 기울기 방향으로 볼 수 있다. 3.3절에서 다양한 보폭 함수 \\(\eta\\)를 제시하고 BOPR과 비교할 것이다.
  
위에서 설명한 SGD 기반 LR과 BOPR 모두 훈련 데이터 하나 하나에 적응하는 스트림 학습기이다.

### 3.1 의사 결정 트리 변수 변환

선형 분류기 입력 변수를 변환하여 정확성 향상시키는 방법은 간단하게 두 가지가 있다. 연속형 변수의 경우 비선형 변환을 학습하기위한 간단한 트릭은 피쳐를 비우고 빈 인덱스를 범주 형 피쳐로 처리하는 것입니다. 선형 분류기는 피쳐에 대해 조각 별 일정한 비선형 맵을 효과적으로 습득합니다. 유용한 bin 경계를 익히는 것이 중요하며이를 수행하는 방법을 최대화하는 많은 정보가 있습니다.
  
두 번째 간단하지만 효과적인 변환은 튜플 입력 기능을 구성하는 것입니다. 범주 형 기능의 경우, 무차별 방식은 데카르트 제품을 취하는 것, 즉 원래 기능의 가능한 모든 값을 값으로 취하는 새로운 범주 형 기능을 만드는 데 있습니다. 모든 조합이 유용하지는 않지만 유용하지 않은 조합은 제거 할 수 있습니다. 입력 특징이 연속적이라면, 예를 들어 k-d 트리를 사용하여 공동 비닝을 수행 할 수 있습니다.
  
우리는 증가 된 의사 결정 트리가 방금 설명한 종류의 비선형 및 튜플 변환을 구현하는 강력하고 편리한 방법이라는 것을 알았습니다. 우리는 각 개인을
트리는 인스턴스가 끝나는 리프의 인덱스를 값으로 취하는 범주 적 기능입니다.이 유형의 기능에 대해 1-of-K 코딩을 사용합니다. 예를 들어 첫 번째 하위 트리가 3 개의 리프가 있고 두 번째 하위가 2 개의 리프가있는 2 개의 서브 트리가있는 그림 1의 부스트 트리 모델을 생각해보십시오. 인스턴스가 첫 번째 하위 트리의 리프 2에서 끝나고 두 번째 하위 트리의 리프 1에서 끝나는 경우 선형 분류기에 대한 전체 입력은 이진 벡터 \\(\[0, 1, 0, 1, 0]\\)가됩니다. 여기서 첫 번째 3 개의 항목은 첫 번째 하위 트리의 잎과 두 번째 하위 트리의 잎의 잎이 있습니다. 우리가 사용하는 boosted decision trees는 전통적인 \\(L_2\\)-TreeBoost 알고리즘이 사용되는 Gradient Boosting Machine (GBM) [5]을 따릅니다. 각 학습 반복에서, 잔여를 모델링하기 위해 새로운 트리가 생성된다
이전 나무들. 우리는 실수 값 벡터를 컴팩트 이진 값 벡터로 변환하는 감독 된 특징 인코딩으로 승격 된 의사 결정 트리 기반 변환을 이해할 수 있습니다. 루트 노드에서 리프 노드로의 순회는 특정 기능에 대한 규칙을 나타냅니다. 이진 벡터에 선형 분류자를 적용하는 것은 본질적으로 규칙 집합에 대한 가중치를 학습하는 것입니다. 부스트 된 의사 결정 트리는 배치 방식으로 학습됩니다.
  
선형 모델에 대한 입력으로서 트리 피처를 포함시키는 효과를 보여주기위한 실험을 수행합니다. 이 실험에서는 두 가지 로지스틱 회귀 모델을 비교합니다. 하나는 트리 피쳐 변환이고 다른 하나는 일반 (변형되지 않은) 피쳐입니다. 비교를 위해서만 boosted decision tree 모델을 사용합니다. 그 결과를 표 1에 나타낸다.
  
트리 특성 변환은 트리 변환이없는 모델의 정규화 엔트로피에 비해 표준화 엔트로피를 3.4 % 이상 감소시킵니다. 이는 매우 중요한 개선점입니다. 참고로, 전형적인 피쳐 엔지니어링 실험은 상대 NE의 수십 퍼센트를 줄입니다. 고립되어 사용되는 LR 및 Tree 모델은 예측 정확도 (LR이 조금 더 좋음)와 비교할 만하지만 정확도의 도약을 이끌어내는 조합이라는 점을 보면 흥미 롭습니다. 예측 정확도의 향상은 중요합니다. 참고로, 피쳐 엔지니어링 실험의 대부분은 Normalized Entropy를 백분율로 감소시킵니다.
