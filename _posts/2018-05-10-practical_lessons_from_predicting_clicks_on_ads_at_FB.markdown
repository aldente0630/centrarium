...
Varian 또는 Edelman 등이 쓴 2007년경 논문은 Google과 Yahoo!가 개척한 클릭마다 입찰하고 지불하는 경매 시스템을 설명한다. 같은 해 Microsoft는 동일한 경매 모형을 기반으로 스폰서 검색 시장을 구축했다. 광고 입찰의 효율성은 클릭 예측의 정확도 및 조정에 달려있다. 클릭 예측 시스템은 견고하고 적응력이 있어야하며 대량의 데이터를 학습할 수 있어야한다. 본 논문의 목표는 이런 요구 사항을 염두에 두고 실제 데이터로 수행한 실험에서 얻은 통찰을 공유하는 일이다.
...
**평가 척도:** 기계 학습 모형에 영향을 주는 요인에 대해 가장 큰 관심이 있기에 이익과 수익에 직접 관련된 지표 대신 예측 정확도를 사용한다. 본 작업에서는 정규화 엔트로피(NE)와 조정을 주요 평가 척도로 사용한다.
...
*조정*은 평균 예상 CTR과 경험 CTR의 비율입니다. 즉, 실제로 관찰 된 클릭 수에 대한 예상 클릭 수의 비율입니다.
정확하고 잘 보정 된 클릭률 (CTR) 예측은 온라인 입찰 및 경매의 성공에 필수적이므로 조정은 매우 중요한 척도입니다. 보정 값이 1보다 작을수록 모델이 더 좋습니다. 우리는 교정에서 교정이 단지 사소한 경우에만보고합니다.
  
AUC (Area-Under-ROC)는 또한 보정을 고려하지 않고 순위 품질을 측정하는 데 매우 좋은 측정 기준입니다. 현실적인 환경에서는 잠재적 인 미달 게재 또는 초과 게재를 피하기 위해 최적의 순서 만 지정하는 대신 예측이 정확할 것으로 기대합니다. NE는 예측의 우수성을 측정하고 보정을 암시 적으로 반영합니다. 예를 들어, 모델이 2 배의 과대 예측을하고 보정을 위해 글로벌 배율 0.5를 적용하면 AUC가 동일하게 유지 되더라도 해당 NE가 향상됩니다. 이러한 측정 기준에 대한 심도있는 연구는 [12]를 참조하십시오.
