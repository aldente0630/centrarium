...
이 절은 그림 1의 묘사처럼 부스팅 결정 트리와 확률론적 희소 선형 분류기를 연결한 하이브리드 모형 구조를 제시한다. 3.1절은 의사 결정 트리가 확률론적 선형 분류기의 정확도를 상당히 향상시키는 매우 강력한 입력 변수 변환임을 설명한다. 3.2절은 훈련 데이터의 신선도가 예측의 정확도로 어떻게 이어지는지 설명한다. 이를 통해 선형 분류기를 훈련시킬 때 온라인 학습 방법을 사용해보자는 아이디어를 얻었다. 3.3절은 확률론적 선형 분류기 두 족(族)에 대한 온라인 학습 변형을 비교합니다.
