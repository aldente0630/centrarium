...
트리 변환을 하지 않은 모형 정규화 엔트로피와 비교해서 트리 변수 변환은 정규화 엔트로피를 3.4% 이상 감소시켰다. 이는 매우 중요한 개선점이다. 참고로 전형적인 피쳐 엔지니어링 실험은 비교 대상 NE의 수십여 퍼센트를 감소시킨다. LR과 Tree 모형을 단독으로 사용하면 예측 정확도가 비슷한데(LR이 조금 더 좋음) 조합하는 경우 정확도가 크게 향상한다는 점이 흥미롭다. 예측 정확도 향상은 중요합니다. 참고로 피쳐 엔지니어링 실험 대부분은 정규화 엔트로피 단지 몇 퍼센트 정도만 감소시킨다.
