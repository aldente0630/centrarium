...
3 장에서는 다양한 확률 론적 선형 분류기와 다양한 온라인 학습 알고리즘을 평가합니다. 선형 분류와 관련하여 우리는 피쳐 변환 및 데이터 신선도의 영향을 평가하기 위해 계속 진행합니다. 특히 데이터 신선도 및 온라인 학습과 관련된 실제 교훈에 영감을 얻어 우리는 매우 컴팩트 한 모델을 생산하면서 온라인 학습 계층을 통합 한 모델 아키텍처를 제시합니다. 섹션 4에서는 실시간 학습 데이터의 실시간 스트림을 생성 할 수있는 인프라의 실험적 부분 인 온라인 학습자 인 온라인 학습 계층에 필요한 핵심 구성 요소에 대해 설명합니다.
