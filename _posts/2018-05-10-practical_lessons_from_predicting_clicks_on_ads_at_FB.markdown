...
부스팅 결정 트리는 방금 설명한 종류의 비선형과 튜플 변환을 수행하는 강력하고 편리한 방법이다. 개별 트리는 샘플이 최종 할당되는 리프의 색인을 값으로 취하는 범주형 변수로 볼 수 있다. 이 변수 유형은 '1-of-K' 코딩을 한다. 예를 들어 첫 번째 하위 트리가 3 개의 리프가 있고 두 번째 하위가 2 개의 리프가있는 2 개의 서브 트리가있는 그림 1의 부스트 트리 모델을 생각해보십시오. 인스턴스가 첫 번째 하위 트리의 리프 2에서 끝나고 두 번째 하위 트리의 리프 1에서 끝나는 경우 선형 분류기에 대한 전체 입력은 이진 벡터 \\(\[0, 1, 0, 1, 0]\\)가됩니다. 여기서 첫 번째 3 개의 항목은 첫 번째 하위 트리의 잎과 두 번째 하위 트리의 잎의 잎이 있습니다. 우리가 사용하는 boosted decision trees는 전통적인 \\(L_2-TreeBoost\\) 알고리즘이 사용되는 Gradient Boosting Machine (GBM) [5]을 따릅니다. 각 학습 반복에서, 잔여를 모델링하기 위해 새로운 트리가 생성된다
