...
데이터 신선도를 최대화하기 위해 선형 분류기를 온라인으로, 즉 라벨이 지정된 광고 노출 수가 직접적으로 발생하도록 교육하는 것이 하나의 옵션입니다. 다음 섹션 4에서는 실시간 교육 데이터를 생성 할 수있는 인프라를 설명합니다. 이 섹션에서는 로지스틱 회귀에 대한 SGD 기반 온라인 학습의 학습 속도 설정. 그런 다음 BOPR 모델에 대한 온라인 학습에 가장 적합한 대안을 비교합니다.
