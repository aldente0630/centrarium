...
위 결과에서 상위 10개 변수가 전체 변수 중요도의 약 절반을 차지하는 반면 마지막 300개 변수은 변수 중요도 1% 미만임을 알 수 있다. 이 결과를 바탕으로 상위 10, 20, 50, 100 및 200개 변수만 유지하고 성능이 어떻게 영향을 받는지 평가했다. 실험 결과는 그림 7에 나와있다. 그림에서 변수를 포함시킬수록 표준화 엔트로피의 한계 이익이 비슷한 수준으로 감소함을 볼 수 있다.
...
다음으로 이력과 맥락 관련 변수의 유용성에 대해 탐구할 것이다. 데이터의 본질적으로 민감한 사안과 회사 정책으로 사용한 실제 변수의 세부 정보는 공개할 수 없다. 맥락 관련 변수의 일부 예로 현지 시간대, 요일 등이 있다. 이력 관련 변수는 광고 등에 대한 누적 클릭 수이다.

### 5.3 이력 관련 변수
  
Boosting 모델에 사용 된 기능은 두 가지 유형으로 분류 할 수 있습니다. 문맥 기능과 기록 기능. 문맥 적 기능의 가치는 사용자가 사용하는 기기 또는 사용자가 현재 사용중인 페이지와 같이 광고가 게재 될 문맥에 관한 최신 정보에만 의존합니다. 반대로 과거의 기능은 광고 또는 사용자의 이전 상호 작용 (예 : 지난 주 광고의 클릭률 또는 사용자의 평균 클릭률)에 따라 달라집니다.

이 파트에서는 시스템의 성능이 두 가지 유형의 기능에 어떻게 의존하는지 연구합니다. 우선 우리는 두 가지 유형의 피처의 상대적인 중요성을 확인합니다. 중요도에 따라 모든 지형지 물을 정렬 한 다음 첫 번째 중요한 지형지물의 역사적 지형지 물의 비율을 계산합니다. 결과를 그림 8에 나타냅니다. 결과에서 역사적 기능이 문맥 적 기능보다 훨씬 더 많은 설명력을 제공한다는 것을 알 수 있습니다. 중요도에 따라 상위 10 개 기능은 모두 역사적인 기능입니다. 상위 20 개 기능 중에는이 데이터 세트의 기능 중 약 75 %를 차지하는 역사적 기능에도 불구하고 2 개의 문맥 기능 만 있습니다. 전체 유형의 각 유형의 피쳐 비교 값을 더 잘 이해하기 위해 문맥 피쳐와 기록 피쳐만으로 두 개의 부스팅 모델을 학습 한 다음 두 모델을 모든 피쳐의 전체 모델과 비교합니다. 그 결과를 표 4에 나타낸다.

![그림 8](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB11.PNG)
**그림 8: 과거의 지형지 물 비율에 대한 결과입니다. X 축은 피쳐의 수에 해당합니다. Y 축은 중요 기능 중 주요 기능의 히스토리 기능 비율을 나타냅니다.** 

표에서 우리는 전체적인 역사적 특징이 문맥 적 특징보다 더 큰 역할을한다는 것을 다시 확인할 수 있습니다.
