...
음의 레이블 다운 샘플링은 훈련 속도를 높이고 모형 성능을 향상시킬 수 있다. 음의 레이블 다운 샘플링을 적용한 데이터셋에서 모형을 학습시키면 모형은 다운 샘플링 공간에서 예측을 보정한다는 점 염두해라. 예를 들어 샘플링 이전 평균 CTR이 0.1%이고 0.01 음의 레이블 다운 샘플링을 수행하면 경험적 CTR은 대략 10%가 된다. 실시간 트래픽 실험을 위해 모형을 재보정하고 \\(q = {q \over p + (1 - p) / w}\\)로 0.1% 예측으로 돌아가야한다. 여기서 \\(p\\)는 다운 샘플링 공간에서의 예측이고 \\(w\\)는 음의 레이블 다운 샘플링 비율이다.
