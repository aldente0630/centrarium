...
**표 1: 로지스틱 회귀(LR)과 부스팅 결정 트리(Trees)는 강력한 조합을 만든다. 그 정규화 엔트로피(NE)를 트리만 사용한 모형과 비교하여 측정했다. **
  
트리 변환을 하지 않은 모형 정규화 엔트로피와 비교해서 트리 변수 변환은 정규화 엔트로피를 3.4 % 이상 감소시켰다. 매우 중요한 개선점이다. 참고로 전형적인 피쳐 엔지니어링 실험은 비교 대상 NE의 수십 퍼센트를 줄입니다. 고립되어 사용되는 LR 및 Tree 모델은 예측 정확도 (LR이 조금 더 좋음)와 비교할 만하지만 정확도의 도약을 이끌어내는 조합이라는 점을 보면 흥미 롭습니다. 예측 정확도의 향상은 중요합니다. 참고로, 피쳐 엔지니어링 실험의 대부분은 Normalized Entropy를 백분율로 감소시킵니다.
