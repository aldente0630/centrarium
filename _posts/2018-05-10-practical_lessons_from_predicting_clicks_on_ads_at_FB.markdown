...
트리 개수를 1개에서 2,000개까지 증가시키며 하루치 데이터로 모형을 훈련한 후 익일 예측 성능을 테스트했다. 각 트리 리프가 12개가 넘지않도록 제한했다. 이전 실험과 비슷하게 정규화 엔트로피를 평가 척도로 사용했다. 실험 결과는 그림 5에 나와 있다. 정규화 엔트로피는 부스팅 트리의 트리 개수를 늘리면 줄어든다. 그러나 트리를 추가할 때마다 얻는 한계 이익은 감소한다. 거의 대부분의 NE 개선폭은 첫 500개 트리에서 이뤄졌다. 마지막 트리 1,000개는 NE 0.1% 이하만을 감소시켰다. 또 서브 모델 2에 대한 표준화 엔트로피가 트리 1,000개 이후 점점 다시 증가함을 볼 수 있다. 이런 현상은 과적합 때문에 일어난다. 서브 모델 2에 대한 훈련 데이터는 서브 모델 0과 1의 것보다 4배 적기 때문이다.

![그림 5](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB8.PNG)
**그림 5: 부스팅 트리 개수에 대한 실험 결과이다. 서로 다른 실선은 서로 다른 서브 모델에 해당한다. X축은 부스팅 트리 개수이다. Y축은 정규화 엔트로피이다.** 
  
### 5.2 부스팅 변수 중요도
  
변수 개수는 추정 정확도와 계산 성능 간 절충에 영향을 줄 수 있는 또 하나의 모형 특성이다. 변수 개수의 영향을 더 잘 이해하기 위해 각 변수마다 변수 중요도를 먼저 산출했다.
  
피쳐의 중요성을 측정하기 위해 우리는 피쳐로 인한 누적 손실 감소를 포착하는 것을 목표로하는 통계 부스트 피쳐 중요성을 사용합니다. 각 트리 노드 구성에서 제곱 된 오류 감소를 최대화하기 위해 최상의 기능을 선택하고 분할합니다. 피쳐가 여러 트리에서 사용될 수 있으므로 각 피쳐에 대한 (부스트 피쳐 중요성)은 모든 트리에서 특정 피쳐의 총 감소를 합산하여 결정됩니다.
  
일반적으로 적은 수의 기능이 대부분의 설명력에 기여하는 반면 나머지 기능에는 한계가 있습니다. 그림 6에서 누적 피쳐 중요도 대 피쳐 수를 플로팅 할 때 이와 동일한 패턴을 볼 수 있습니다.

![그림 6](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB9.PNG)
**그림 6: 기능의 중요성을 높입니다. X 축은 피쳐의 수에 해당합니다. 왼쪽 측면 1 차 y 축의 로그 스케일에서 특징의 중요성을 그리고 오른쪽 측면의 2 차 y 축은 누적 특징 중요성을 표시합니다.** 
  
위의 결과에서 상위 10 개 기능은 전체 기능 중요성의 약 절반을 담당하지만 마지막 300 개 기능은 기능 중요성이 1 % 미만이라는 것을 알 수 있습니다. 이 결과를 바탕으로 상위 10, 20, 50, 100 및 200 기능 만 유지하고 성능이 어떻게 영향을 받는지 평가합니다. 실험 결과는 그림 7에 나와 있습니다. 그림에서 표준화 된 엔트로피는 더 많은 기능을 포함 할 때와 비슷한 감소하는 반환 특성을 나타냅니다.

![그림 7](https://aldente0630.github.io/assets/practical_lessons_from_predicting_clicks_on_ads_at_FB10.PNG)
**그림 7: 최고 기능으로 모델을 부스트하는 결과입니다. 왼쪽 1 차 y 축에 보정을 그리고 표준화 된 엔트로피는 오른쪽 2 차 y 축에 표시됩니다.** 
  
다음에서는 역사적 및 문맥 적 기능의 유용성에 대해 연구 할 것입니다. 사실상 데이터 민감성과 회사 정책으로 인해 우리가 사용하는 실제 기능에 대한 세부 정보를 공개 할 수는 없습니다. 일부 상황 별 기능의 예는 현지 시간대, 요일 등이 될 수 있습니다. 역사적 특징은 광고 등의 누적 클릭 수입니다.
