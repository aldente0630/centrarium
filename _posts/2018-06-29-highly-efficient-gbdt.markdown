...
제안된 GOSS 방법론은 우선 기울기 절대값에 따라 훈련 개체 순위를 내림차순으로 매긴다. 그런 다음 기울기가 큰 상위 \\(\alpha \times 100\%\\) 개체를 가지고 개체 부분 집합 \\(A\\)를 만든다. 기울기가 작은 \\((1 - \alpha) \times 100\%\\) 개체가 속한 여집합 \\(A^c\\)에 대해 \\(b \times \| A^c \|\\) 크기의 부분 집합 \\(B\\)를 더 무작위 표본 추출한다. 마지막으로 부분 집합 \\(A \cup B\\)에 대해 추정 분산 획득 \\(\tilde{V}_j(d)\\)에 따라 개체를 분할한다. 추정 분산 획득은
