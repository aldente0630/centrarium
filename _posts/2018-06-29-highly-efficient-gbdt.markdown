...
### 3.2 이론적 분석    
GBDT는 결정 트리를 사용하여 입력 공간 X s에서 그래디언트 공간 G까지의 함수를 학습합니다. 우리는 n i.i.d로 훈련 세트를 가지고 있다고 가정합니다. 인스턴스 {x1, ..., xn}. 각 xi는 공간 Xs에 차원 s가있는 벡터입니다. 기울기 증폭의 각 반복에서, 모델의 출력에 대한 손실 함수의 음의 기울기는 {g1, ..., gn}로 표시됩니다. 의사 결정 트리 모델은 가장 유익한 기능으로 각 노드를 분할합니다 (정보 이득이 가장 큼). GBDT의 경우 정보 이득은 일반적으로 분할 후 분산으로 측정되며, 이는 다음과 같이 정의됩니다.
