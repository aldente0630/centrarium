...
두 번째 문제로 해당 번들의 복잡성을 줄이기 위해 동일한 번들의 기능을 병합하는 좋은 방법이 필요합니다. 핵심은 기능 번들에서 원래 기능의 값을 식별 할 수 있는지 확인하는 것입니다. 히스토그램 기반 알고리즘은 특성의 연속 값 대신 개별 이력을 저장하기 때문에 독점적 인 기능을 다른 저장소에 두어 기능 번들을 구성 할 수 있습니다. 이는 피처의 원래 값에 옵셋을 추가하여 수행 할 수 있습니다. 예를 들어, 기능 번들에 두 개의 기능이 있다고 가정합니다. 원래 특징 A는 \[0, 10\]에서 값을 취하고 특성 B는 값 \[0, 20\]을 취합니다. 그런 다음 피쳐 B의 값에 10의 오프셋을 추가하여 세련된 피쳐가 \[10, 30\]에서 값을 가져옵니다. 그 후에는 피처 A와 B를 병합하고 \[0, 30\] 범위의 피처 번들을 사용하여 원래 피처 A와 B를 대체하는 것이 안전합니다. 자세한 알고리즘은 Alg에 표시됩니다. 4.
