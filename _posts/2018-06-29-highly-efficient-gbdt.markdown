...
다음으로 GOSS의 일반화 성능을 분석합니다. GOSS \\(\mathcal{E}^{GOSS}_{gen}(d) = \| \tilde{V}_j(d) - V_*(d)\|\\)의 일반화 오차는 GOSS에서 표본 추출 된 훈련 인스턴스에 의해 계산 된 분산 이득과 기본 배포. 우리는 \\(\mathcal{E}^{GOSS}_{gen}(d) \le \| \tilde{V}_j(d) - V_j(d) \| + \| V_j(d) - V_*(d) \| \triangleq EGOSS (d) + Egen (d)\\). 따라서 GOSS 근사화가 정확하다면 GOSS의 일반화 오류는 전체 데이터 인스턴스를 사용하여 계산 된 오류와 비슷할 것입니다. 반면에 표본 추출은 기본 학습자의 다양성을 증가시켜 잠재적으로 일반화 성능을 향상시키는 데 도움을 줄 것입니다 [24].
