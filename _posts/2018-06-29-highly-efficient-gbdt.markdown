...
*증명:* 우리 문제를 그래프 채색 문제로 한정지을 수 있다. 그래프 채색 문제는 NP-hard이기 때문에 결론은 추론 가능하다.

그래프 채색 문제에서 임의의 개체를 \\(G = (V, E)\\)라고 하자. 우리 문제에 해당하는 개체를 다음과 같이 만들 수 있다. \\(G\\)의 발생 행렬 각 행을 변수로 하여 \\(\\|V\\|\\)개의 변수를 사용하여 문제의 인스턴스를 가져옵니다. 우리의 문제에있는 특징의 배타적 인 묶음이 같은 색깔을 가진 꼭지점의 집합에 해당한다는 것을 쉽게 알 수 있으며 그 반대의 경우도 마찬가지입니다.
