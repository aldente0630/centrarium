...
해결해야 할 문제가 두 가지 있다. 첫째, 어떤 변수를 함께 묶을지 결정하는 것이다. 둘째, 어떻게 묶을까 방법에 관한 것이다.

**정리 4.1** *가장 적은 수의 배타적 묶음으로 변수를 나누는 문제는 NP-hard이다.*  
*증명:* 우리는 그래프 착색 문제를 우리의 문제로 줄입니다. 그래프 채색 문제는 NP-hard이기 때문에 결론을 추론할 수 있다. 그래프 채색 문제의 어떤 인스턴스 \\(G = (V, E)\\)가 주어 졌을 때. 우리는 다음과 같이 문제의 인스턴스를 만듭니다. \\(G\\)의 발생 행렬의 각 행을 피쳐로 가져 와서 \\(\| V \|\\)를 사용하여 문제의 인스턴스를 얻으십시오. 풍모. 우리의 문제에있는 독점적 인 특징들의 묶음이 같은 색깔을 가진 꼭지점들의 집합에 해당한다는 것을 쉽게 알 수 있으며 그 반대의 경우도 마찬가지입니다.
