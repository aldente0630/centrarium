...
첫째, 우리는 GOSS의 속도 향상 능력을 연구합니다. 표 2의 LightGBM과 EFB_only (GOSS가없는 LightGBM)를 비교하면 GOSS가 10 % - 20 %의 데이터를 사용하여 거의 2 배의 속도 향상을 가져올 수 있음을 알 수 있습니다. GOSS는 샘플 데이터 만 사용하여 나무를 배울 수 있습니다. 그러나 예측을 수행하고 그래디언트를 계산하는 것과 같이 전체 데이터 세트에 대한 일부 계산을 유지합니다. 따라서 전반적인 속도 향상은 샘플링 된 데이터의 비율과 선형 적으로 상관 관계가 없음을 알 수 있습니다. 그러나 GOSS로 인한 속도 향상은 여전히 매우 중요하며 기술은 다른 데이터 세트에 보편적으로 적용됩니다.
  
둘째, Stochastic Gradient Boosting (SGB) [20]과 비교하여 GOSS의 정확성을 평가한다. 보편성을 잃지 않고 테스트를 위해 LETOR 데이터 세트를 사용합니다. GOSS에서 다른 a 및 b를 선택하여 샘플링 비율을 조정하고 SGB에 대해 동일한 전체 샘플링 비율을 사용합니다. 조기 정지를 사용하여 수렴 될 때까지이 설정을 실행합니다. 결과는 표 4에 나와 있습니다. 동일한 샘플링 비율을 사용하는 경우 GOSS의 정확도가 SGB보다 항상 우수함을 알 수 있습니다. 이 결과는 3.2 절의 논의와 일치한다. 모든 실험은 GOSS가 확률 적 샘플링보다 더 효과적인 샘플링 방법임을 입증합니다.

### 5.3 EFB에 대한 분석
  
우리는 lgb_baseline과 EFB_only를 비교하여 EFB의 속도 향상에 대한 기여도를 확인합니다. 결과는 표 2에 나와 있습니다. 여기서는 번들 찾기 프로세스 (예 : γ = 0)에서 충돌을 허용하지 않습니다.[^10] EFB가 대규모 데이터 세트에서 상당한 속도 향상을 달성하는 데 도움이 될 수 있음을 발견했습니다.
...
본 논문에서는 LightGBM이라는 새로운 GBDT 알고리즘을 제안했다.이 알고리즘은 많은 수의 데이터 인스턴스와 많은 수의 기능을 처리하는 두 가지 새로운 기술, 즉 Gradient 기반 One-Side Sampling과 Exclusive Feature Bundling을 포함한다. 우리는이 두 가지 기법에 대한 이론적 분석과 실험적 연구를 모두 수행했습니다. 실험 결과는 이론과 일치하며 GOSS 및 EFB의 도움으로 LightGBM이 계산 속도와 메모리 소비면에서 XGBoost 및 SGB보다 훨씬 뛰어난 성능을 보임을 보여줍니다. 향후 연구를 위해 그라디언트 기반의 일방 샘플링 (One-Side Sampling)에서 a와 b의 최적 선택을 연구하고 드문 드문 드문 상관없이 다수의 기능을 처리하기 위해 Exclusive Feature Bundling의 성능을 지속적으로 향상시킬 것입니다.
