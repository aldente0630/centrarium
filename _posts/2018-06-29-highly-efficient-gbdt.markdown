...
고차원 데이터는 일반적으로 매우 희소하다. 변수 공간의 희소성은 변수 개수를 줄일 때 손실이 거의 없는 접근 방식에 대한 설계 가능성을 제공한다. 특히 희소 변수 공간에서 많은 변수가 상호 배타적이다. 즉, 0이 아닌 값을 동시에 갖지 않는다. 배타적 변수를 단일 변수(이를 배타적 변수 묶음이라고 함)로 안전하게 묶을 수 있다. 신중하게 설계된 기능 스캔 알고리즘을 통해 피쳐 번들에서 동일한 피쳐 히스토그램을 개별 피쳐의 피쳐 번들로 작성할 수 있습니다. 이렇게하면 히스토그램 작성의 복잡성이 \\(O(\\# data \times \\# feature)\\)에서 \\(O(\\# data \times \\# bundle)\\)로 바뀌고 \\(\\# bundle << \\# feature\\)입니다. 그런 다음 정확성을 해치지 않으면 서 GBDT의 교육 속도를 크게 높일 수 있습니다. 다음에서는이를 어떻게 달성 할 수 있는지 자세히 설명 할 것입니다.
