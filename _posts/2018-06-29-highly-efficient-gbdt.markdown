...
두 번째 문제로 해당 묶음의 훈련 복잡도를 줄이기 위해 같이 묶인 변수를 병합하는 좋은 방법이 필요하다. 핵심은 변수 묶음에서 원래 기능의 값을 식별 할 수 있는지 확인하는 것입니다. 히스토그램 기반 알고리즘은 특성의 연속 값 대신 개별 이력을 저장하기 때문에 독점적 인 기능을 다른 저장소에 두어 기능 번들을 구성 할 수 있습니다. 이는 피처의 원래 값에 옵셋을 추가하여 수행 할 수 있습니다. 예를 들어, 기능 번들에 두 개의 기능이 있다고 가정합니다. 원래 특징 A는 \[0, 10\]에서 값을 취하고 특성 B는 값 \[0, 20\]을 취합니다. 그런 다음 피쳐 B의 값에 10의 오프셋을 추가하여 세련된 피쳐가 \[10, 30\]에서 값을 가져옵니다. 그 후에는 피처 A와 B를 병합하고 \[0, 30\] 범위의 피처 번들을 사용하여 원래 피처 A와 B를 대체하는 것이 안전합니다. 자세한 알고리즘은 Alg에 표시됩니다. 4.
...
EFB 알고리즘은 훨씬 적은 밀도의 특징에 많은 독점적 인 특징을 묶어 줄 수 있기 때문에 0 개의 특징 값에 대한 불필요한 계산을 효과적으로 피할 수 있습니다. 사실, 우리는 또한 각 기능에 대한 테이블을 사용하여 0이 아닌 값으로 데이터를 기록함으로써 제로 피쳐 값을 무시하도록 기본 히스토그램 기반 알고리즘을 최적화 할 수 있습니다. 이 표의 데이터를 스캔하면 기능의 히스토그램 작성 비용이 \\(O(\\#data)\\)에서 \\(O(\\#non_zero_data)\\)로 변경됩니다. 그러나이 방법은 전체 트리 성장 프로세스에서 이러한 기능별 테이블을 유지 관리하기 위해 추가 메모리 및 계산 비용이 필요합니다. 우리는 LightGBM에서이 최적화를 기본 기능으로 구현합니다. 이 최적화는 번들이 희박한 경우에도 사용할 수 있으므로 EFB와 충돌하지 않습니다.
