...
*증명:* 우리는 그래프 채색 문제를 우리의 문제로 줄입니다. 그래프 채색 문제는 NP-hard이기 때문에 결론을 추론해낼 수 있다. 그래프 채색 문제의 어떤 개체 \\(G = (V, E)\\)가 주어졌다고 하자. 우리는 다음과 같이 문제의 인스턴스를 만듭니다. \\(G\\)의 발생 행렬의 각 행을 피쳐로 가져 와서 \\(\| V \|\\)를 사용하여 문제의 인스턴스를 얻으십시오. 풍모. 우리의 문제에있는 독점적 인 특징들의 묶음이 같은 색깔을 가진 꼭지점들의 집합에 해당한다는 것을 쉽게 알 수 있으며 그 반대의 경우도 마찬가지입니다.
