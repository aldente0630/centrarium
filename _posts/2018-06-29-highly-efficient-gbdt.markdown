...
제안된 GOSS 방법론은 우선 기울기 절대값에 따라 훈련 개체 순위를 내림차순으로 매긴다. 그런 다음 기울기가 큰 상위 \\(\alpha \times 100\%\\) 개체를 가지고 개체 부분 집합 \\(A\\)를 만든다. \\((1 - \alpha) \times 100\%\\)의 기울기가 작은 개체가 속하는 여집합 \\(A^c\\)에 대해 \\(b \times \| A^c \|\\) 크기의 부분 집합 \\(B\\)를 무작위 표본 추출하여 만든다. 마지막으로 부분 집합 \\(A \cup B\\)에 대해 추정 분산 획득 \\(\tilde{V}_j(d)\\)에 따라 개체를 분할한다. 추정 분산 획득은
...
따라서 GOSS는 분할점 결정 시 모든 개체를 사용한 정확한 \\(\tilde{V}_j(d)\\) 대신 더 적은 수의 개체 부분 집합을 통해 추정한 \\(V_j(d)\\)를 이용하므로 계산 비용을 크게 줄일 수 있다. 보다 중요하게 다음 정리는 GOSS가 많은 훈련 정확도를 잃지 않고 무작위 표본 추출보다 우월함을 나타냅니다. 지면의 제약으로 보충 자료에 정리 증명을 남긴다.
