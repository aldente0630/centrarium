...
실제 응용 프로그램에서 사용하는 대규모 데이터셋은 보통 굉장히 희소하다. 사전 정렬 알고리즘을 사용하는 GBDT의 경우 변수의 0값은 무시함으로써 학습 비용을 줄일 수 있다. 그러나 히스토그램 기반 알고리즘을 사용하는 GBDT의 경우 효율적인 희소 변수 최적화 해법이 없다. 그 이유는 히스토그램 기반 알고리즘이 변수 값이 0인지 아닌지 여부와 상관없이 각 데이터 개체에 대한 변수 구간 값 (Alg. 1 참조)을 검색해야하기 때문입니다. GBDT가 히스토그램 기반 알고리즘은 이러한 희소 속성을 효과적으로 활용할 수 있습니다.
