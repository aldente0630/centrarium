...
GOSS는 기울기가 큰 개체는 모두 유지하되 기울기가 작은 개체에 대해 무작위 표본 추출을 수행한다. 이 때 데이터 분포에 미치는 영향을 보상하기 위해서 정보 획득 계산 시 기울기가 작은 데이터 개체에 상수 승수를 적용한다(알고리즘 2 참조). 구체적으로 GOSS는 우선 기울기 절대값에 따라 데이터 개체를 정렬하고 상위 \\(a \times 100%\\) 개체를 선택한다. 그런 다음 나머지 데이터에서 b × 100 % 인스턴스를 무작위로 샘플링합니다. 그 후에, GOSS는 정보 이득을 계산할 때 작은 기울기를 가진 표본 데이터를 일정한 1-a b로 증폭합니다. 이렇게함으로써 우리는 원래의 데이터 분배를 많이 변경하지 않고도 훈련이 덜 된 인스턴스에 더 많은 초점을 둡니다.
- - -
