...
첫 번째 문제에 대해 우리는 정리 4.1에서 다항식 시간 내에서 정확한 해를 찾는 것이 불가능 함을 나타내는 최적의 번들링 전략을 찾는 것이 NP-Hard라는 것을 증명한다. 좋은 근사 알고리즘을 찾기 위해, 먼저 특징을 정점으로 취하고 두 특징의 모서리를 상호 배타적이지 않은 경우 추가함으로써 그래프 착색 문제에 대한 최적의 번들링 문제를 줄이고 합리적으로 생성 할 수있는 탐욕적인 알고리즘을 사용합니다 번들을 생성하기위한 그래프 채색에 좋은 결과 (일정한 근사 비율). 더구나 우리는 100 % 상호 배타적이지는 않지만 보통 0이 아닌 값을 거의 취하지는 않지만 대다수의 기능이 있음을 알 수 있습니다. 우리의 알고리즘이 충돌의 작은 부분을 허용 할 수 있다면, 더 적은 수의 특징 묶음을 얻을 수 있고 계산 효율을 더욱 향상시킬 수 있습니다. 간단한 계산에 의해, 특징 값의 작은 부분을 무작위로 오염시키는 것은 최대 \\(\mathcal{O}(\[(1 - \gamma)n\]^(-2/3))\\)만큼 훈련 정확도에 영향을 미칠 것이다 (보충 자료의 법칙 2.1 참조). 여기서 \\(\gamma\\)는 최대 각 번들의 충돌 비율. 따라서 상대적으로 작은 \\(\gamma\\)를 선택하면 정확도와 효율성 사이의 균형을 잘 맞출 수 있습니다.
