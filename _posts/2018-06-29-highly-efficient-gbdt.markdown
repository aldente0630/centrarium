...
XGBoost, pGBRT, scikit-learn, R의 gbm[^4]을 포함해 GBDT를 꽤 여러 종류로 구현해놨다. Scikit-learn과 R의 gbm은 사전 정렬 알고리즘을 구현했고 pGBRT는 히스토그램 기반 알고리즘을 구현했다. XGBoost는 사전 정렬 알고리즘과 히스토그램 기반 알고리즘 모두 지원한다. 논문[^5]에서 볼 수 있듯이 XGBoost가 다른 것들을 능가한다. 그래서 본 논문 실험 시 XGBoost를 기준선으로 삼았다.
...
고차원 데이터는 일반적으로 매우 희소하다. 변수 공간의 희소성은 변수 개수를 줄일 때 손실이 거의 없는 방법 설계에 관한 단초를 제공한다. 특히 희소 변수 공간에선 변수 다수가 상호 배타적이다. 즉, 0이 아닌 값을 동시에 갖지 않는다. 배타적 변수를 단일 변수(이를 *배타적 변수 묶음*이라고 함)로 안전하게 묶을 수 있다. 신중하게 설계한 변수 탐색 알고리즘을 통해 개별 변수들의 히스토그램과 동일한 것을 변수 묶음을 통해 만들어낼 수 있다. 이 경우 히스토그램 생성 복잡도는 \\(O(\\# data \times \\# feature)\\)에서 \\(O(\\# data \times \\# bundle)\\)로 바뀐다. #bundle << #feature라고 할 때 정확도를 저하시키지 않으면서 GBDT 훈련 속도를 크게 향상시킬 수 있다. 이제 이게 어떻게 달성 가능한지 상세히 설명할 것이다.
...
EFB 알고리즘은 많은 배타적 변수를 훨씬 적은 밀집 변수로 묶어 줄 수 있기 때문에 변수의 0값에 대한 불필요한 계산을 효과적으로 피할 수 있다. 사실 각 변수에 대한 테이블을 사용하여 0이 아닌 값의 데이터를 기록함으로써 변수 0값을 무시하게 기본 히스토그램 기반 알고리즘을 최적화시킬 수 있다. 해당 테이블의 데이터를 훑는 경우 변수 히스토그램 생성 비용은 \\(O(\\#data)\\)에서 \\(O(\\#non\\_zero\\_data)\\)로 바뀐다. 그러나 트리를 전반적으로 성장시키는 와중에 변수 별 테이블을 이런 식으로 유지, 관리하기 위해서는 메모리와 계산 비용이 추가로 필요하다. LightGBM은 해당 최적화 방법을 기본 기능으로 구현했다. 본 최적화는 묶음이 희소한 경우에도 사용할 수 있으므로 EFB와 충돌하지 않는다.

## 5. 실험
  
이 장에서는 제안한 LightGBM 알고리즘에 대해 실험한 결과를 보고하겠다. 공개되어 이용 가능한 데이터셋 5가지를 사용했다. 그 중 Microsoft Learning to Rank(LETOR) 데이터셋은 웹 검색 질의 30,000개를 포함하고있다. 이 데이터셋에 사용된 변수 대부분은 수치형 밀집 변수이다. [Allstate Insurance Claim](https://www.kaggle.com/c/ClaimPredictionChallenge)과 [Flight Delay](https://github.com/szilard/benchm-ml#data) 데이터셋은 원-핫 코딩된 변수를 많이 포함하고있다. 마지막 두 데이터셋은 KDD CUP 2010과 KDD CUP 2012에서 가져온 것이다. NTU가 [우승 해법](https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/binary.html)에서 사용한 변수(밀집 및 희소 변수 모두 포함한)를 직접 사용할 것이다. 이 두 데이터셋은 매우 크다. 이러한 데이터셋은 크기가 크고 밀집 및 희소 변수 모두 포함하므로 실제 분석 과제를 대신할 수 있다. 따라서 알고리즘을 철저히 시험해보기 위해 그것들을 이용할 수 있다.
  
E5-2670 v3 CPU 2개(총 24코어)와 256GB 메모리의 리눅스 서버가 실험 환경이다. 모든 실험은 멀티스레딩으로 진행했고 스레드 수는 16개로 고정했다.
...
![그림1](https://aldente0630.github.io/assets/highly-efficient-gbdt4.PNG)
...
첫째, 우리는 GOSS의 속도 향상 능력을 연구합니다. 표 2의 LightGBM과 EFB_only (GOSS가없는 LightGBM)를 비교하면 GOSS가 10% - 20%의 데이터를 사용하여 거의 2배의 속도 향상을 가져올 수 있음을 알 수 있습니다. GOSS는 샘플 데이터 만 사용하여 나무를 배울 수 있습니다. 그러나 예측을 수행하고 그래디언트를 계산하는 것과 같이 전체 데이터 세트에 대한 일부 계산을 유지합니다. 따라서 전반적인 속도 향상은 샘플링 된 데이터의 비율과 선형 적으로 상관 관계가 없음을 알 수 있습니다. 그러나 GOSS로 인한 속도 향상은 여전히 매우 중요하며 기술은 다른 데이터 세트에 보편적으로 적용됩니다.
  
둘째, Stochastic Gradient Boosting (SGB)과 비교하여 GOSS의 정확성을 평가한다. 보편성을 잃지 않고 테스트를 위해 LETOR 데이터 세트를 사용합니다. GOSS에서 다른 \\(a\\) 및 \\(b\\)를 선택하여 샘플링 비율을 조정하고 SGB에 대해 동일한 전체 샘플링 비율을 사용합니다. 조기 정지를 사용하여 수렴 될 때까지이 설정을 실행합니다. 결과는 표 4에 나와 있습니다. 동일한 샘플링 비율을 사용하는 경우 GOSS의 정확도가 SGB보다 항상 우수함을 알 수 있습니다. 이 결과는 3.2 절의 논의와 일치한다. 모든 실험은 GOSS가 확률 적 샘플링보다 더 효과적인 샘플링 방법임을 입증합니다.

![표4](https://aldente0630.github.io/assets/highly-efficient-gbdt6.PNG)
표 4: 다양한 샘플링 비율에서 GOSS 및 SGB에 대한 LETOR 데이터 세트의 정확도 비교. 조기 중단과 함께 대규모 반복을 사용하여 모든 실험이 수렴 지점에 도달하도록합니다. 다른 설정에 대한 표준 편차는 작습니다. GOSS의 \\(a\\) 및 \\(b\\) 설정은 보충 자료에서 찾을 수 있습니다.
  
### 5.3 EFB에 대한 분석
  
우리는 lgb_baseline과 EFB_only를 비교하여 EFB의 속도 향상에 대한 기여도를 확인합니다. 결과는 표 2에 나와 있습니다. 여기서는 번들 찾기 프로세스 (예 : \\(\gamma = 0\\))에서 충돌을 허용하지 않습니다.[^10] EFB가 대규모 데이터 세트에서 상당한 속도 향상을 달성하는 데 도움이 될 수 있음을 발견했습니다.
  
lgb_baseline은 스파 스 기능에 최적화되어 있으므로 EFB는 여전히 큰 요인으로 교육 속도를 높일 수 있습니다. 이는 EFB가 많은 스파 스 기능 (단일 핫 코딩 기능과 암시 적으로 독점 기능 모두)을 훨씬 적은 기능으로 병합하기 때문입니다. 기본 스파 스 기능 최적화가 번들링 프로세스에 포함됩니다. 그러나 EFB에는 트리 학습 프로세스의 각 기능에 대해 0이 아닌 데이터 테이블을 유지 관리하는 데 추가 비용이 들지 않습니다. 또한 이전에 격리 된 많은 기능이 함께 번들로 제공되므로 공간적 지역성을 높이고 캐시 적중률을 크게 향상시킬 수 있습니다. 따라서 전반적인 효율성 향상은 극적입니다. 위의 분석을 통해 EFB는 히스토그램 기반 알고리즘에서 스파 스 속성을 활용하는 데 매우 효과적인 알고리즘이며 GBDT 교육 프로세스의 속도를 크게 높일 수 있습니다.

## 6. 결론
  
본 논문에서는 LightGBM이라는 새로운 GBDT 알고리즘을 제안했다.이 알고리즘은 많은 수의 데이터 인스턴스와 많은 수의 기능을 처리하는 두 가지 새로운 기술, 즉 *기울기 기반 단측 표본추출*과 *배타적 변수 묶음*을 포함한다. 우리는이 두 가지 기법에 대한 이론적 분석과 실험적 연구를 모두 수행했습니다. 실험 결과는 이론과 일치하며 GOSS 및 EFB의 도움으로 LightGBM이 계산 속도와 메모리 소비면에서 XGBoost 및 SGB보다 훨씬 뛰어난 성능을 보임을 보여줍니다. 향후 연구를 위해 그라디언트 기반의 일방 샘플링 (One-Side Sampling)에서 \\(a\\)와 \\(b\\)의 최적 선택을 연구하고 드문 드문 드문 상관없이 다수의 기능을 처리하기 위해 Exclusive Feature Bundling의 성능을 지속적으로 향상시킬 것입니다.
...
[^10]: 보충 자료에 \\(\gamma\\) 조정에 관한 상세 연구가 있다.
