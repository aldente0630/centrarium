...
lgb_baseline과 EFB_only를 비교하여 속도 향상에 대한 EFB 기여도를 확인했다. 결과는 표 2에 나와있다. 여기서 묶음 찾기 절차(예: \\(\gamma = 0\\))중 충돌을 허용하지 않았다.[^10] 대규모 데이터셋에서 EFB가 속도 향상에 상당히 도움이 됨을 알 수 있다.
  
lgb_baseline가 희소 변수에 최적화되어 있음에도 EFB는 여전히 큰 요소로 작용하여 훈련 속도를 높일 수 있었다. 이는 EFB가 많은 희소 변수(원-핫 코딩 변수와 암시적으로 배타적인 변수 모두)을 훨씬 적은 변수로 병합하기 때문이다. 기본적인 희소 변수 최적화가 묶음 절차에 포함된다. 그러나 EFB의 경우 트리 학습 절차 중 변수 별 0이 아닌 데이터 테이블을 유지, 관리하는 데 추가 비용이 들지 않는다. 또한 이전에 격리 된 많은 기능이 함께 번들로 제공되므로 공간적 지역성을 높이고 캐시 적중률을 크게 향상시킬 수 있습니다. 따라서 전반적인 효율성은 극적으로 향상한다. 위의 분석을 통해 EFB는 히스토그램 기반 알고리즘에서 스파 스 속성을 활용하는 데 매우 효과적인 알고리즘이며 GBDT 교육 프로세스의 속도를 크게 높일 수 있습니다.
