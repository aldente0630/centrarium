...
두 번째 문제로 해당 묶음의 훈련 복잡도를 줄이기 위해 같이 묶인 변수를 병합시키는 괜찮은 방법이 필요하다. 변수 묶음에서 원래 변수 값을 식별해낼 수 있는지 확인하는 것이 핵심이다. 히스토그램 기반 알고리즘은 변수의 연속적인 값 대신 개별 구간을 저장하기 때문에 배타적 변수를 각기 다른 구간에 두어 변수 묶음을 구성할 수 있다. 이는 변수의 원래 값에 오프셋을 더하여 수행할 수 있다. 예를 들어 변수 묶음에 변수 두 개가 속한다고 가정하자. 원래의 변수 A는 \[0, 10\] 값을 취하고 변수 B는 \[0, 20\]값을 취한다. 그렇다면 변수 B 값에 오프셋 10을 더하여 가공한 변수가 \[10, 30\]에서 값을 가져옵니다. 그 후에는 피처 A와 B를 병합하고 \[0, 30\] 범위의 피처 번들을 사용하여 원래 피처 A와 B를 대체하는 것이 안전합니다. 자세한 알고리즘은 Alg에 표시됩니다. 4.
