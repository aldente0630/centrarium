...
정리에 의거해 다음과 같이 논의해보 수 있다. (1) GOSS의 점근적 근사 비율은 \\(\mathcal{O}\left( \right)\\) isO njl (d) + njr (d) + √n의 점근 적 근사 비율. 분할이 너무 불균형 적이 지 않으면 (즉, nl (d) ≥O (n) 및 111jnjr (d) ≥ O (√n)), 근사 오차는 Ineq의 두 번째 항이 지배하게됩니다. n → ∞ 일 때 O (√ n)에서 0으로 감소한다. 즉, 데이터 수가 많을 때, 근사값
