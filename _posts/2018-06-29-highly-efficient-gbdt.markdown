...
그래프 색칠 문제에서 임의의 그래프를 \\(G = (V, E)\\)라고 하자. 우리 문제에 해당하는 그래프를 다음과 같이 만들 수 있다. \\(G\\)의 발생 행렬 각 행이 각 변수에 해당하게 \\(\|V\|\\)개 변수를 사용하여 우리 문제에 해당하는 개체를 만든다. 우리의 문제에있는 특징의 배타적 인 묶음이 같은 색깔을 가진 꼭지점의 집합에 해당한다는 것을 쉽게 알 수 있으며 그 반대의 경우도 마찬가지입니다.
