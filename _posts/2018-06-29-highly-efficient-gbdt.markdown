...
고차원 데이터는 일반적으로 매우 희소하다. 변수 공간의 희소성은 변수 개수를 줄일 때 손실이 거의 없는 방법 설계에 대한 단초를 제공한다. 희소 변수 공간에서 특히 다수의  변수가 상호 배타적이다. 즉, 0이 아닌 값을 동시에 갖지 않는다. 배타적 변수를 단일 변수(이를 *배타적 변수 묶음*이라고 함)로 안전하게 묶을 수 있다. 신중하게 설계한 변수 스캔 알고리즘을 통해 개별 변수의 히스토그램과 동일한 것을 변수 묶음을 통해 만들 수 있다. 이렇게 하면 #bundle << #feature라고 할 때 히스토그램 생성 복잡도는 \\(O(\\# data \times \\# feature)\\)에서 \\(O(\\# data \times \\# bundle)\\)로 바뀐다. 이 경우 정확도를 저하시키지 않으면서 GBDT 훈련 속도를 크게 높일 수 있다. 다음으로 이것을 어떻게 달성했는지 상세히 설명할 것이다.
  
해결해야 할 문제가 두 가지 있다. 첫 번째는 어떤 변수를 함께 묶을지 결정하는 것이다. 두 번째는 어떻게 묶을지 방법에 관한 것이다.

**정리 4.1** *변수를 가장 적은 개수의 배타적 묶음으로 나누는 건 NP-hard 문제이다.*  
