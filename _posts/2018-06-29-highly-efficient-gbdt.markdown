...
고차원 데이터는 일반적으로 매우 드물다. 특징 공간의 희박성은 기능의 수를 줄이기 위해 거의 무손실 접근 방식을 설계 할 수있는 가능성을 제공합니다. 특히, 스파 스 특징 공간에서 많은 특징들은 상호 배타적이다. 즉, 결코 0이 아닌 값을 동시에 취하지 않는다. 독점적 기능을 단일 기능 (이를 독점적 기능 번들이라고 함)에 안전하게 묶을 수 있습니다. 신중하게 설계된 기능 스캔 알고리즘을 통해 피쳐 번들에서 동일한 피쳐 히스토그램을 개별 피쳐의 피쳐 번들로 작성할 수 있습니다. 이렇게하면 히스토그램 작성의 복잡성이 \\(O(\\# data \times \\# feature)\\)에서 \\(O(\\# data \times \\# bundle)\\)로 바뀌고 \\(\\# bundle << \\# feature)\\)입니다. 그런 다음 정확성을 해치지 않으면 서 GBDT의 교육 속도를 크게 높일 수 있습니다. 다음에서는이를 어떻게 달성 할 수 있는지 자세히 설명 할 것입니다.
  
해결해야 할 두 가지 문제가 있습니다. 첫 번째 기능은 함께 제공 할 기능을 결정하는 것입니다. 두 번째 방법은 번들을 만드는 방법입니다.

**정리 4.1** *기능을 가장 작은 수의 전용 번들로 분할하는 문제는 NP 하드입니다.*  
*증명:* 우리는 그래프 착색 문제를 우리의 문제로 줄입니다. 그래프 착색 문제는 NP 어렵 기 때문에 결론을 추론 할 수 있습니다. 그래프 채색 문제의 어떤 인스턴스 \\(G = (V, E)\\)가 주어 졌을 때. 우리는 다음과 같이 문제의 인스턴스를 만듭니다. \\(G\\)의 발생 행렬의 각 행을 피쳐로 가져 와서 \\(\| V \|\\)를 사용하여 문제의 인스턴스를 얻으십시오. 풍모. 우리의 문제에있는 독점적 인 특징들의 묶음이 같은 색깔을 가진 꼭지점들의 집합에 해당한다는 것을 쉽게 알 수 있으며 그 반대의 경우도 마찬가지입니다. 􏰀
