...
이번 장에서는 데이터 개체 수를 줄이는 것과 학습한 의사 결정 트리의 정확도 유지하는 것 사이에서 균형을 잘 잡을 수 있는 GBDT의 새로운 표본 추출 방법을 제안한다.

### 3.1 알고리즘 서술
  
AdaBoost에서 샘플 가중치는 데이터 인스턴스의 중요성을 나타내는 좋은 지표가됩니다. 그러나 GBDT에는 원시 샘플 가중치가 없으므로 AdaBoost에 제안 된 샘플링 방법을 직접 적용 할 수 없습니다. 다행히도 GBDT의 각 데이터 인스턴스에 대한 그래디언트는 데이터 샘플링에 유용한 정보를 제공합니다. 즉, 인스턴스가 작은 그래디언트와 연결되어 있으면이 인스턴스에 대한 학습 오류가 적고 이미 잘 훈련 된 것입니다. 간단한 아이디어는 작은 그라데이션으로 데이터 인스턴스를 버리는 것입니다. 그러나 그렇게함으로써 데이터 분포가 변경되므로 학습 된 모델의 정확성이 떨어질 수 있습니다. 이 문제를 피하기 위해 Gradient 기반 One-Side Sampling (GOSS)이라는 새로운 방법을 제안합니다.
