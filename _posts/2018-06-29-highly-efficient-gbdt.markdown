...
본 논문에서는 LightGBM이라는 새로운 GBDT 알고리즘을 제안했다. 이 알고리즘은 다수의 데이터 개체와 다수의 변수를 처리하는 두 가지 새로운 기술, 즉 *기울기 기반 단측 표본추출*과 *배타적 변수 묶음*을 포함한다. 이 두 가지 기법에 대한 이론적 분석과 실험적 연구 모두 수행했다. 실험 결과는 이론과 일치하며 GOSS와 EFB 도움으로 LightGBM이 계산 속도와 메모리 소비면에서 XGBoost 및 SGB보다 훨씬 뛰어난 성능을 보여준다. 향후 진행할 작업은 기울기 기반 단측 표본 추출에서 \\(a\\)와 \\(b\\) 최적 선택을 연구하고 희소 혹은 그렇지 않은 변수 다수를 처리하기 위해 배타적 변수 묶음 성능을 지속적으로 향상시키는 것이다.
