...
첫 번째 문제에 대해 다항 시간 안에 정확한 해를 찾는 것이 불가능함을, 즉 최적 묶음 전략을 찾는 것이 NP-hard임을 정리 4.1를 통해 증명했다. 괜찮은 근사 알고리즘을 찾기 위해 먼저 변수를 꼭지점으로 취하고 두 변수가 상호 배타적이지 않다면 변수 사이의 변을 추가하면서 최적 묶음 문제를 그래프 색칠 문제로 한정짓고 합리적으로 생성 할 수있는 탐욕 알고리즘을 사용한다. 번들을 생성하기위한 그래프 채색에 좋은 결과 (일정한 근사 비율). 더구나 우리는 100 % 상호 배타적이지는 않지만 보통 0이 아닌 값을 거의 취하지는 않지만 대다수의 기능이 있음을 알 수 있습니다. 우리의 알고리즘이 충돌의 작은 부분을 허용 할 수 있다면, 더 적은 수의 특징 묶음을 얻을 수 있고 계산 효율을 더욱 향상시킬 수 있습니다. 간단한 계산에 의해, 특징 값의 작은 부분을 무작위로 오염시키는 것은 최대 \\(\mathcal{O}(\[(1 - \gamma)n\]^{-2/3})\\)만큼 훈련 정확도에 영향을 미칠 것이다 (보충 자료의 법칙 2.1 참조). 여기서 \\(\gamma\\)는 최대 각 번들의 충돌 비율. 따라서 상대적으로 작은 \\(\gamma\\)를 선택하면 정확도와 효율성 사이의 균형을 잘 맞출 수 있습니다.
