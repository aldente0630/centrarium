...
AdaBoost에서의 표본 가중치는 각 데이터 개체의 중요도를 알려주는 좋은 지표이다. 그러나 GBDT에는 표본의 고유한 가중치가 없으므로 AdaBoost에 제안된 표본 추출 방법을 직접 적용할 수 없다. 다행히도 GBDT의 각 데이터 인스턴스에 대한 그래디언트는 데이터 샘플링에 유용한 정보를 제공합니다. 즉, 인스턴스가 작은 그래디언트와 연결되어 있으면이 인스턴스에 대한 학습 오류가 적고 이미 잘 훈련 된 것입니다. 간단한 아이디어는 작은 그라데이션으로 데이터 인스턴스를 버리는 것입니다. 그러나 그렇게함으로써 데이터 분포가 변경되므로 학습 된 모델의 정확성이 떨어질 수 있습니다. 이 문제를 피하기 위해 Gradient 기반 One-Side Sampling (GOSS)이라는 새로운 방법을 제안합니다.
