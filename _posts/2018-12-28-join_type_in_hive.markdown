...
**비대칭 조인**은 해당 문제 해결을 목표로 한다. 실행 시간에 데이터를 훑어보고 **hive.skewjoin.key** 매개 변수로 제어하는, 데이터가 굉장히 쏠린 키를 감지한다. 해당 키를 처리하는 대신 HDFS 디렉토리에 임시로 저장한다. 그런 다음 맵 리듀스 작업으로 이 데이터가 쏠린 키를 처리한다. 동일한 키가 모든 테이블에 대해 비뚤어 질 필요가 없으므로 맵 조인이 될 수 있기 때문에 후속 map-reduce 작업 (기울어 진 키의 경우)이 훨씬 빠릅니다.
