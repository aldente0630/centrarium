...
예를 들어 테이블 A와 B를 조인한다고 하자. 테이블 A와 B 모두 조인하는 컬럼에 "mytest" 데이터가 편중되어있다. 테이블 B가 테이블 A보다 편중된 데이터의 행 수가 적다면 첫 단계는 B를 훑어보고 "mytest"키가 있는 모든 행을 메모리 내 해시 테이블에 저장하는 것이다. 그런 다음 매퍼 집합을 실행하여 테이블 A를 읽으며 다음을 수행한다.
