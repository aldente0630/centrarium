...
지금까지 일반 조인과 맵 조인에 대해 설명했다. 이번 절에서는 **비대칭 조인**에 대해 논의할 것이다. 일반 조인에서 했던 이야기를 기억해보자. **일반 조인**의 중요한 문제점 중 하나는 데이터가 **비대칭일 때** 조인을 제대로 수행하지 못한다는 점이다. 리듀서 대부분이 조인 작업을 완료했어도 쿼리는 데이터가 쏠린 키에서 돌아가는 실행 시간이 가장 긴 리듀서를 기다리게된다.
  
**비대칭 조인**은 해당 문제 해결을 목표로 한다. 실행 시간에 데이터를 훑어보고 **hive.skewjoin.key** 매개 변수로 제어하는, 데이터가 굉장히 쏠린 키를 감지한다. 해당 키를 처리하는 대신 HDFS 디렉토리에 임시로 저장한다. 그 다음, 후속 맵 리듀스 작업을 통해 이 데이터가 쏠린 키를 처리한다. 동일한 키가 모든 테이블에 대해 비뚤어 질 필요가 없으므로 맵 조인이 될 수 있기 때문에 후속 map-reduce 작업 (기울어 진 키의 경우)이 훨씬 빠릅니다.
