...
지금까지 일반 조인과 맵 조인에 대해 설명했다. 이번 절에서는 **비대칭 조인**에 대해 논의할 것이다. 일반 조인에서 했던 얘기를 기억해보자. **일반 조인**의 주요 문제점 중 하나는 데이터가 **비대칭일 때** 조인이 제대로 수행되지 않는다는 점이다. 리듀서 대부분이 조인 작업을 완료하는 동안 쿼리는 분포가 쏠린 키에서 가장 긴 실행 리듀서를 기다리고 있습니다.
