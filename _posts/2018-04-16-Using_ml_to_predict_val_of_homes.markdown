...
학습 데이터를 구성하는데 여러 변수가 필요한 경우 Zipline은 지능형 key join을 자동으로 수행하고 뒤편에서 학습 데이터를 채워넣는다. 숙소 LTV 모형을 위해 기존 Zipline 변수를 사용하고 우리만의 변수를 일부 추가했다. 결론적으로 모형에 다음을 포함하는 변수 150개 이상이 사용되었다.
