...
### 두 번 이상 측정하라
  ...
  새로운 현상을 분석하려는 경우 기본적으로 분석할 내용을 똑같이 설정하고 다양한 접근법으로 측정해봐라. 그렇게 구한 여러 측정 값들이 일치하는지 확인하라. 복수의 측정 방식을 사용하면 측정, 로깅 코드의 버그, 기본 데이터의 예상하지 못한 변수 또는 중요한 필터링 단계를 식별 할 수 있습니다. 측정에 다른 데이터 소스를 사용할 수 있으면 더 좋습니다.
