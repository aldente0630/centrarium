...
### 표준 작업이 우선, 커스터마이징은 나중에
  ...
  변경하지 않아야 할 사항을 확인하는 변형입니다. 특히 신규 기능과 데이터를 살펴보다보면 해당 기능에 대해 새로운 지표를 특별히 만들어 바꿔서 측정하고 싶은 마음이 생긴다. 그러나 곧 변경할 것으로 예상하더라도 표준 지표를 항상 먼저 살펴봐야한다. 예를 들어 새로운 UI 기능을 검색 페이지에 추가할 때 이 새로운 UI 기능에 대해 지표를 특별히 만들어 측정하기 전에 결과에 대한 클릭 같이 표준 지표에 미치는 영향을 먼저 이해해야한다. 표준 지표의 유효성이 훨씬 향상되고 정확하기 때문에이 작업을 수행합니다. 새로운 맞춤 측정 항목이 표준 측정 항목에 맞지 않으면 새로운 맞춤 측정 항목이 잘못되었을 수 있습니다.
