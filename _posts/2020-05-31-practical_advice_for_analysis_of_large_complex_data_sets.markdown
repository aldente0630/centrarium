...
탐색 적 데이터 분석에 대해 3 가지 상호 관련된 단계가 있다고 생각합니다.
유효성 검사 또는 초기 데이터 분석 : 데이터가 일관성이 있고 데이터가 올바르게 수집되었으며 그 데이터가 내가 생각하는 것을 나타내는 것으로 생각합니까? 이것은 종종 “위생 검사”라는 이름으로갑니다. 예를 들어, 기능의 수동 테스트가 수행 된 경우 해당 수동 테스트의 로그를 볼 수 있습니까? 휴대 기기에서 시작된 기능의 경우 내 로그에 해당 기능이 데스크톱에 있다고 주장합니까?
설명 :이 데이터의 객관적인 해석은 무엇입니까? 예를 들어, “사용자는 7 개의 단어로 된 쿼리를 더 적게 수행합니까?”, “클릭 할 때 클릭 한 페이지로드 시간이 1 % 증가”및 “다음 페이지로 이동하는 사용자 수가 더 적음” 결과.”
평가 : 설명이 주어지면 데이터는 사용자, Google, 세계에 좋은 일이 일어나고 있다고 알려줍니까? 예를 들어 ‘사용자가 더 빨리 결과를 찾음’또는 ‘클릭 품질이 높음’
