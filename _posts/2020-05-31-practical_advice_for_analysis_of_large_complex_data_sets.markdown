...
- - -
  ...
  * *절차적인 영역*: 어떻게 데이터에 접근할지, 스스로 대답해야 할 질문은 무엇인지, 확인해야 할 내용은 무엇인지에 대한 안내.
...
### 데이터를 조각으로 나눠라
  ...
  두 집단을 비교하기 위해 데이터를 쪼갤 때(실험/대조군뿐만 아니라 A와 B 시간대 비교할 경우에도) 혼합 비율 변화에 신경 써야 한다. 혼합 비율의 변화란 비교하려는 집단에 따라 데이터 조각의 크기가 서로 다른 경우이다. [심슨의 역설](https://en.wikipedia.org/wiki/Simpson%27s_paradox)을 포함해 여러 혼선이 발생할 수 있다. 일반적으로 두 집단의 데이터 조각 크기가 동일하면 안전하게 비교, 분석할 수 있다.  
### 실질적으로 중요한지 생각하라
...
후자를 수행하는 가장 좋은 방법은 제외할 모집단에 대해 모든 지표를 실제 계산해보는 것이다. 해당 데이터를 보고 나면 ‘어떤 질의 건들이 필터링으로 제거됐는가?’ 같은 질문에 대답할 수 있다. 
  필터링 대상 관측치를 살펴보는 과정 또한 분석에 신규 필터링 단계를 적용할 경우 필수로 거쳐야 한다. 데이터 제외 규칙을 단순하 적용할 경우 ‘좋은’ 데이터까지 실수로 포함시키는 일이 비일비재하다.  
### 비율에는 명확한 분자와 분모가 있어야 한다
...
결과를 전달할 때 이를 명확히 해야 한다. 그렇지 않으면 잠재 고객(그리고 분석가 당신)이 과거 결과와 비교해서 지표를 바르게 해석하기 어려워진다.
