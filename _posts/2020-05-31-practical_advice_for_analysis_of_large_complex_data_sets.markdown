...
### 신규 측정 항목은 오래된 데이터/기능에 먼저 적용해라
  ...
  완전히 새로운 데이터를 수집해서 새로운 지표를 측정하려 한다면 그게 바르게 산출된 데이터 값인지 알 길이 없다. 새로운 종류의 데이터를 수집할 때 먼저 이 데이터로 알려진 기능에 적용해봐야한다. 예를 들어 사용자 만족도에 대한 새로운 지표가 있는 경우 최상의 기능이 만족을 돕는 데 도움이되는지 확인해야한다. 이렇게하면 새로운 것을 배우기 위해 갈 때 검증할 수 있다.
