...
### 재현성을 검증하라
  ...
  데이터를 조각으로 나누거나, 시간에 따라 일관성을 확인하는 작업은 재현성 검증의 일환으로 볼 수 있다. 발생한 현상이 중요하고 의미있다면 다른 모집단의 사용자에게도, 다른 시간대에서도 동일하게 발생해야한다. 그러나 재현성은 그 이상을 의미한다. 데이터를 활용해 모형을 만들었다면 해당 모형은 데이터의 미세한 값 변동에 안정적이어야한다. 다른 시간대 혹은 일부 무작위 표본을 사용하면 모형을 얼마나 신뢰할 수 있고 재현 가능한 성질의 것인지 알 수 있다. 재현해낼 수 없다면 데이터를 만들어낸 기저 활동에 관한 근원적인 정보를 제대로 담아내지 못했을 수 있다.
