...
### 동료와의 공유가 우선, 외부 고객은 나중에
  ...
  보고 싶은 결과가 따로 있는 고객과는 달리 숙련된 동료 검토자는 질적으로 다른 피드백 제공과 새너티 확인을 해줄 수 있다. 보고있는 데이터에 대해 잘 알고있는 동료가 제일 좋지만 데이터를 일반적으로 분석해본 경험만 있다면 그 동료의 피드백은 매우 유용할 것이다. 앞의 내용에서 새너티 확인과 검증을 스스로 올바르게 할 수 있는 몇 가지 방법을 제안했다. 그러나 동료와 공유하는 것이 이 모든 일을 스스로 수행하게 만드는 가장 좋은 방법 중 하나이다. 동료는 분석의 여러 지점에서 유용하다. 동료가 일찍이 알고있던 문제, 측정해야 할 항목에 대한 제안과 해당 분야의 과거 연구들에 대해 알려줄 수 있다. 즉, 동료는 이상한 점, 불일치 또는 다소 혼란스러운 내용을 매우 잘 지적해줄 수 있다.
