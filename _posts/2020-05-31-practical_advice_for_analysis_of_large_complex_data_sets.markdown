...
### 실질적으로 중요한지 생각하라
  ...
  반대로 종종 소량의 데이터가 가지고 있을 때가 있다. 상당한 지표 차이가 통계적으로 유의하지 않을 수 있지만 “중립”이라고 주장하는 것과는 또 다르다. 스스로 물어보자. “상당한 지표 차이가 실제로 존재할 가능성은 얼마나 될까?”
  
### 시간의 흐름에 따라 일관성을 점검하라

거의 항상 사용해야하는 특정 슬라이싱은 시간 단위로 분할하는 것입니다 (우리는 종종 일을 사용하지만 다른 단위도 유용 할 수 있습니다). 시스템이 시간이 지남에 따라 발전함에 따라 기본 데이터에 많은 방해가 발생하기 때문입니다. 일반적으로 기능의 초기 버전 또는 초기 데이터 수집을주의 깊게 검사하지만 문제가 발생하는 경우는 드물지 않습니다.

특정 요일이나 요일이 특이 치라고해서 폐기해야한다는 의미는 아닙니다. 데이터를 후크로 사용하여 삭제하기 전에 해당 날짜가 다른 인과 적 원인을 찾으십시오.

하루 동안의 데이터를 볼 때 얻을 수있는 또 다른 이점은 데이터의 변동을 감지하여 결국 신뢰 구간 또는 통계적 유의성에 대한 주장을 초래할 수 있다는 것입니다. 이것은 일반적으로 엄격한 신뢰 구간 계산을 대체하지는 않지만, 큰 변화로 인해 매일 그래프에서 통계적으로 유의미한 것을 볼 수 있습니다.
