...
### 데이터 분석은 데이터나 분석 기법이 아닌 질문에서 시작된다
  ...
  분석을 진행하는데는 항상 이유가 있다. 당신이 원하는 바를 질문 또는 가설 형태로 공들여 정리해두면 수집해야 할 데이터를 제대로 수집하고있는지, 데이터 상 괴리가 발생할지 판단하는데 도움이 될 것이다. 물론 질문은 데이터를 보면서 변할 수 있고 변해야만 한다. 그러나 질문이 없는 분석은 방향성 없이 끝난다.
