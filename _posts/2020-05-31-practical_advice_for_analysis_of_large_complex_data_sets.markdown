...
1. *검증 또는 [데이터 초기 분석](https://en.wikipedia.org/wiki/Data_analysis)*: 데이터가 균질하고 올바르게 수집됐으며 내가 생각한 대로 데이터가 형태를 갖추고 있는가? 이는 종종 “새너티 확인”라는 이름으로 불린다. 예를 들어 어떤 기능에 대해 수동 테스트를 수행한 경우 해당 테스트의 로그를 봐보자. 모바일을 위한 기능임에도 로그에는 데스크톱으로 기록되지 않았나?
2. *설명*: 데이터에 대한 객관적인 해석은 무엇인가? 예를 들어 “사용자는 일곱 단어의 검색어를 더 적게 질의하는가?”, “(클릭 시) 클릭한 페이지를 불러오는 시간이 1% 더 길다”와 “적은 비중의 사용자만이 다음 페이지 결과로 이동한다.” 등이다.
3. *평가*: 설명이 주어졌을 때 사용자에게, 또는 Google과 세상에 더 좋은 일이 실제로 벌어지고 있음을 데이터가 이야기하고 있는가? 예를 들어 ‘사용자가 더 빨리 결과를 찾는다’ 또는 ‘클릭 품질이 더 높다’ 등이다.
  
  이러한 단계를 분리하면 다른 사람과보다 쉽게 ​​합의 할 수 있습니다. 설명은 모든 사람이 데이터에서 동의 할 수있는 것이어야합니다. 데이터에 의미와 가치를 부여하기 때문에 평가는 훨씬 더 논쟁의 여지가 있습니다. 설명과 평가를 분리하지 않으면보고자하는 데이터의 해석 만 볼 가능성이 훨씬 높습니다. 또한 일반적으로 다른 기능 및 메트릭과의 엄격한 비교를 통해 메트릭의 표준 값을 설정하는 데 상당한 투자가 필요하기 때문에 평가가 훨씬 어려워집니다.

이 단계는 선형으로 진행되지 않습니다. 데이터를 탐색 할 때 단계간에 앞뒤로 이동할 수 있지만 언제든지 어떤 단계에 있는지 분명해야합니다.

### 실험/데이터 수집 설정을 확인하라

데이터를 확인하기 전에 실험 및 데이터 수집 설정을 이해해야합니다. 실험자와 분석가 사이의 정확한 의사 소통은 큰 도전입니다. 실험 프로토콜 또는 구성을 직접 볼 수 있다면 그렇게해야합니다. 그렇지 않으면 설정에 대한 자신의 이해를 적어두고 데이터 생성 담당자가 올바른지 확인하십시오.

비정상적이거나 잘못된 구성 또는 인구 제한 (예 : 특정 브라우저의 유효한 데이터)이있을 수 있습니다. 여기서 주목할만한 것은 나중에 이론을 구축하고 확인하는 데 도움이 될 수 있습니다. 고려해야 할 사항 :
제품의 기능이라면 직접 사용해보십시오. 당신이 할 수 없다면, 적어도 스크린 샷 / 행동 설명을 살펴보십시오.
실험이 실행 된 시간 범위 (휴일, 대규모 출시 등)에 특이한 것이 있는지 확인합니다.

### 데이터의 생체 신호를 확인하라

실제로 관심있는 질문에 대답하기 전에 (예 :“내 멋진 새로운 기능을 사용 했습니까?”) 관심있는 내용과 관련이 없지만 나중에 분석에 유용 할 수있는 다른 많은 사항을 확인해야합니다. 또는 데이터에 문제가 있음을 나타냅니다. 사용자 수가 변경 되었습니까? 모든 하위 그룹에 올바른 수의 영향을받는 쿼리가 표시 되었습니까? 오류율이 변경 되었습니까? 의사가 들어올 때 항상 키, 몸무게 및 혈압을 확인하는 것처럼 잠재적 인 큰 문제를 파악하기 위해 데이터 생체 신호를 확인하십시오.
이것은 “유효성 검사”단계에서 중요한 부분 중 하나입니다.

### 표준 작업이 우선, 커스터마이징은 나중에

변경하지 않아야 할 사항을 확인하는 변형입니다. 특히 새로운 기능과 새로운 데이터를 살펴보면이 새로운 기능에 대해 새롭거나 특별한 측정 항목으로 바로 이동하고 싶은 유혹이 있습니다. 그러나 변경 될 것으로 예상 되더라도 항상 표준 측정 항목을 먼저 살펴 봐야합니다. 예를 들어, 새로운 UI 기능을 검색 페이지에 추가 할 때이 새로운 UI 기능에 대한 특수 메트릭으로 들어가기 전에 결과 클릭과 같은 표준 메트릭에 미치는 영향을 이해해야합니다. 표준 측정 항목의 유효성이 훨씬 향상되고 정확하기 때문에이 작업을 수행합니다. 새로운 맞춤 측정 항목이 표준 측정 항목에 맞지 않으면 새로운 맞춤 측정 항목이 잘못되었을 수 있습니다.
