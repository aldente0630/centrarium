...
### 가설을 세우고 증거를 찾아라
  ...
  복잡한 문제의 경우 탐색적 데이터 분석을 보통 반복 진행한다. 그 과정을 통해 데이터의 이상치, 추세 또는 여타 특징을 발견할 수 있다. 자연스럽게 해당 데이터를 설명하는 가설을 수립한다. 수립한 가설이 진실이라고 손쉽게 여기지 않는게 중요하다. 해당 논리를 동의/반박할 증거(데이터 내부 또는 외부에서)를 찾아라. 예를 들어 카트만두의 휴일 또는 출시한 다른 기능으로 인해 이상치가 발생했다고 여겨지는 경우 해당 기능을 출시한 모집단에서만 이상치가 발생하는지 확인하라. 또는 변동의 규모가 출시 당시 예상한 정도와 일치하는지 확인하라. 
  
  좋은 데이터 분석 내용 속에는 이야기가 존재한다. 그 이야기가 올바른 것인지 확인하기 위해 이야기를 스스로 되새기고 해당 가설이 사실일 때 데이터에서 무얼 보아야하는지 가늠한다음 그것이 틀렸는지 증거를 찾아라. 이를 수행하는 방법 중 하나는 다음처럼 자문하는 것이다. “내가 만든 이야기를 검증/무효화하기 위해 어떤 실험을 수행해야하는가?” 이러한 실험을 수행하지 않거나 할 수 없어도 보유 데이터로 유효성을 확인하는 방법에 대해 아이디어를 줄 수 있다.
