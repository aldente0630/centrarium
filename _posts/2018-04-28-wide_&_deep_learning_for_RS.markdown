...
추천 시스템 논문들은 등급 행렬에 대한 컨텐츠 정보와 협업 필터링(CF)을 결합한 심층 학습을 사용해 협업 심층 학습을 탐구하였다. 또한 AppJoy 같이 사용자 앱 사용 기록을 CF에 사용하는 모바일 앱 추천 시스템도 있었다. CF 기반 또는 콘텐츠 기반 접근 방식의 이전 작업과 달리 우리는 공동으로 Wide & Deep 모형을 앱 추천 시스템에 대한 사용자 및 노출 데이터에 대해 교육합니다.
