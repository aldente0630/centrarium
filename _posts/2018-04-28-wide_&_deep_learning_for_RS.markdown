...
### 4.2 모형 훈련
  
실험에서 사용한 모형 구조는 그림 4와 같다. 훈련 과정에서 입력층은 훈련 데이터와 어휘를 받아들여 레이블과 함께 희소하거나 밀집한 변수를 생성한다. 넓은 부분 구성 요소는 사용자가 설치한 앱과 노출된 앱의 교차곱 변수 변환으로 구성된다. 모형 깊은 부분의 경우 32차원 임베딩 벡터가 각 범주형 변수에 대해 학습된다. 모든 임베딩이 밀집한 변수와 연결되어 약 1200 차원의 밀집한 벡터를 생성한다. 연결된 벡터는 3 ReLU 층으로 전달되고 마지막으로 로지스틱 출력 단위로 전달된다.

![앱 추천을 위한 Wide & Deep 모형 구조](https://aldente0630.github.io/assets/wide_&_deep_learning_for_RS4.png)
  
Wide & Deep 모형은 5천억 개가 넘는 샘플로 훈련된다. 새로운 일련의 학습 데이터 가 도착할 때마다,
