...
앱 추천 시스템에 대한 개요가 그림 2에 나와있다. 사용자가 앱 스토어를 방문하면 다양한 사용자 및 맥락 관련 변수가 포함 가능한 질의문이 생성된다. 추천 시스템은 사용자가 클릭이나 구매 같은 특정 작업을 수행할 수 있는 앱 목록(노출이라고도 함)을 반환한다. 이러한 사용자 동작은 질의 및 노출과 함께 학습기를 위한 훈련 데이터로 로그에 기록된다.
  
데이터베이스에는 100만 개가 넘는 앱이 있기 때문에 검색 대기 시간 요구 사항(대부분 O(10) 밀리세컨드)내에 모든 검색어에 대해 모든 앱에 철저한 점수를 매기는 것은 어렵습니다. 따라서 쿼리를 수신하면 첫 번째 단계는 검색입니다. 검색 시스템은 다양한 신호 (일반적으로 기계 학습 모델과 사람 정의 규칙의 조합)를 사용하여 쿼리와 가장 일치하는 항목의 짧은 목록을 반환합니다. 후보 풀을 줄이면 순위 시스템은 점수로 모든 항목의 순위를 매 깁니다. 점수는 대개 P (y | x), 사용자 기능 (예 : 국가, 언어, 인구 통계), 컨텍스트 특징 (예 : 기기, 시간대, 하루)을 포함하여 기능 x가 주어진 사용자 작업 라벨의 확률 노출 수 (예 : 앱 연령, 앱의 과거 통계). 본 논문에서는 Wide & Deep 학습 프레임 워크를 사용하여 순위 모델에 초점을 맞 춥니 다.
