...
범주형 변수 문자열을 정수 ID로 연결하는 어휘 테이블도 이 단계에서 생성된다. 시스템은 최소 횟수 이상으로 발생하는 문자열 변수 모두에 대해 ID 공간을 계산한다. 연속적인 실수값 변수는 변수값 \\(x\\)를 누적 분포 함수 \\(P(X\lex)\\)에 연결하여 \\(n_q\\) 분위수로 나누어 \\([0, 1]\\)로 정규화한다. 정규화한값은 \\(i\\)번째 분위수 값에 대해 {i-1 \over n_q-1}이다. 데이터 생성 동안 분위값 경계가 계산된다.
