...
## 6. 관련 작업
  
교차곱 변수 변환을 넓은 선형 모형과 밀집 임베딩을 통한 깊은 신경망을 결합하는 아이디어는 2 가지 변수 사이의 상호 작용을 두 제품 간 내적 (dot product)으로 분해함으로써 선형 모델에 일반화를 추가하는 인수 분해 기계 [5]와 같은 이전 연구에서 영감을 얻었습니다 낮은 차원의 임베딩 벡터. 이 논문에서는 점 제품 대신 신경망을 통한 임베딩 간의 비선형 상호 작용을 학습하여 모델 용량을 확장했습니다.
