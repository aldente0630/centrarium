...
앱 추천 시스템에 대한 개요가 그림 2에 나와있다. 사용자가 앱 스토어를 방문하면 다양한 사용자 및 맥락 변수를 포함할 수 있는 검색어가 생성됩니다. 추천 시스템은 사용자가 클릭이나 구매와 같은 특정 작업을 수행 할 수있는 앱 목록 (노출이라고도 함)을 반환합니다. 이러한 사용자 동작은 쿼리 및 노출과 함께 학습자를위한 학습 데이터로 로그에 기록됩니다.
