...
Wide 구성 요소와 Deep 구성 요소는 출력된 로그 확률의 가중치 합계를 예측치로 사용하여 결합 된 다음 공동 훈련을위한 하나의 일반적인 물류 손실 함수로 공급됩니다. *공동 훈련*과 *앙상블* 사이에는 구별이 있음에 유의하십시오. 앙상블에서, 개별 모델은 서로 알지 못하게 개별적으로 훈련되며, 그들의 예측은 추론 시간에만 결합되지만 훈련 시간에는 결합되지 않는다. 대조적으로, 공동 훈련은 모든 매개 변수를 동시에 넓고 깊은 부분뿐만 아니라 훈련 시간에 그들의 합계의 가중치를 고려해야합니다. 모델 크기에도 영향을 미칩니다. 앙상블의 경우 교육이 분리되어 있으므로 합리적인 정확도를 얻기 위해 각각의 모델 크기가 더 커야합니다 (예 : 더 많은 기능과 변형이 필요한 경우). 이와는 대조적으로, 공동 훈련의 경우 넓은 부분은 전체 크기의 와이드 모델보다는 소수의 교차 제품 기능 변환으로 깊은 부분의 약점을 보완하기 만하면됩니다.
  
전체 크기의 와이드 모델이 아닌 피쳐 변환. Wide & Deep Model의 공동 교육은 미니 배치 확률 적 최적화를 사용하여 출력의 그라디언트를 모델의 넓고 깊은 부분까지 동시에 역 전파함으로써 수행됩니다. 실험에서 우리는 모델의 넓은 부분에 대한 최적화로서 L1 정규화를 따르는 FTRL 알고리즘을 사용하였고 심층 부분에 대해서는 AdaGrad [1]을 사용 하였다.
  
결합 된 모델은 그림 1 (가운데)에 나와 있습니다. 로지스틱 회귀 문제의 경우 모델의 예측은 다음과 같습니다.
