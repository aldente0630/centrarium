...
언어 모형에서 입출력 간 직접적인 가중치를 학습함으로써 RNN 복잡도(예: 은닉층 크기)를 크게 줄이는 형태의 순환신경망(RNNs)과 n-gram 변수를 사용한 최대 엔트로피 모형을 공동 훈련시키는 방법론이 제안되었다. 컴퓨터 비전에서 심층 잔여 학습은 더 깊은 모델을 훈련하는 어려움을 줄이고 하나 이상의 레이어를 건너 뛰는 지름길 연결을 사용하여 정확도를 향상시키는 데 사용되었습니다. 그래픽 모델을 이용한 신경망의 공동 훈련도 이미지로부터 사람의 자세 추정에 적용되었다 [6]. 이 작업에서 우리는 희소 한 특징과 출력 단위 사이의 직접 연결, 희소 한 입력 데이터로 인한 일반적인 추천 및 순위 지정 문제에 대한 피드 - 포워드 신경망 및 선형 모델의 공동 훈련을 탐구했습니다.

추천 시스템 논문들은 등급 행렬에 대한 컨텐츠 정보와 협업 필터링(CF)을 결합한 딥러닝을 사용해 협업 심층 학습을 탐구하였다. 또한 AppJoy 같이 사용자 앱 사용 기록을 CF에 사용하는 모바일 앱 추천 시스템도 있었다. CF 기반 또는 콘텐츠 기반 접근 방식의 이전 작업과는 달리 우리는 앱 추천 시스템의 사용자와 노출 데이터에 대해 Wide & Deep 모형을 공동 훈련시켰다.
