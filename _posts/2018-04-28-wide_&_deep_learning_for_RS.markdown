...
### 4.2 모형 제공
  
모형이 훈련되고 검증되면 모형 서버에 모형을 적재한다. 각 요청에 대해 서버는 앱 검색 시스템에서 앱 후보 집합을 수신하고 사용자 기능을 사용하여 각 앱에 점수를 매 깁니다. 그런 다음 앱은 가장 높은 점수에서 가장 낮은 점수로 순위가 매겨지며이 순서대로 사용자에게 앱을 표시합니다. 점수는 Wide & Deep 모델에 대해 순방향 추론을 실행하여 계산됩니다.
  
