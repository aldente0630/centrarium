...
넓은 선형 모델과 교차 제품 특징 변환 및 밀도가 높은 삽입을 가진 심 신경 네트워크를 결합한다는 아이디어는 2 가지 변수 사이의 상호 작용을 두 제품 간 내적 (dot product)으로 분해함으로써 선형 모델에 일반화를 추가하는 인수 분해 기계 [5]와 같은 이전 연구에서 영감을 얻었습니다 낮은 차원의 임베딩 벡터. 이 논문에서는 점 제품 대신 신경망을 통한 임베딩 간의 비선형 상호 작용을 학습하여 모델 용량을 확장했습니다.

언어 모델에서 n-gram을 사용한 반복적 인 신경 회로망 (RNNs)과 최대 엔트로피 모델의 공동 훈련 입력 및 출력 간의 직접 가중치를 학습함으로써 RNN 복잡성 (예 : 숨겨진 계층 크기)을 크게 줄이는 기능이 제안되었습니다 [4]. 컴퓨터 비전에서 깊은 잔여 학습 [2]은 더 깊은 모델을 훈련하는 어려움을 줄이고 하나 이상의 레이어를 건너 뛰는 지름길 연결을 사용하여 정확도를 향상시키는 데 사용되었습니다. 그래픽 모델을 이용한 신경망의 공동 훈련도 이미지로부터 사람의 자세 추정에 적용되었다 [6]. 이 작업에서 우리는 희소 한 특징과 출력 단위 사이의 직접 연결, 희소 한 입력 데이터로 인한 일반적인 추천 및 순위 지정 문제에 대한 피드 - 포워드 신경망 및 선형 모델의 공동 훈련을 탐구했습니다.

추천 시스템 문헌에서 컨텐츠 매트릭스에 대한 깊이있는 학습과 등급 매트릭스에 대한 협업 필터링 (collaborative filtering)을 결합하여 협력적인 심층 학습이 탐구되었다. 또한 사용자의 앱 사용 기록에 CF를 사용하는 AppJoy와 같은 모바일 앱 추천 시스템에서도 이전의 작업이있었습니다 [8]. 이전 작업에서 CF 기반 또는 콘텐츠 기반 접근 방식과 달리, 우리는 공동으로 W 딥 모델을 app recommender 시스템에 대한 사용자 및 노출 데이터에 대해 교육합니다.
  
## 7. 결론
  
암기 및 일반화는 추천 시스템에서 중요합니다. 넓은 선형 모델은 교차 제품 기능 변환을 사용하여 스파 스 기능 상호 작용을 효과적으로 암기 할 수 있지만 심층 신경 네트워크는 저 차원 임베딩을 통해 이전에는 보이지 않는 기능 상호 작용으로 일반화 할 수 있습니다. 두 가지 유형의 모델의 장점을 결합하기 위해 Wide & Deep 학습 프레임 워크를 제시했습니다. 대규모 상업용 앱 스토어 인 Google Play의 추천 시스템에서 프레임 워크를 제작하고 평가했습니다. 온라인 실험 결과에 따르면 Wide & Deep 모델은 와이드 전용 모델과 Deep-Only 모델에 비해 앱을 크게 개선했습니다.
