...
Deep 구성 요소는 그림 1(우측)과 같은 피드-포워드 신경망이다. 범주형 변수의 경우 원래 입력값은 변수 문자열(예: "language=en")이다. 이러한 희소하고 고차원적인 범주형 특징 각각은 먼저 종종 임베딩 벡터라고하는 저 차원 및 고밀도 실수 값 벡터로 변환됩니다. 삽입의 차원은 일반적으로 O (10)에서 O (100)의 순서입니다. 매입 벡터는 무작위로 초기화되고 모델 훈련 중 최종 손실 함수를 최소화하도록 값이 훈련됩니다. 이러한 저 차원의 고밀도 임베딩 벡터는 순방향 패스에서 신경망의 숨겨진 레이어로 공급됩니다. 구체적으로, 각
