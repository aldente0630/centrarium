...
상용 모바일 앱 스토어는 트래픽이 많기 때문에 짧은 대기 시간 내로 높은 수준의 처리량을 유지하며 서비스하기가 까다롭다. 최고의 트래픽에서 추천 서버는 1 초당 천만 개 이상의 앱을 기록합니다. 단일 스레딩을 사용하면 모든 후보를 단일 일괄 처리로 채점하는 데 31 밀리 초가 걸립니다. 우리는 멀티 스레딩을 구현하고 각 배치를 작은 크기로 분할하여 표 2에 표시된대로 클라이언트 측 대기 시간을 14ms (서비스 오버 헤드 포함)로 크게 줄였습니다.
