...
일반적인 컨텍스츄얼 밴딧 문제에서 특별히 중요한 경우는 (ⅰ) 슬롯 손잡이의 집합 \\(\mathcal{A}_t\\)이 변하지 않으며 모든 \\(t\\)마다 슬롯 손잡이가 \\(K\\)개인 (ⅱ) 사용자 \\(u_t\\)(또는 이와 동치로 맥락 \\(\(\mathbf{x}\_{t, 1}, ,\mathbf{x}\_{t, K}\)\\) )가 모든 \\(t\\)마다 동일한, 잘 알려진 *\\(K\\)개의 슬롯 손잡이를 갖는 다중 슬롯머신 실험 문제*이다. 팔 세트와 컨텍스트는 매 시도마다 일정하기 때문에 적기 알고리즘과 아무런 차이가 없으므로이 유형의 적기를 상황없는 적기라고 부른다.
  
기사 추천의 맥락에서 우리는 수영장에있는 기사를 팔로 볼 수 있습니다. 선물 된 기사를 클릭하면 1의 보수가 발생합니다. 그렇지 않으면 보수는 0입니다.이 지불금 정의에 따라 기사의 예상 수익은 정확히 클릭률 (CTR)이며, 최대 클릭률 (CTR)이있는 기사를 선택하는 것은 사용자의 예상 클릭 수를 최대화하는 것과 같습니다. 우리의 적기 공식에서 총 기대 보수를 극대화하는 것과 같다.
