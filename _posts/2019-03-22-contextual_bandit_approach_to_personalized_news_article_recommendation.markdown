...
보다 정확하게 \\((\mathbf{x}_1, \ldots, \mathbf{x}_K, r_1, \ldots, r_K)\\) 형태의, 즉 관측한 변수 벡터와 모든 슬롯 손잡이에 대해 *감춰진* 손익으로 구성된 튜플을 i.i.d.로 추출 가능한 미지의 분포 \\(D\\)가 있다고 가정하자. 또한 로깅 정책과 현실 세계 간의 상호 작용으로 인해 발생한 대량의 사건 기록을 이용할 수 있다고 하자. 각각의 사건은 맥락 벡터 \\(\mathbf{x}_1, \ldots, \mathbf{x}_K\\), 선택된 슬롯 손잡이 \\(a\\)와 결과적으로 관측한 손익 \\(r_a\\)로 구성된다. 결정적으로, 무작위로 균일하게 선택된 단일 암 \\(a\\)에 대해 지불 \\(r_a\\)만이 관찰된다. 간단한 표현을 위해, 우리는이 일련의 기록 된 이벤트를 무한히 긴 스트림으로 취합니다. 그러나 우리는 또한 평가 방법에 필요한 실제 유한 한 이벤트 수에 대한 명확한 한계를 제시합니다.
