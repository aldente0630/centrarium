...
목표는 해당 데이터를 사용하여 밴딧 알고리즘 \\(\pi\\)를 평가하는 것이다. 공식적으로 \\(\pi\\)는 현 시점의 맥락 벡터\\(\mathbf{x}_1, \ldots, \mathbf{x}_K\\)와 함께 \\(t-1\\)개의 이전 시점 이력 \\(h_t-1\\)을 기반으로 현 시점 \\(t\\)에서 슬롯 손잡이를 선택하는 (무작위성이 가미된) 연결 논리이다.
