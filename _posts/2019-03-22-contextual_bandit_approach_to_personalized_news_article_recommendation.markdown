...
맥락 무관한 다중 슬롯머신 알고리즘에 대한 UCB 방법의 점근적 최적성, 그리고 강력한 리그릿 수준을 감안할 때 컨텍스츄얼 밴딧 문제에 대해서도 유사한 알고리즘을 고안해볼 만하다. 손익 함수가 모수적 형태로 주어진다고 가정할 때 추정 된 팔 보수의 UCB를 계산할 수있는 모수의 신뢰 구간을 데이터로부터 추정하기 위한 수많은 방법이 존재한다. 그러나 이러한 접근법은 일반적으로 매우 큰 비용이 들어간다.
