...
3.알고리즘은 새로운 관측치 \\(\(\mathbf{x}\_{t, a}, a_t, r\_{t, a_t}\)\\)를 이용하여 슬롯 손잡이 선택 전략을 개선한다. 여기에서 중요한 것은 *선택하지 않은* 슬롯 손잡이 \\(a = a_t\\)에 대해서 관측되는 피드백(즉, 손익 \\(r\_{t, a_t}\\)) 또한 *없다*는 점이다. 이 사실로부터 이어지는 결론은 다음 절에서 좀 더 자세히 논의할 것이다.  
