...
알고리즘 1은 입력 파라미터가 오직 \\(\alpha\\)인 **LinUCB** 알고리즘 전반에 대하여 자세하게 설명한다. 식 (4)에 나타난 \\(\alpha\\)를 주목하라. 이 값은 일부 응용 프로그램에서 보수적으로 큰 값을 취할 수 있으므로 매개 변수를 최적화하면 전체 손익을 높일 수 있다. 모든 UCB 방법과 마찬가지로 **LinUCB**는 UCB가 가장 높은 슬롯 손잡이를 항상 선택한다(식 (5)와 같다).
  
이 알고리즘은 몇 가지 좋은 성질이 있다. 첫째, 계산 복잡도가 슬롯 손잡이 개수에 선형이며 변수 개수에 최대 세제곱이다. 계산량을 더 줄이기 위해 모든 단계(\\(O(d^2)\\) 시간이 걸리는)에서 \\(A_{a_t}\\)를 실시간으로 업데이트하는 대신 주기적으로 Qa def = A-1 a (모두 a)를 계산하고 캐시합니다. 둘째, 알고리즘은 동적 암 세트에 대해 잘 작동하며 At의 크기가 너무 크지 않은 한 효율적입니다. 이 경우는 많은 애플리케이션에서 마찬가지입니다. 예를 들어, 뉴스 기사 추천에서 편집자는 기사를 풀에 추가 / 제거하고 풀 크기는 본질적으로 일정합니다. 셋째, 본 논문의 초점은 아니지만 [6]의 분석을 다음과 같이 나타낼 수있다. At 세트가 고정되어 있고 K 개의 팔을 포함하고 있다면, 신뢰 구간 (즉, 오른손 (4)의 측면)은 점점 더 많은 데이터로 충분히 빠르게 감소하고 최첨단 결과 [6]와 일치하는 O ~ (√ KdT)의 강한 후회 경계를 증명한다.
