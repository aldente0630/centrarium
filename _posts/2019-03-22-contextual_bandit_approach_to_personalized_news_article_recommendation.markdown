...
개별 사건이 i.i.d.이고 데이터 기록을 수집하는데 사용한 로깅 정책이 각 시점마다 슬롯손잡이를 균일하게 무작위로 선택했다고 가정하자. 본 가정 하에 평가를 수행하는 신뢰도 높은 기법을 이 절에서 설명하겠다. 세부적인 내용은 생략하겠지만 임의의 로깅 정책이 허용되게끔 후자의 가정을 느슨하게 가져갈 수 있다. 또 본 해법이 리젝션 샘플링을 사용하게 수정할 수 있지만 그럴 경우 데이터 사용의 효율이 떨어진다.
