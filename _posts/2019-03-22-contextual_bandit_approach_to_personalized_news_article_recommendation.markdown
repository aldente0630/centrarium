...
보다 정확하게, 우리는 튜플이 그려지는 알려지지 않은 분포 \\(D\\)가 있다고 가정합니다. \\((\mathbf{x}_1, \ldots, \mathbf{x}_K, r_1, \ldots, r_K)\\) 형식으로 표시되며, 각각은 모든 특징점에 대해 관측 된 특징 벡터와 숨겨진 페이 오프로 구성됩니다. 또한 로깅 정책과 세계의 상호 작용으로 인해 발생하는 대량의 기록 된 이벤트에 액세스 할 수 있습니다. 각각의 이러한 이벤트는 컨텍스트 벡터 \\(\mathbf{x}_1, \ldots, \mathbf{x}_K\\), 선택된 암 \\(a\\) 및 결과적인 관찰 된 지불금 \\(r_a\\)로 구성됩니다. 결정적으로, 무작위로 균일하게 선택된 단일 암 \\(a\\)에 대해 지불 \\(r_a\\)만이 관찰된다. 간단한 표현을 위해, 우리는이 일련의 기록 된 이벤트를 무한히 긴 스트림으로 취합니다. 그러나 우리는 또한 평가 방법에 필요한 실제 유한 한 이벤트 수에 대한 명확한 한계를 제시합니다.
