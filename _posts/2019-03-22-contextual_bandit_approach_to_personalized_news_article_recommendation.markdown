...
지도 학습 상황에서 진행하는 보다 표준화된 기계 학습과 비교하자면 컨텍스츄얼 밴딧의 평가 방법론은 꽤나 까다롭다. *밴딧 알고리즘* \\(\pi\\), 즉 앞서 발생한 상호 작용을 기반으로 각 시점마다 슬롯 손잡이를 선택하는 규칙(예컨대 위에서 설명한 알고리즘)의 성능을 측정하는 것이 목표이다. 그러나 상호작용적인 특성 때문에 "라이브" 데이터 위에서 알고리즘을 실제로 수행해보는 것 외에 목표를 달성할 방법이 없어보인다. 그러나 서비스에 들어갈 비용을 생각하면 이런 접근법은 사실상 실행 불가능하다. 지금 알고리즘과 완전히 *다른* 예전의 로깅 정책을 사용해 수집한 *오프라인* 데이터만 이용 가능할 수 있다. 로깅 정책에 의해 선택된 슬롯 손잡이에서만 결과 값이 관찰되고 이는 평가 대상 알고리즘 \\(\pi\\)에 의해 선택될 슬롯손잡이의 값과 매우 *다를* 것이기에 기록된 데이터에만 의존하여 \\(\pi\\)를 어떻게 평가할 수 있을지 명확하지 않다. 이 평가 문제는 강화 학습에서의 "오프-폴리시 평가"의 특별한 경우로 볼 수 있다(해당 논문[^4]를 참조하라).
...
이 섹션에서는 개별 이벤트가 i.i.d.이고 로그 된 데이터를 수집하는 데 사용 된 로깅 정책이 각 시간 단계에서 무작위로 균일하게 각 팔을 선택한 것으로 가정하여 이러한 평가를 수행하기 위해 신뢰할 수있는 기술을 설명합니다. 세부 사항을 생략하더라도 임의의 로깅 정책이 허용되도록 후자의 가정을 상당히 약화시킬 수 있습니다.
우리의 솔루션은 리 젝션 샘플링을 사용하여 그에 따라 수정 될 수 있지만 데이터 사용의 효율성이 저하됩니다.
  
보다 정확하게, 우리는 튜플이 그려지는 알려지지 않은 분포 D가 있다고 가정합니다. (x1, ..., xK, r1, ..., rK) 형식으로 표시되며, 각각은 모든 특징점에 대해 관측 된 특징 벡터와 숨겨진 페이 오프로 구성됩니다. 또한 로깅 정책과 세계의 상호 작용으로 인해 발생하는 대량의 기록 된 이벤트에 액세스 할 수 있습니다. 각각의 이러한 이벤트는 컨텍스트 벡터 x1, ..., xK, 선택된 암 a 및 결과적인 관찰 된 지불금 ra로 구성됩니다. 결정적으로, 무작위로 균일하게 선택된 단일 암 a에 대해 지불 ra만이 관찰된다. 간단한 표현을 위해, 우리는이 일련의 기록 된 이벤트를 무한히 긴 스트림으로 취합니다. 그러나 우리는 또한 평가 방법에 필요한 실제 유한 한 이벤트 수에 대한 명확한 한계를 제시합니다.
