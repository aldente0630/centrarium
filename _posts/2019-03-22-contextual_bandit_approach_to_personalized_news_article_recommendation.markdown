...
정책 평가자 내의 *모든* 이력 \\(h_T\\)가 현실 세계에서 발생할 확률과 동일하다고 이 정리는 말한다. 알고리즘 3이 반환한 평균 손익 \\(R_T / T\\)와 같은 이러한 이력에 대한 많은 통계는 알고리즘 π의 값에 대한 편견없는 추정치입니다. 또한 정리에 따르면 KT 기록 된 이벤트는 크기 T의 표본을 보유해야합니다.
