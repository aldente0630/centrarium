...
$$R_{\mathsf{A}}(T) \overset{\underset{\mathrm{def}}{}}{=} \mathbf{E}[\sum_{t = 1}^T r_{t, a_t^*}] - \mathbf{E}[\sum_{t = 1}^T r_{t, a_t}]$$
...
적기에 대한 근본적인 문제는 탐사와 착취의 균형을 맞추어야한다는 것입니다. Eq. (1) 알고리즘 A는 과거의 경험을 바탕으로 가장 잘 나타나는 팔을 선택합니다. 반면에,이 겉으로보기에 최적 인 팔은 A의 지식에 부정확 함으로 인해 차선책 일 수 있습니다. 이러한 바람직하지 않은 상황을 피하기 위해, A는 그들에 대한 더 많은 정보를 수집하기 위해보기 흉한 무기를 실제로 선택함으로써 탐구해야한다 (c.f., 이전 하위 섹션에서 정의 된 적기 프로세스의 3 단계). 일부 최적의 무기가 선택 될 수 있으므로 탐사는 단기 후회를 증가시킬 수 있습니다. 그러나 무기의 평균 보수 (즉, 탐사)에 대한 정보를 얻는 것은 A의 무기 보수에 대한 추정을 개선 할 수 있으며, 결과적으로 장기적인 후회를 줄일 수 있습니다. 분명히 순수하게 탐색하는 알고리즘이나 순수하게 악용 알고리즘은 일반적으로 가장 잘 작동하지 않으며 좋은 절충이 필요합니다.
