...
마지막으로 입력 변수 \\(\mathbf{x}_{t, a}\\)를 정규분포에서 i.i.d로 추출한다는 가정 하에서(식 (2)의 모형화 가정에 덧붙여) Pavlidis 등은 UCB를 계산하기 위해 릿지 회귀의 해(식 (3)의 \\(\boldsymbol{\theta}_a\\)) 대신 최소 자승 해 ~ a를 사용하는 유사한 알고리즘을 제안했습니다. 그러나 우리의 접근법 (및 이론적 분석)은보다 일반적이며, 입력 특징이 비 정적 인 경우에도 유효합니다. 더 중요한 것은, 우리는 다음 절에서 기본 알고리즘 1을 Pavlidis 등이 다루지 않는 훨씬 더 흥미로운 경우로 확장하는 방법에 대해 논의 할 것이다.
