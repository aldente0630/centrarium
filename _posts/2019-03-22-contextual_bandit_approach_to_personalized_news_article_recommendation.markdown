...
마지막으로 기틴스 지수 방법처럼 베이즈 법칙에 기반하여 또 다른 종류의 다중 슬롯머신 알고리즘이 존재한다. 베이즈 접근법은 정의한 사전 분포이 적절한 경우 좋은 성능을 발휘한다. 이 방법은 적합한 사전 모형을 얻기 위해 오프라인 엔지니어링이 방대하게 필요하며 근사 기법을 결합시키지 않으면 계산이 불가능하다.
  
# 3. 알고리즘
  
맥락 무관한 다중 슬롯머신 알고리즘에 대한 UCB 방법의 점근적 최적성, 그리고 강력한 리그릿 수준을 감안할 때 컨텍스츄얼 밴딧 문제에 대해서도 유사한 알고리즘을 고안해볼 만하다. 일부 감안할 때
