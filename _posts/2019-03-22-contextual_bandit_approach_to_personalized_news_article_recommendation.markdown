...
사용자와 컨텐츠는 흔히 변수의 집합으로 표현된다. 사용자 변수는 정의한 인구 통계 정보뿐만 아니라 집계 수준의 활동 기록을 포함할 수 있다. 컨텐츠 변수는 설명 정보와 범주를 포함할 수 있다. 이 시나리오에서는 동일한 컨텐츠에 대해 사용자마다 시각이 크게 다를 수 있으므로 탐색과 활용은 개별 수준으로 배포해야한다. 매우 많은 수의 가능한 선택 또는 동작이있을 수 있으므로 콘텐츠 항목 간의 공통점을 인식하고 해당 지식을 콘텐츠 풀로 전송하는 것이 중요합니다.
