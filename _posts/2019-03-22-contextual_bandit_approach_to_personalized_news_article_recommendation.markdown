...
다른 정보는 사용자를 식별하는 데 사용되었습니다. 유사하게, 각 기사는 동일한 방식으로 구성된 약 100 개의 범주 형 특징의 원시 특징 벡터로 표현되었다. (i) URL 범주 : 기사 리소스의 URL에서 유추 된 수십 개의 클래스; 및 (ii) 편집자 카테고리 : 기사 내용을 요약하기 위해 인간 편집자가 태그를 붙인 수십 개의 주제.
  
기사 내용을 요약하기 위해 인간 편집자가 태그 한 주제. 앞의 절차 [12]에 따라 범주 형 사용자 / 문서 기능을 이진 벡터로 인코딩 한 다음 각 기능 벡터를 단위 길이로 정규화했습니다. 또한 각 피쳐 벡터를 상수 피쳐 값 1로 보강했습니다. 이제 각 기사와 사용자는 각각 83과 1193 개의 피쳐 벡터로 표시되었습니다.
  
이러한 원시 기능을 사용하여 2008 년 9 월에 수집 된 무작위 탐색 데이터를 기반으로 공동 분석을 수행했습니다. 차원 축소에 대한 이전의 접근 방식 [13]에 따라 사용자 기능을 기사 범주에 투영 한 다음 유사한 선호도를 가진 사용자를 그룹으로 묶었습니다. 더 구체적으로:
