...
본 논문에서 제안하는 정책 평가자는 알고리즘 3에 나와있다. 이 방법론은 정책 \\(\pi\\)와 평가의 기초인 "양질의" 사건에 대해 원하는 횟수 \\(T\\)를 입력으로서 취한다. 그런 다음 기록된 사건의 스트림을 하나씩 살펴본다. 주어진 현재 이력 \\(h\_{t-1}\\)에 대해 정책 \\(\pi\\)이 선택한 슬롯 손잡이 \\(a\\)가 로깅 정책에 의해 선택한 것과 동일하면 발생 사건을 보존한다. 즉, 이력에 추가하고 총 손익 \\(R_t\\)을 업데이트한다. 반대로 정책 \\(\pi\\)가 선택한 슬롯 손잡이가 로깅 정책에 의해 취한 것과 다르면 발생 사건은 무시하고 알고리즘은 상태의 변경 없이 다음 사건으로 진행한다.
  
로깅 정책은 각 슬롯 손잡이를 무작위로 균일하게 선택하기 때문에 각 이벤트는 다른 알고리즘과 상관없이 정확히 1 / K의 확률로이 알고리즘에 의해 유지됩니다. 이는 보유 된 이벤트가 D에 의해 선택된 것과 동일한 분포를 가짐을 의미합니다. 결과적으로 두 프로세스가 동일하다는 것을 증명할 수 있습니다. 첫 번째는 D의 T 실제 이벤트에 대한 정책을 평가하고, 두 번째는 기록 된 이벤트 스트림에서 정책 평가기를 사용하여 정책을 평가하는 것입니다.
