...
## 5.3 비교 알고리즘  
  ...
  **I. 변수를 사용하지 않는 알고리즘.** 이들은 모든 맥락(예: 사용자 / 기사 정보)를 무시하는, 맥락 무관한 슬롯 손잡이 \\(K\\)개에 대한 밴딧 알고리즘에 해당한다.
  ...
  * **ucb**: 섹션 2.2에 설명 된대로이 정책은 각 기사의 CTR과 추정치의 신뢰 구간을 추정하고 항상 UCB가 가장 높은 기사를 선택합니다. 특히 UCB1에 따라 기사 \\(a\\)의 신뢰 구간을 \\(c_{t, a} = {a \over \sqrt{n_{t, a}}}\\)로 계산했습니다. 여기서 \\(n_{t, a}\\)는 시험 \\(t\\) 전에 \\(a\\)를 선택한 횟수이고 \\(α > 0\\)은 모수입니다.  
  * **전지전능성:** 이러한 정책은 경험적 맥락이없는 최상의 CTR을 *가늠자로* 달성합니다. 먼저 기록 된 이벤트에서 각 기사의 경험적 CTR을 계산 한 다음 동일한 기록 된 이벤트를 사용하여 평가할 때 항상 경험적 CTR이 가장 높은 기사를 선택합니다. 이 알고리즘은 매개 변수가 필요하지 않으며 시간이 지남에 따라 "학습"하지 않습니다.
   
 **II. "웜 스타트"를 적용한 알고리즘.** - 개인화 된 서비스를 향한 중간 단계. 아이디어는 전체 트래픽에서 기사의 문맥없는 CTR을 오프라인에서 예측 한 사용자 별 조정을 제공하는 것입니다. 오프셋은 새 콘텐츠 (예 : "웜 스타트")에 대한 CTR 추정치의 초기화 역할을합니다. 우리는 위에서 구축 된 특징 \\(\mathbf{z}\_{t, a}\\)를 사용하여 2008 년 9 월의 랜덤 트래픽 데이터에 대해 [12]에서 연구 한 쌍 선형 로지스틱 회귀 모델을 재교육했다. 그러면 선택 기준이 상황에 따른 CTR 추정치와 사용자 별 CTR 조정에 대한 쌍 선형 항의 합이됩니다. 훈련에서 CTR은 \\(\epsilon = 1\\) 인 문맥없는 \\(\epsilon\\)-탐욕을 사용하여 추정되었습니다.
 
  * **\\(\epsilon\\)-탐욕 (웜)**: 이 알고리즘은 기사의 문맥없는 CTR 추정치에 사용자 별 CTR 수정을 추가한다는 점을 제외하면 \\(\epsilon\\)-탐욕와 동일합니다.
  * **ucb (웜)**: 이 알고리즘은 이전 알고리즘과 동일하지만 \\(\epsilon\\)-탐욕을 ucb로 대체합니다.
    
**III. 온라인에서 사용자 별 CTR을 배우는 알고리즘.** 
  
  * **\\(\epsilon\\)-탐욕 (세분화)**: 각 사용자는 섹션 5.2.2에서 구성된 5 개 중 가장 가까운 사용자 클러스터에 할당되므로 모든 사용자는 5 개의 그룹 (즉, 사용자 세그먼트)으로 분할되며 각 그룹은 별도의 \\(\epsilon\\)-탐욕 사본이 실행됩니다.  
  * **ucb (세분화)**: 이 알고리즘은 5 개의 사용자 세그먼트 각각에서 ucb의 복사본을 실행 한 것을 제외하고는 \\(\epsilon\\)-탐욕 (세분화)와 유사합니다.  
  * **\\(\epsilon\\)-탐욕 (배타)**: 이 모델은 \\(\epsilon\\)-탐욕입니다. **에포크 탐욕**의 밀접한 변형으로 볼 수있다. 
  * **linucb (배타)**: 비 연속 모델이있는 알고리즘 1입니다.
  * **ε-greedy (혼합)**: 하이브리드 모델의 경우 \\(\epsilon\\)-탐욕입니다. **에포크 탐욕**의 변종으로 볼 수 있습니다.
  * **linucb (하이브리드)**: 하이브리드 모델이있는 알고리즘 2입니다.  
