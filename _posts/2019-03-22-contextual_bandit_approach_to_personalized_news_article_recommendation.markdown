...
**그림 2: 파라미터 조정: 조정 데이터셋 1일치에 대한 여러 알고리즘의 CTR.**
...
두 번째로, 변수 없는 버전의 \\(\epsilon\\)-탐욕과 UCB에 비해 웜 스타트 정보가 사용자 관심사와 기사 컨텐츠 간의 더 나은 대응을 찾는데 큰 도움이 됨을 도표을 통해 알 수 있다. 특히 **\\(\epsilon\\)-탐욕(웜 스타트)** 와 **ucb (웜 스타트)** 는 전적으로 맥락 무관한 정책으로 달성할 수있는 CTR 최고치인 전지적 정책을 능가했다. 그러나 웜 스타트 정보를 사용하는 두 알고리즘의 성능은 온라인으로 가중치를 학습하는 알고리즘만큼 안정적이지 않다. “웜 스타트”에 대한 오프라인 모형은 무작위 트래픽 전체에 대해 계산한 기사 CTR로 훈련시켰으므로 **\\(\epsilon\\)-탐욕(웜 스타트)** 는 \\(\epsilon\\)이 1에 가까울 때 배포 버킷에서 보다 안정적인 성능을 얻는다. 또한 처음부터 사용자에게 더 많은 기사를 선택하여 학습 버킷에서 ucb (따뜻한)를 돕지 만 배치에 가장 적합한 온라인을 결정하는 데 ucb (웜)는 도움이되지 않았습니다. ucb는 탐색을위한 신뢰 구간에 의존하기 때문에 "warm start"에 의해 도입 된 초기화 바이어스를 수정하는 것은 어렵습니다. 대조적으로, 모든 온라인 학습 알고리즘은 전적으로 정책을 능가 할 수있었습니다. 따라서 평가 데이터에 대해 웜 스타트 알고리즘을 시도하지 않았습니다.
