...
뉴스 기사 개인화 추천 문제는 자연스럽게 맥락 상의 정보가 있는 다중 슬롯머신 문제로 모형화할 수 있다. 이전 연구에 따라 이를 *컨텍스츄얼 밴딧*[^1]이라고 부를 것이다. 수식화해보자면 컨텍스츄얼 밴딧 알고리즘 \\(\mathst{A}\\)는 개별 시도 \\(t = 1, 2, 3, \ldots \\)에 따라 수행된다. \\(t\\)번째 시도에 대하여,
  
1.알고리즘은 현재 사용자 \\(u_t\\)와 슬롯 손잡이 또는 행동의 집합 \\(\mathcal{A}_t\\)과 함께 \\(a \in \mathst{A}\\)일 때의 변수 벡터 \\(\mathcbf{x}_{t, a}\\)를 관측한다. 벡터 \\(\mathcbf{x}_{t, a}\\)는 사용자 \\(u_t\\)와 슬롯 손잡이 \\(a\\)의 정보를 *모두* 축약하며 이를 *맥락*이라고 칭한다.
2.이전의 시도에서 관찰 된 보수에 기초하여, A는 ∈ At에서 팔을 선택하고, 그의 기대치는 사용자 ut과 팔 모두에 의존하는 지불 rt를 받는다.
