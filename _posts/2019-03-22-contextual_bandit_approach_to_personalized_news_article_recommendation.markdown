...
알고리즘 1은 입력 매개 변수가 α 인 전체 LinUCB 알고리즘에 대한 자세한 설명을 제공합니다. 식 (1)에 주어진 α의 값에 주목하라. (4)는 일부 애플리케이션에서 보수적으로 커질 수 있으므로이 매개 변수를 최적화하면 실제적으로 전체 보수를 높일 수 있습니다. 모든 UCB 방법과 마찬가지로 LinUCB는 항상 UCB가 가장 높은 팔을 선택합니다 (식 (5)와 같습니다).
