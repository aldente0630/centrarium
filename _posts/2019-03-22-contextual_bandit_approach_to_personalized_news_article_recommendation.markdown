...
* 그런 다음 원천 사용자 변수를 유도된 공간에 투영하여 \\(\boldsymbol{\psi}_u  \overset{\underset{\mathrm{def}}{}}{=} \boldsymbol{\phi}^T_u\mathbf{W}\\)를 계산했다. 여기서 사용자 \\(u\\)에 대한 \\(\boldsymbol{\psi}_u\\)의 \\(i^{th}\\) 성분은 사용자가 기사의 \\(i^{th}\\) 범주를 좋아하는 정도로 해석할 수 있다. K-means는 유도된 \\(\boldsymbol{\psi}_u\\) 공간에서 사용자를 5개의 군집으로 묶기 위해 사용했다.  
...
## 5.3 비교 알고리즘  
  실험에서 실증적으로 평가한 알고리즘은 세 그룹으로 분류할 수 있다.
  ...
  * **전지(全知)적 알고리즘:** 이 정책은 맥락 무관한 CTR의 최고 통계치를 *사후 평가로* 계산한다. 즉, 기록된 사건으로부터 각 기사의 CTR 통계치를 먼저 계산한 다음 기록된 사건의 *동수*를 이용하여 CTR 통계치가 가장 높은 기사를 무조건 선택한 후 평가한다. 이 알고리즘은 파라미터가 필요하지 않으며 시간에 따른 "학습"을 수행하지 않는다.
...
알고리즘의 클릭률은 클릭 수와 실행 단계 수의 비율로 정의됩니다. 성능 비교를 위해 무작위로 기록 된 이벤트에 대해 모든 알고리즘의 CTR을 사용했습니다. 비즈니스에 민감한 정보를 보호하기 위해 알고리즘의 CTR을 임의 정책으로 나눈 알고리즘의 *상대적* CTR을보고합니다. 따라서 임의 정책의 상대 CTR은 정의에 따라 항상 1이므로보고하지 않습니다. 편의상 "상대 CTR"대신 "CTR"이라는 용어를 사용합니다.  
  
각 알고리즘에 대해 우리는 응용 프로그램에서 동기를 부여한 두 개의 CTR에 관심이 있으며 다른 유사한 응용 프로그램에 유용 할 수 있습니다. 방법을 Yahoo!의 첫 페이지에 배포 할 때 합리적인 방법 중 하나는이 페이지의 모든 트래픽을 두 개의 버킷으로 무작위로 나누는 것입니다 [3]. 첫 번째 "학습 버킷"은 일반적으로 기사 CTR을 학습 / 추정하기 위해 다양한 산적 알고리즘이 실행되는 소량의 트래픽으로 구성됩니다. "배치 버킷"이라고하는 다른 하나는 Yahoo! Front Page는 학습 버킷에서 얻은 CTR 추정치를 사용하여 사용자에게 탐욕스럽게 서비스를 제공합니다. 이 문제에서는 "학습"과 "배포"가 인터리브되므로 배포 버킷에 속하는 모든 관점에서 *현재* (사용자 별) CTR 추정치가 가장 높은 기사가 선택됩니다. 이 버킷은 학습 버킷이 더 많은 데이터를 가져 오면 나중에 변경 될 수 있습니다. 두 버킷의 CTR은 알고리즘 3으로 추정되었습니다.
  
배포 버킷은 종종 학습 버킷보다 크기 때문에 배포 버킷의 CTR이 더 중요합니다. 그러나 학습 버킷에서 CTR이 높을수록 적기 알고리즘에 대한 학습 속도가 빨라지거나 유감스럽게도 유감입니다. 따라서 두 버킷 모두에서 알고리즘 CTR을보고하기로 결정했습니다.
