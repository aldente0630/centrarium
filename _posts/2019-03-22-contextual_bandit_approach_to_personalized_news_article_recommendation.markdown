...
이러한 원천 변수를 사용하여 2008년 9월에 수집한 무작위 탐색 데이터를 기반으로 공동 분석을 수행했다. 선행 연구[^7]의 차원축소 접근법에 따라 사용자 변수를 기사 범주에 투영시킨 다음 유사한 선호도를 가진 사용자를 집단으로 묶었다. 더 구체적으로,
  
* 먼저 로지스틱 회귀 분석(LR)을 사용하여 원천 사용자 / 기사 변수가 주어질 경우의 클릭 확률에 대해 이중 선형 모형을 적합시켰다. 즉, \\(\boldsymbol{\phi}^T\mathbf{W}\boldsymbol{\phi}_a\\)를 사용자 \\(u\\)가 기사 \\(a\\)를 클릭할 확률로 근사시킨 것이며 이 때 \\(\mathbf{W}\\)는 LR에 의해 최적화한 가중치 행렬이다.  
  
* 그런 다음 원천 사용자 변수 \\(\psi\\)를 계산하여 유도 공간에 투영했습니다. def φ⊤ th ψ u = φu. W. 여기서, 사용자 u에 대한 ψu의 i 성분은 사용자가 기사의 i 번째. 카테고리를 좋아하는 정도로 해석 될 수있다. K-means는 유도 된 ψu 공간의 사용자들을 5 개의 클러스터로 그룹화하기 위해 적용되었다.  
