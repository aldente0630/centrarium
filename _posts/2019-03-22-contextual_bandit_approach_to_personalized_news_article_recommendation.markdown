...
각 알고리즘에 대해 우리는 응용 프로그램에서 동기를 부여한 두 개의 CTR에 관심이 있으며 다른 유사한 응용 프로그램에 유용 할 수 있습니다. 방법을 Yahoo!의 첫 페이지에 배포 할 때 합리적인 방법 중 하나는이 페이지의 모든 트래픽을 두 개의 버킷으로 무작위로 나누는 것입니다. 첫 번째 "학습 버킷"은 일반적으로 기사 CTR을 학습 / 추정하기 위해 다양한 산적 알고리즘이 실행되는 소량의 트래픽으로 구성됩니다. "배치 버킷"이라고하는 다른 하나는 Yahoo! Front Page는 학습 버킷에서 얻은 CTR 추정치를 사용하여 사용자에게 탐욕스럽게 서비스를 제공합니다. 이 문제에서는 "학습"과 "배포"가 인터리브되므로 배포 버킷에 속하는 모든 관점에서 *현재* (사용자 별) CTR 추정치가 가장 높은 기사가 선택됩니다. 이 버킷은 학습 버킷이 더 많은 데이터를 가져 오면 나중에 변경 될 수 있습니다. 두 버킷의 CTR은 알고리즘 3으로 추정되었습니다.
