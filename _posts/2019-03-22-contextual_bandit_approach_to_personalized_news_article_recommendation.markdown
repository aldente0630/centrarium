...
## 5.3 비교 알고리즘  
  ...
  이들은 모든 컨텍스트 (예 : 사용자 / 문서 정보)를 무시하는 컨텍스트가없는 K- 무장 산적 알고리즘에 해당합니다.  
  * **무작위**: 무작위 정책은 항상 동일한 확률로 풀에서 후보 기사 중 하나를 선택합니다. 이 알고리즘에는 매개 변수가 필요하지 않으며 시간이 지남에 따라 "학습"되지 않습니다.
  * **\\(\epsilon\\)-탐욕**: 섹션 2.2에 설명 된대로 각 기사의 CTR을 추정합니다. 그런 다음 확률이 ε 인 임의의 기사를 선택하고 확률이 1-ε 인 CTR 추정치가 가장 높은 기사를 선택합니다. 이 정책의 유일한 매개 변수는 ε입니다.
