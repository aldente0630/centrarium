...
이 정리는 모든 역사 (hT)가 실제 세계에서 정책 평가자와 같은 가능성을 가지고 있다고 말한다. 알고리즘 3에 의해 반환 된 평균 지불 RT / T와 같은 이러한 이력에 대한 많은 통계는 알고리즘 π의 값에 대한 편견없는 추정치입니다. 또한 정리에 따르면 KT 기록 된 이벤트는 크기 T의 표본을 보유해야합니다.
