...
맥락 무관한, \\(K\\)개의 슬롯 손잡이를 갖는 다중 슬롯머신 실험 문제는 통계학자들이 오랫동안 연구해왔다. 가장 단순하고 직관적인 알고리즘 중의 하나는 **\\(\epsilon\\)-탐욕**이다. 먼저 각 시도 \\(t\\)마다 해당 알고리즘은 모든 슬롯 손잡이 \\(a\\)의 평균 손익 \\(\mu_{t, a}\\)를 추정해 나간다. 그런 다음 \\(1 - \epsilon\\)의 확률로 *탐욕적* 슬롯 손잡이(즉, 가장 높은 손익 추정치를 갖는 슬롯 손잡이)을 선택하며 \\(\epsilon\\) 확률로 슬롯 손잡이를 무작위 선택한다. 극한을 취한다면 모든 슬롯 손잡이를 무한히 자주 시도할 것이다. 그래서 손익 추정치 \\(\mu_{t, a}\\)는 1의 확률로 참 값 \\(\mu_a\\)에 수렴한다. 더욱이 탐색 확률을 적절히 감소시키면 매 단계 당 리그릿, 즉 \\(R_{\mathsf{A}}(T) / T\\)는 1의 확률로 0에 수렴한다.
  
욕심쟁이가 채택한 유인되지 않은 탐사 전략과는 달리, 일반적으로 상위 신뢰 경계 알고리즘이라고 알려진 또 다른 클래스의 알고리즘은 탐사와 착취의 균형을 취하는 더 똑똑한 방법을 사용합니다. 구체적으로 말하자면,이 알고리즘은 각 팔 a의 평균 지불금 μt와 상응하는 신뢰 구간 ct, a를 추정하므로 | μt, a - μa | <ct, 높은 확률로 유지됩니다. 그런 다음 최상위 신뢰 경계 (UCB)를 달성하는 팔을 선택합니다. at = arg maxa (μt, a + ct, a). 적절히 정의 된 신뢰 구간을 사용하면, 그러한 알고리즘은 최적의 것으로 판명 된 총 시도 횟수 T에서 대수적 인 총 Ttrial 후회가 작은 것으로 나타낼 수 있습니다 [17].

맥락없는 K- 무장 도둑이 광범위하게 연구되고 잘 이해되는 동안,보다 일반적인 상황 적 적기 문제는 여전히 도전적이다. EXP4 알고리즘 [8]은 O- (√T) 후회를 달성하기 위해 지수 가중 기법을 사용하지만, 계산 복잡도는 피쳐의 수에 지수적일 수있다. 또 다른 일반적 맥락 적 강도 알고리즘은 수축과 함께 - 변이와 비슷한 에포크 그리 디 알고리즘 [18]이다. 이 알고리즘은 오라클 옵티 마이저를 고려할 때 계산 상 효율적이지만 O ~ (T 2/3)의 후회 보장은 약합니다.
  
적기에 대한 다양한 모델링 가정하에 더 강한 후회 보장 알고리즘이 설계 될 수있다. 팔의 기대 수익률이 선형 적이라고 가정하면, Auer [6]는 본질적으로 UCB 유형 접근법 인 LinRel 알고리즘을 설명하고 변종 중 하나에는 O ~ (√T)의 후회가 있음을 보여 주며, 이전 알고리즘 [1].
