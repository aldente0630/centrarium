...
개인화 뉴스 기사 추천 문제는 자연스럽게 맥락 상의 정보가 있는 다중 슬롯머신 문제로 모형화할 수 있다. 이전 연구에 따라 이를 *컨텍스츄얼 밴딧*이라고 부를 것이다. 수식화해보자면 컨텍스츄얼 밴딧 알고리즘 \\(\mathbb{A}\\)는 개별 시도 \\(t = 1, 2, 3, \ldot \\)에 따라 수행된다. \\(t\\)번째 시도에 대하여,
