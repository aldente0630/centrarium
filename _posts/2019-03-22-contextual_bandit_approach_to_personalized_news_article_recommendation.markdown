...
Today Module은 Yahoo! 앞단 페이지 중 가장 눈에 띄는 패널이며 가장 많이 방문하는 인터넷 페이지 중 하나이다. 그림 1의 스냅샷을 참조하라. Today Module의 "추천" 기본 탭은 양질의 기사 4개 중 하나(주로 뉴스)를 강조 표시한다. 이 4개의 기사는 편집자가 시간 단위로 선별하는 뉴스 풀에서 선택한다. 그림 1에 나와있는 것처럼 바닥 글 위치에 F1–F4로 색인화한 4개의 기사가 있다. 각 기사는 작은 그림과 제목으로 표시된다. 4개의 기사 중 하나가 스토리 위치에서 강조 표시되며 관련 링크와 함께 큰 그림, 제목 그리고 짧은 요약이 게시된다. 기본적으로 F1 기사가 스토리 위치에서 강조 표시된다. 기사에 관심이 있는 경우 스토리 위치에 강조 표시된 기사를 클릭하면 자세한 내용을 읽을 수 있다. 해당 사건은 스토리 클릭으로 기록된다. 방문자의 주목을 끌기 위해 개별 관심사에 따라 사용 가능한 기사의 순위를 매기고 스토리 위치에 각 방문자가 가장 흥미 있을 기사를 강조, 노출하고 싶다.
...
2009년 5월에 임의화 버킷에서 사건을 수집했다. 방문 보기 별로 특정 확률를 갖고 사용자마다 해당 버킷이 무작위 배정되었다.[^5] 해당 버킷은 사용자 서비스를 위해 뉴스 풀에서 기사를 임의로 선택했다. 바닥 글 위치에서의 노출 편향을 피하기 위해 스토리 위치 내 F1 기사와 사용자 간의 상호 작용에만 초점을 맞췄다. 사용자 별 상호 작용 *사건*은 (i) 사용자 서비스 제공을 위해 선택한 무작위 기사, (ii) 사용자 / 기사 정보 (iii) 스토리 위치에서 기사를 클릭하는지 여부로 구성된다. 이러한 임의의 사건을 밴딧 알고리즘 기대 손익을 신뢰성 있게 평가하는데 이용할 수 있다는 걸 4장에서 보였다.  
  
5월 1일에 임의화 버킷에 약 470만 개 사건이 있었다. 우리는 모델 검증을 위해이 날의 이벤트 ( "튜닝 데이터"라고 함)를 사용하여 각 경쟁 산적 알고리즘에 대한 최적의 파라미터를 결정했습니다. 그런 다음 5 월 3 일부터 9 일까지 랜덤 버킷에서 1 주일 이벤트 세트 ( "평가 데이터"라고 함)에 대해 조정 된 매개 변수를 사용하여 이러한 알고리즘을 실행했습니다.
