...
알고리즘 1은 입력 파라미터가 오직 \\(\alpha\\)인 **LinUCB** 알고리즘 전반에 대하여 자세하게 설명한다. 식 (1)에 주어진 \\(\alpha\\) 값에 주목하라. 식 (4)는 일부 응용 프로그램에서 보수적으로 클 수 있으므로이 매개 변수를 최적화하면 실제적으로 전체 보수를 높일 수 있습니다. 모든 UCB 방법과 마찬가지로 **LinUCB**는 항상 UCB가 가장 높은 팔을 선택합니다 (식 (5)와 같습니다).
  
이 알고리즘에는 몇 가지 좋은 속성이 있습니다. 첫째, 계산 복잡도는 팔의 수에 선형이며, 피처의 수는 최대 세제입니다. 계산을 더 줄이기 위해, 우리는 모든 단계 (O (d2) 시간 소요)에서 Aat를 업데이트 할 수 있지만 실시간 대신 주기적으로 Qa def = A-1 a (모두 a)를 계산하고 캐시합니다. 둘째, 알고리즘은 동적 암 세트에 대해 잘 작동하며 At의 크기가 너무 크지 않은 한 효율적입니다. 이 경우는 많은 애플리케이션에서 마찬가지입니다. 예를 들어, 뉴스 기사 추천에서 편집자는 기사를 풀에 추가 / 제거하고 풀 크기는 본질적으로 일정합니다. 셋째, 본 논문의 초점은 아니지만 [6]의 분석을 다음과 같이 나타낼 수있다. At 세트가 고정되어 있고 K 개의 팔을 포함하고 있다면, 신뢰 구간 (즉, 오른손 (4)의 측면)은 점점 더 많은 데이터로 충분히 빠르게 감소하고 최첨단 결과 [6]와 일치하는 O ~ (√ KdT)의 강한 후회 경계를 증명한다.
Eq. (2). 이러한 이론적 인 결과는 알고리즘의 기본적인 건전성과 효율성을 나타냅니다.
