...
알고리즘 1(또는 해당 논문[^3]과 유사한 알고리즘)은 행렬 \\(\mathbf{D}^T_a\mathbf{D}_a + \mathbf{I}_d\\)(또는 \\(\mathbf{D}^T_a\mathbf{D}_a\\))의 역행렬을 계산한다. 여기서 \\(\mathbf{D}_a\\)는 학습 데이터 변수에 해당하는 행을 갖는 설계 행렬이다. 모든 슬롯 손잡이의 행렬은 고정된 차원 \\(d \times d\\)를 가지며 업데이트를 점진적, 효율적으로 진행할 수 있다. 또한 알고리즘 1의 파라미터는 *상호 배타적*이기 때문에 역행렬을 쉽게 계산할 수 있다. 식 (3) 중 \\(\hat{\boldsymbol{\theta}}\_a\\)는 다른 슬롯 손잡이의 훈련 데이터 영향을 받지 않으므로 별도 계산할 수 있다. 이제 *혼합* 모형이라는 더 재밌는 사례를 살펴보자. 
  
우리를 비롯한 많은 응용 프로그램에서 팔 관련 기능 외에도 모든 팔이 공유하는 기능을 사용하는 것이 좋습니다. 예를 들어, 뉴스 기사 추천에서 사용자는 이것이 메커니즘을 제공하는 정치에 대한 기사 만 선호 할 수 있습니다. 따라서 공유 구성 요소와 비공유 구성 요소가 모두있는 기능을 갖는 것이 좋습니다. 공식적으로 우리는 식 (2)의 오른쪽에 다른 선형 항을 추가하여 다음의 하이브리드 모델을 채택한다. (6) 여기서 zt, a ∈ Rk는 현재 사용자 / 기사 조합의 특징이고, β *는 다음과 같다. 모든 팔에 공통된 알려지지 않은 계수 벡터. 이 모델은 계수 β * 중 일부는 모든 팔에 의해 공유되는 반면 θ * a는 그렇지 않다는 점에서 *하이브리드*입니다.
