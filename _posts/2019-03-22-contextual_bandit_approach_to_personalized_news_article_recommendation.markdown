...
맥락 무관한, \\(K\\)개의 슬롯 손잡이를 갖는 다중 슬롯머신 실험의 경우 광범위하게 연구되면서 이해가 깊어졌지만 보다 일반적인 컨텍스츄얼 밴딧 문제는 여전히 난제이다. **EXP4** 알고리즘은 \\(\tilde{O}(\sqrt{T})\\)의 리그릿을 달성하기 위해 지수 가중 기법을 이용하지만 변수 개수가 늘어남에 따라 계산 복잡도가 지수적으로 증가할 수 있다. 일반적인 컨텍스츄얼 밴딧 알고리즘 중 하나는 \\(\epsilon\\)을 축소시켜 나가는 **\\(\epsilon\\)-탐욕**과 비슷한 유형의 **에폭 탐욕** 알고리즘이다. 해당 알고리즘은 신탁적 최적기가 주어진다면 계산 상 효율적이지만 보장되는 리그릿은 \\(\tilde{O}(T^{2 \over 3})\\) 수준으로 약한 편이다.
