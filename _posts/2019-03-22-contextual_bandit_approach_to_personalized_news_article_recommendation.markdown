...
이제 실험을 위해 구성한 사용자 / 기사 변수에 대해 설명하겠다. 3장에서의 **LinUCB** 두 가지 형태를 실험하고 혼합 모형이 학습 속도를 향상시킬 수 있다는 가설을 검증하기 위해 배타 모형, 혼합 모형 각각을 위해 두 종류의 변수 집합을 사용했다.
...
\\(t\\) 시점에서 기사 \\(a\\)는 별도의 6차원 변수 \\(\mathbf{x}_{t, a}\\)을 가지며 이는 사용자 \\(u_t\\)와 정확히 동일하게, 상기한 절차로 구성한 6차원 변수이다. 이 기사 변수는 서로 겹치지 않기에 3장에서 정의한 배타 선형 모형을 위한 것이다.
...
**III. 온라인에서 사용자 별 CTR을 학습하는 알고리즘.** 
  ...
  * **\\(\epsilon\\)-탐욕(배타)**: 배타 선형 모형을 사용한 **\\(\epsilon\\)-탐욕**이며 **에폭 탐욕**의 밀접한 변종으로 볼 수 있다. 
  * **linucb(배타)**: 배타 선형 모형을 사용한 알고리즘 1이다.
...
5.3절에 나온 비교 알고리즘(**무작위**와 **전지적** 정책 제외)은 모두 조정해야할 파라미터가 한 개씩 있다. \\(\epsilon\\)-탐욕 알고리즘의 경우 \\(\epsilon\\)을, UCB 알고리즘의 경우 \\(\alpah\\). 이러한 데이터를 최적화하기 위해 튜닝 데이터를 사용했습니다. 그림 2는 각 알고리즘의 CTR이 각 매개 변수에 따라 어떻게 변하는 지 보여줍니다. 모든 결과는 단일 실행으로 얻은 것이지만 데이터 세트의 크기와 정리 1의 편견없는 결과를 고려할 때보고 된 수치는 통계적으로 신뢰할 수 있습니다.
  
먼저, 그림 2에서 볼 수 있듯이 학습 버킷의 CTR 곡선에는 종종 역 U 자형이 있습니다. 매개 변수 (ε 또는 α)가 너무 작 으면 탐색이 불충분하고 알고리즘이 좋은 기사를 식별하지 못하고 클릭 수가 적습니다. 반면에 매개 변수가 너무 크면 알고리즘이 과도하게 탐색되어 클릭 수를 늘릴 수있는 기회를 낭비했습니다. 튜닝 데이터에 대한 이러한 도표를 기반으로 각 알고리즘에 적합한 매개 변수를 선택하고 다음 하위 섹션의 평가 데이터에서 한 번 실행했습니다.
  
두 번째로, 웜 스타트 정보가 기능이없는 버전의 ε-greedy와 UCB에 비해 사용자 관심사와 기사 콘텐츠 사이의 더 나은 일치를 찾는 데 실제로 도움이된다는 플롯에서 결론을 내릴 수 있습니다. 특히 ε-greedy (따뜻한)와 ucb (따뜻한)는 전적으로 문맥없는 정책으로 달성 할 수있는 최고 CTR 인 전지 전위를 능가했습니다. 그러나 웜 스타트 정보를 사용하는 두 알고리즘의 성능은 온라인으로 가중치를 학습하는 알고리즘만큼 안정적이지 않습니다. “warm start”에 대한 오프라인 모델은 모든 랜덤 트래픽에 대해 계산 된 기사 CTR로 교육되었으므로 [12] ε-greedy (warm)는 ε이 1에 가까울 때 배포 버킷에서보다 안정적인 성능을 얻습니다. 또한 처음부터 사용자에게 더 많은 기사를 선택하여 학습 버킷에서 ucb (따뜻한)를 돕지 만 배치에 가장 적합한 온라인을 결정하는 데 ucb (웜)는 도움이되지 않았습니다. ucb는 탐색을위한 신뢰 구간에 의존하기 때문에 "warm start"에 의해 도입 된 초기화 바이어스를 수정하는 것은 어렵습니다. 대조적으로, 모든 온라인 학습 알고리즘은 전적으로 정책을 능가 할 수있었습니다. 따라서 평가 데이터에 대해 웜 스타트 알고리즘을 시도하지 않았습니다.
  
세 번째로, ε- 욕심 많은 알고리즘 (그림 2의 왼쪽)은 적절한 매개 변수가 사용될 때 배치 버킷에서 신뢰 상한값으로 (그림 2의 오른쪽) 유사한 CTR을 달성했습니다. 따라서 두 가지 유형의 알고리즘 모두 비슷한 정책을 배우는 것으로 나타났습니다. 그러나 학습 버킷에서 CTR이 더 낮은 것으로 보였으며 실제 버킷 테스트에서 컨텍스트가없는 알고리즘의 실험 결과와 일치합니다.
  
마지막으로, 데이터가 희소 한 경우 알고리즘을 비교하기 위해 30 %, 20 %, 10 %, 5 % 및 1 % 수준에서 더 적은 데이터로 각 알고리즘에 대해 동일한 매개 변수 조정 프로세스를 반복했습니다. 알고리즘 3에서 수행 한 것처럼 알고리즘의 CTR을 평가하기 위해 모든 데이터를 계속 사용했지만 알고리즘에서 정책을 개선하기 위해 사용 가능한 일부 데이터 만 임의로 선택했습니다.
  
### 5.5.2 검증 데이터를 위한 결과
  
튜닝 데이터 (c.f., 그림 2)에 최적화 된 파라미터를 사용하여 평가 데이터에 대해 알고리즘을 실행하고 표 1의 CTR을 요약했습니다. 전지의 CTR은 1.615이므로 알고리즘의 CTR이 훨씬 더 커서 개인화에 사용자 / 문서 기능을 효과적으로 사용함을 나타냅니다. 보고 된 CTR은 임의 정책의 CTR에 의해 정규화되었음을 기억하십시오. 다음 하위 섹션에서 결과를 자세히 살펴 봅니다.
  
### 변수 사용에 대하여

  먼저 기사 추천에서 기능을 사용하는 것이 도움이되는지 조사합니다. 사용자 기능을 고려하여 ε-greedy (seg / disjoint / hybrid) 및 UCB 방법 (ucb (seg) 및 linucb (disjoint / hybrid))이 모두 약 10의 CTR 상승을 달성 할 수 있음이 표 1에서 분명합니다. 기준 ε- 욕심과 비교 한 %.
기능의 효과를 더 잘 시각화하기 위해 그림 3은 기사의 CTR (알고리즘에 의해 선택된 경우)이 기본 CTR (컨텍스트가없는 CTR)[^8]과 비교하여 어떻게 향상되었는지를 보여줍니다 .4 여기서 기사의 기본 CTR은 얼마나 흥미로운 지 측정합니다. 임의의 사용자를 대상으로하며 기록 된 이벤트에서 추정되었습니다. 따라서 기사의 해제 및 기본 CTR 비율이 높으면 알고리즘이 관심있는 사용자에게이 기사를 추천한다는 강력한 지표가됩니다. 그림 3 (a)는 ε-greedy 나 ucb가 사용자 정보를 사용하지 않았기 때문에 기사 CTR을 들어 올릴 수 없었 음을 보여줍니다. 대조적으로, 다른 세 가지 도표는 모두 개인화 된 권장 사항을 고려함으로써 분명한 이점을 보여줍니다. 극단적 인 경우 (그림 3 (c)), 기사의 CTR 중 하나가 1.31에서 3.03으로 132 % 개선되었습니다.
