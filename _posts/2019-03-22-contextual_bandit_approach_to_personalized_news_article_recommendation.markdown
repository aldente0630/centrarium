...
### ucb(세분화)와 linucb(분리)를 비교할 때
  ...
  그림 5는 가장 가까운 군집, 즉 사용자의 5 가지 비정형 기능 중 가장 큰 구성 요소에 대한 사용자의 상대적 회원 측정에 대한 막대 그래프를 보여줍니다. 대부분의 사용자는 5 개의 클러스터 센터 중 하나에 매우 근접한 것으로 나타났습니다. 최대 약 85 %의 사용자는 0.5보다 높았으며, 그 중 약 40 %는 0.8보다 높았습니다. 따라서 이러한 기능 중 상당수는 지배적 인 구성 요소를 갖기 때문에 기능 벡터가 "하드"버전의 사용자 그룹 할당과 유사합니다. 
