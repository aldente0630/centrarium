...
본 연구에서 손익 구간이 선형 일 때 닫힌 형태로 신뢰 구간을 효율적으로 계산할 수 있고이 알고리즘을 LinUCB라고 부른다. 설명의 편의를 위해, 우리는 먼저 분리 된 선형 모델에 대한 더 간단한 형태를 기술하고, 3.2 절에서 하이브리드 모델의 일반적인 경우를 고려한다. LinUCB는 개인화 된 뉴스 기사 권장 사항 이외의 애플리케이션에 적용되는 일반적인 문맥  적 산적 알고리즘입니다.
