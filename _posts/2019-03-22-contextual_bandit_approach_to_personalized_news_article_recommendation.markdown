...
3.알고리즘은 새로운 관측치 \\(\(\mathbf{x}\_{t, a}, a_t, r\_{t, a_t}\)\\)를 이용하여 슬롯 손잡이 선택 전략을 개선한다. 여기에서 강조하지 않는 것은 중요하지 않은 것은 피드백 (즉, 보상 rt, a)이 선택되지 않은 무기 a = at에서 관찰된다는 것입니다. 이 사실의 결과는 다음 소단원에서 더 자세히 논의됩니다.
