...
일반적인 컨텍스츄얼 밴딧 문제에서 특별히 중요한 경우는 (ⅰ) 슬롯 손잡이 집합 \\(\mathcal{A}_t\\)가 변하지 않으며 모든 \\(t\\)마다 슬롯 손잡이가 \\(K\\)개인 (ⅱ) 사용자 \\(u_t\\)(또는 이와 동치로 맥락 \\(\(\mathbf{x}\_{t, 1}, \cdots ,\mathbf{x}\_{t, K}\)\\) )가 모든 \\(t\\)마다 동일한 문제이다. 즉, 잘 알려진 *\\(K\\)개의 슬롯 손잡이를 갖는 다중 슬롯머신 실험 문제*이다. 슬롯 손잡이 집합과 맥락이 모든 시도마다 동일하므로 다중 슬롯머신 실험 알고리즘과 아무런 차이도 없으며 해당 유형을 *맥락 무관한* 다중 슬롯머신 실험이라고 부른다.
  
기사 추천의 관점에서 컨텐츠 풀에 속하는 기사는 하나의 슬롯 손잡이로 볼 수 있다. 제공한 기사를 클릭하면 손익이 1이고 그렇지 않으면 0이다.이 지불금 정의에 따라 기사의 예상 수익은 정확히 클릭률 (CTR)이며, 최대 클릭률 (CTR)이있는 기사를 선택하는 것은 사용자의 예상 클릭 수를 최대화하는 것과 같습니다. 우리의 적기 공식에서 총 기대 보수를 극대화하는 것과 같다.
  
또한 웹 서비스에서 우리는 종종 사용자의 관심을 유추하고 아마 그녀에게 가장 흥미로운 뉴스 기사를 선택하는 데 사용할 수있는 사용자 정보에 액세스 할 수 있습니다. 예를 들어, 남성 십대가 은퇴 계획보다는 iPod 제품에 대한 기사에 관심을 가질 가능성이 훨씬 큽니다. 따라서 우리는 사용자와 기사를 압축하여 설명하는 유익한 기능으로 요약 할 수 있습니다. 이렇게하면 적기 알고리즘은 한 기사 / 사용자의 CTR 정보를 일반화 할 수 있고 특히 새로운 사용자와 기사의 경우 우수한 기사를 더 빨리 선택할 수 있습니다.
