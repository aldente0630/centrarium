...
목표는 해당 데이터를 사용하여 밴딧 알고리즘 \\(\pi\\)를 평가하는 것이다. 공식적으로 \\(\pi\\)는 현 시점의 맥락 벡터\\(\mathbf{x}_1, \ldots, \mathbf{x}_K\\)와 함께 \\(t-1\\)개의 이전 시점 이력 \\(h_{t-1}\\)을 기반으로 현 시점 \\(t\\)에서 슬롯 손잡이를 선택하는 (무작위성이 가미된) 연결 논리이다.
  
우리의 제안 된 정책 평가자는 알고리즘 3에 제시되어있다.이 방법은 정책 π와 평가의 기초가되는 원하는 수의 "좋은"이벤트 T를 입력으로서 취한다. 그런 다음 기록 된 이벤트 스트림을 하나씩 살펴 봅니다. 현재 히스토리 ht-1이 주어지면, 정책 π가 로깅 정책에 의해 선택된 것과 동일한 암 a를 선택하면, 이벤트가 유지되고, 즉, 이력에 추가되고, 업데이트 된 총 지불 Rt. 그렇지 않으면, 정책 π가 로깅 정책에 의해 취해진 것과 다른 팔을 선택하면, 이벤트는 완전히 무시되고 알고리즘은 상태의 다른 변경없이 다음 이벤트로 진행합니다.
  
로깅 정책은 각 암을 무작위로 균일하게 선택하기 때문에 각 이벤트는 다른 알고리즘과 상관없이 정확히 1 / K의 확률로이 알고리즘에 의해 유지됩니다. 이는 보유 된 이벤트가 D에 의해 선택된 것과 동일한 분포를 가짐을 의미합니다. 결과적으로 두 프로세스가 동일하다는 것을 증명할 수 있습니다. 첫 번째는 D의 T 실제 이벤트에 대한 정책을 평가하고, 두 번째는 기록 된 이벤트 스트림에서 정책 평가기를 사용하여 정책을 평가하는 것입니다.
