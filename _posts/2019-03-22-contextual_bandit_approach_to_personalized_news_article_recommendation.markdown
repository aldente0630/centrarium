...
*여기서 \\(S\\)는 무작위로 균일하게 선택하는 로깅 정책과 \\(D\\\)로부터 i.i.d.로 추출한 사건들의 스트림이다. 덧붙여 길이 \\(T\\)의 이력 \\(h_T\\)을 모으기 위해 스트림에서 추출할 사건의 기대 건수는 \\(KT\\)이다.*
  
정책 평가자 내의 *모든* 이력 \\(h_T\\)가 현실 세계에서 발생할 확률과 동일하다고 이 정리는 이야기한다. 알고리즘 3이 반환하는 평균 손익 \\(R_T / T\\)처럼 이력에 관한 다수의 통계량은 알고리즘 \\(\pi\\) 값에 대한 편향되지 않은 추정치이다. 또한 정리에 따르면 길이 \\(T\\)의 표본을 보유하기 위해 \\(KT\\)건의 기록된 사건이 평균적으로 필요하다.
...
로깅 정책은 슬롯 손잡이 \\(a\\)를 무작위로 균일하게 선택하므로 정책 평가자가 내부 루프를 종료 할 확률은 모든 정책, 모든 기록, 기능 및 모든 암에 대해 동일하므로 확률이있는 마지막 이벤트에서 발생합니다. 마지막 이벤트의 \\(\Pr_{D} (\mathbf{x}\_{t, 1}, \ldots, \mathbf{x}\_{t, K}, r_{t, a})\\). 마찬가지로 정책 \\(\pi\\)의 슬롯 손잡이에 대한 분포는 이력 \\(h_{t-1}\\)과  \\((\mathbf{x}\_{t, 1}, \ldots, \mathbf{x}\_{t, K})\\)에 따라 독립적이며 팔 \\(a\\)의 확률은 단지 \\(\Pr_{\pi(h_{t-1})}(a \mid \mathbf{x}\_{t, 1}, \ldots, \mathbf{x}\_{t, K})\\).
  
마지막으로 스트림의 각 사건은 정확히 \\(1 / K\\)의 확률로 보존되므로 \\(T\\)개의 사건을 보유하는데 필요한 기대 건수는 정확히 \\(KT\\)개이다.
