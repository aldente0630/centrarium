...
알고리즘 1 (또는 [22]의 유사한 알고리즘)은 행렬의 역행렬 Da Da + Id (또는 Da Da)를 계산합니다. 여기서 Da는 학습 데이터의 피처에 해당하는 행을 갖는 디자인 행렬입니다. 모든 팔의이 행렬은 고정 된 차원 d × d를 가지며 효율적으로 점차적으로 업데이트 될 수 있습니다. 또한 알고리즘 1의 파라미터가 서로 다르기 때문에 이들의 역수를 쉽게 계산할 수 있습니다. (3) 다른 팔의 훈련 데이터의 영향을받지 않으므로 별도로 계산할 수 있습니다. 지금 우리는 *하이브리드* 모델에 대해 더 흥미로운 사례를 고려합니다. 
