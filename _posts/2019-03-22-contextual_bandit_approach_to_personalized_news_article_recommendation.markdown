...
각 기사 \\(a\\)마다 6차원의 기사 변수(상수 변수 1 포함)를 얻기위해 동일한 차원 축소를 수행했다. 사용자 변수를 적용한 외적은 식 (6)의 공유 변수에 해당하며 \\(\mathbf{z}_{t, a} \in \mathbb{R}^36\\)로 표기하는 \\(6 \dot 6 = 36\\)개의 변수를 제공한다. 따라서 \\((\mathbf{z}_{t, a}, \mathbf{x}_{t, a})\\)는 혼합 선형 모형에서 사용할 수 있다. \\(\mathbf{z}_{t, a}\\) 변수는 사용자-기사의 상호 작용 정보를 포함하고 \\(\mathbf{x}_{t, a}\\)는 사용자 정보만 포함한다.
