...
로깅 정책은 각 슬롯 손잡이를 무작위로 균일하게 선택하기 때문에 정책 평가자를 거치는 동안 각 사건은 정확히 \\(1 / K\\)의 발생 확률을 독립적으로 유지한다. 이는 보존한 사건이 \\(D\\)에 의해 선택된 것과 동일한 분포를 가짐을 뜻한다. 결과적으로 두 프로세스가 동일하다는 것을 증명할 수 있다. 여기서 두 프로세스란 전자는 \\(D\\)의 \\(T\\) 건의 실제 사건에 대해 정책을 평가함을, 후자는 기록된 사건 스트림에서 정책 평가자를 적용하여 정책을 평가함을 뜻한다.
...
이제 모든 이력 \\(h_t\\)에 대해 동일 내용을 증명하고 싶다. 데이터는 i.i.d.이고 정책의 무작위성은 현실 세계의 무작위 선택과 독립이므로 이력 \\(h_{t-1}\\)이 주어졌을 때 \\(t\\) 번째 사건에 대한 분포가 프로세스 양측에서 동일하다는 걸 증명하면된다. 즉, 다음을 보여야한다.
...
이 절은 4절의 오프라인 평가 방법을 사용하여 제안된 **LinUCB** 알고리즘의 역량을 실제 애플리케이션을 통해 확인한다. 먼저 Yahoo! Today Module의 문제 설정을 소개한다음 실험에 사용한 사용자 / 품목 변수를 설명하겠다. 마지막으로 성능 지표를 정의하고 몇 가지 표준 (컨텍스츄얼) 밴딧 알고리즘과 비교해가며 실험 결과를 제시하겠다.

## 5.1 Yahoo! Today Module

Today Module은 Yahoo!에서 가장 눈에 띄는 패널입니다. 인터넷에서 가장 많이 방문한 페이지 중 하나 인 프론트 페이지; 그림 1의 스냅 샷을 참조하십시오. 오늘 모듈의 기본 "추천"탭은 4 가지 고품질 기사 중 하나 (주로 뉴스)를 강조 표시하고 4 개의 기사는 사람이 편집 한 시간 단위로 새로 고쳐진 기사 풀에서 선택합니다. 그림 1에 나와있는 것처럼 바닥 글 위치에는 F1–F4로 색인화 된 4 개의 기사가 있습니다. 각 기사는 작은 그림과 제목으로 표시됩니다. 4 가지 기사 중 하나가 스토리 위치에서 강조 표시되며, 관련 링크와 함께 큰 그림, 제목 및 짧은 요약으로 표시됩니다. 기본적으로 F1의 기사는 스토리 위치에서 강조 표시됩니다. 기사 위치에서 강조 표시된 기사를 클릭하면 기사에 관심이있는 경우 자세한 내용을 읽을 수 있습니다. 이벤트는 스토리 클릭으로 기록됩니다. 방문자의 관심을 끌기 위해 개별 관심사에 따라 사용 가능한 기사의 순위를 매기고 스토리 위치에서 각 방문자에게 가장 매력적인 기사를 강조하고 싶습니다.

![그림1](https://aldente0630.github.io/assets/contextual-bandit_approach_to_personalized_news _article_ recommendation1.png) 
**그림 1: Yahoo!의 Today Module에서“추천”탭의 스냅 샷 첫 페이지. 기본적으로 F1 위치의 기사는 스토리 위치에서 강조 표시됩니다.
  
## 5.2 실험 설정
  
이 하위 섹션에서는 데이터 수집, 기능 구성, 성능 평가 및 경쟁 알고리즘을 포함하여 실험 설정에 대한 자세한 설명을 제공합니다.
  
### 5.2.1 데이터 수집  

2009 년 5 월에 임의 버킷에서 이벤트를 수집했습니다. 사용자는 방문보기 당 특정 확률로 버킷을 임의로 선택했습니다 .3이 버킷에서 사용자를 지원하기 위해 기사 풀에서 기사를 임의로 선택했습니다. 바닥 글 위치에서 노출 편향을 피하기 위해 스토리 위치에서 F1 기사와의 사용자 상호 작용에만 초점을 맞췄습니다. 각 사용자 상호 작용 이벤트는 (i) 사용자에게 서비스를 제공하기 위해 선택한 임의의 기사, (ii) 사용자 / 기사 정보 및 (iii) 스토리 위치에서 기사를 클릭하는지 여부로 구성됩니다. 섹션 4에서는 이러한 임의의 이벤트를 사용하여 적기 알고리즘의 예상 보수를 안정적으로 평가할 수 있습니다.  
  
5 월 1 일에 랜덤 버킷에 약 470 만 개의 이벤트가있었습니다. 우리는 모델 검증을 위해이 날의 이벤트 ( "튜닝 데이터"라고 함)를 사용하여 각 경쟁 산적 알고리즘에 대한 최적의 파라미터를 결정했습니다. 그런 다음 5 월 3 일부터 9 일까지 랜덤 버킷에서 1 주일 이벤트 세트 ( "평가 데이터"라고 함)에 대해 조정 된 매개 변수를 사용하여 이러한 알고리즘을 실행했습니다.

### 5.2.2 변수 생성  
  
이제 실험을 위해 구성된 사용자 / 기사 기능에 대해 설명합니다. 섹션 3에서 두 가지 형태의 LinUCB를 테스트하고 하이브리드 모델이 학습 속도를 향상시킬 수 있다는 우리의 추측을 검증하기 위해 각각 분리 된 모델과 하이브리드 모델을위한 두 가지 기능이 사용되었습니다.
  
"지원"에 의해 선택된 원시 사용자 기능으로 시작합니다. 기능 지원은 해당 기능을 가진 사용자의 일부입니다. 데이터의 노이즈를 줄이기 위해 높은 지원 기능 만 선택했습니다. 특히 지원이 0.1 이상인 경우 기능을 사용했습니다. 그런 다음 각 사용자는 원래 1000 개가 넘는 범주 구성 요소의 원시 특징 벡터로 표시되었습니다. 여기에는 다음이 포함됩니다. (ii) 지리적 특징 : 전세계 및 미국 주 약 200 곳의 대도시; 그리고 (iii) 행동 범주 : Yahoo! 속성. 이러한 기능 외에 사용자를 식별하는 데 다른 정보는 사용되지 않았습니다.
