...
일반적인 컨텍스츄얼 밴딧 문제에서 특별히 중요한 경우는 (ⅰ) 슬롯 손잡이 집합 \\(\mathcal{A}_t\\)가 변하지 않으며 모든 \\(t\\)마다 슬롯 손잡이가 \\(K\\)개이고 (ⅱ) 사용자 \\(u_t\\)(또는 이와 동치로 맥락 \\(\(\mathbf{x}\_{t, 1}, \cdots ,\mathbf{x}\_{t, K}\)\\))가 모든 \\(t\\)마다 동일한 문제이다. 즉 잘 알려진, *\\(K\\)개의 슬롯 손잡이를 갖는 다중 슬롯머신 실험*이다. 다중 슬롯머신 실험 알고리즘에게 슬롯 손잡이 집합과 맥락은 모든 시도에서 차이점이 없다. 해당 유형을 *맥락 무관한* 다중 슬롯머신 실험이라고 부른다.
