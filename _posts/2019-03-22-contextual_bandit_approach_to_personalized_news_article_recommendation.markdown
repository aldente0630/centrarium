...
상술한 과정에서 \\(\mathsf{A}\\)의 *총 \\(T\\)번의 시도에 따른 손익*은 \\(\sum_\{t = 1}^T r\_{t, a}\\)로 정의된다. 이와 비슷하게 *총 \\(T\\)번의 시도에 따른 최적 손익 기대값*을 \\(\mathbf{E}\[\sum_\{t = 1}^T r\_{t, a}\]\\)으로 정의한다.
t = 1 rt, a * ti, 여기서 a * t는 시험 t에서 최대 기대 보수가있는 팔이다. 우리의 목표는 A를 설계하여 위의 예상 총 보수가 최대가되도록하는 것입니다. 동등하게, 최적의 암 - 선택 전략에 대한 후회가 최소화되도록 알고리즘을 찾을 수 있습니다. 여기서 알고리즘 A의 T-trial 후회 RA (T)는 다음과 같이 공식적으로 정의된다.
