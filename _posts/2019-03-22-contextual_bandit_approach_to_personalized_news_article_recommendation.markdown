...
3: **for** \\(t = 1, 2, 3, \ldots, T\\) **do**  
4: \\(\qquad\\) **repat**  
5: \\(\qquad \qquad\\) 다음 사건 \\((\mathbf{x}_1, \ldots, \mathbf{x}_K, a, r_{t, a})\\)을 취함  
6: \\(\qquad\\) **until** \\(\pi(h_{t_1}, (\mathbf{x}_1, \ldots, \mathbf{x}_K)) = a\\)  
7: \\(\qquad h_t \leftarrow CONCATENATE(h_{t-1}, (\mathbf{x}_1, \ldots, \mathbf{x}_K, a, r_{t, a}))\\)  
8: \\(\qquad R_t \leftarrow R_{t-1} + r_a\\)   
9: **end for**  
10: 출력: \\(R_T / T\\)  
  
로깅 정책은 각 슬롯 손잡이를 무작위로 균일하게 선택하기 때문에 정책 평가자를 거치는 동안 각 사건은 정확히 \\(1 / K\\)의 발생 확률을 독립적으로 유지한다. 이는 보존한 사건이 \\(D\\)에 의해 선택된 것과 동일한 분포를 가짐을 뜻한다. 결과적으로 두 프로세스가 동일하다는 것을 증명할 수 있다. 여기서 두 프로세스란 전자는 \\(D\\)의 \\(T\\) 개의 실제 사건에 대해 정책을 평가함을, 후자는 기록된 사건 스트림에서 정책 평가자를 적용하여 정책을 평가함을 뜻한다.
...
정책 평가자 내의 *모든* 이력 \\(h_T\\)가 현실 세계에서 발생할 확률과 동일하다고 이 정리는 이야기한다. 알고리즘 3이 반환하는 평균 손익 \\(R_T / T\\)처럼 이력에 관한 다수의 통계량은 알고리즘 \\(\pi\\) 값에 대한 편향되지 않은 추정치이다. 또한 정리에 따르면 길이 \\(T\\)의 표본을 보유하기 위해 \\(KT\\)개의 기록된 사건이 평균적으로 필요하다.
...
로깅 정책은 슬롯 손잡이 \\(a\\)를 무작위로 균일하게 선택하므로 정책 평가자가 내부 루프를 종료 할 확률은 모든 정책, 모든 기록, 기능 및 모든 암에 대해 동일하므로 확률이있는 마지막 이벤트에서 발생합니다. 마지막 이벤트의 \\(\Pr_{D} (\mathbf{x}\_{t, 1}, \ldots, \mathbf{x}\_{t, K}, r_{t, a})\\). 마찬가지로 정책 \\(\pi\\)가
팔에 대한 분포는 이력 \\(h_{t-1}\\)과 특징 \\((\mathbf{x}\_{t, 1}, \ldots, \mathbf{x}\_{t, K})\\)에 따라 독립적이며 팔 \\(a\\)의 확률은 단지 \\(\Pr_{\pi(h_{t-1})}(a \mid \mathbf{x}\_{t, 1}, \ldots, \mathbf{x}\_{t, K})\\).
  
마지막으로, 스트림의 각 이벤트는 정확히 \\(1 / K\\)의 확률로 유지되므로 \\(T\\) 이벤트를 유지하는 데 필요한 예상 수는 정확히 \\(KT\\)입니다.
