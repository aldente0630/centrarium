...
세 번째로, ε- 욕심 많은 알고리즘 (그림 2의 왼쪽)은 적절한 매개 변수가 사용될 때 배치 버킷에서 신뢰 상한값으로 (그림 2의 오른쪽) 유사한 CTR을 달성했습니다. 따라서 두 가지 유형의 알고리즘 모두 비슷한 정책을 배우는 것으로 나타났습니다. 그러나 학습 버킷에서 CTR이 더 낮은 것으로 보였으며 실제 버킷 테스트에서 컨텍스트가없는 알고리즘의 실험 결과와 일치합니다 [2].
