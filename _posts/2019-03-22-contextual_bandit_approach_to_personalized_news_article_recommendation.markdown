...
논문[^6]에 나온 전처리 절차에 따라 범주형 사용자 / 기사 변수를 이진 벡터로 인코딩한 다음 각 변수 벡터를 단위 벡터 크기로 정규화했다. 또한 각 변수 벡터에 대해 상수 변수 값 1을 덧붙였다. 결과적으로 기사와 사용자를 각각 83과 1,193개의 변수 벡터로 표현했다.
  
이러한 원천 변수를 사용하여 2008년 9월에 수집한 무작위 탐색 데이터를 기반으로 공동 분석을 수행했다. 논문[^7]에 나온 차원축소 접근법에 따라 사용자 변수를 기사 범주에 투영시킨 다음 유사한 선호도를 가진 사용자를 집단으로 묶었다. 더 구체적으로,
  
* 먼저 로지스틱 회귀 분석 (LR)을 사용하여 원시 사용자 / 기사 기능이 지정된 클릭 확률에 대한 이중 선형 모형을 적합 시켰으므로 φ⊤ Wφa는 사용자 u가 기사 a를 클릭 할 확률과 거의 일치했습니다. W는 LR에 의해 최적화 된 가중치 매트릭스였다.  
  
* 그런 다음 원시 사용자 기능을 ψ를 계산하여 유도 공간에 투영했습니다. def φ⊤ th ψ u = φu. W. 여기서, 사용자 u에 대한 ψu의 i 성분은 사용자가 기사의 i 번째. 카테고리를 좋아하는 정도로 해석 될 수있다. K-means는 유도 된 ψu 공간의 사용자들을 5 개의 클러스터로 그룹화하기 위해 적용되었다.  
  
* 최종 사용자 기능은 6 개의 벡터입니다. 5 개의 항목은이 5 개의 클러스터 (가우시안 커널로 계산 된 다음 정규화되도록 정규화 됨)에서 해당 사용자의 구성원 자격에 해당했으며 6 번째는 상수 기능 1입니다.  
  
시험 (t)에서, 각각의 물품 (a)은 별도의 6 차원 특징 (xt)을 가지며, 이는 사용자 (ut)를 위해 상기와 같이 정확하게 구성된 6 차원 특징이다. 이 기사의 특징은 겹치지 않기 때문에 3 절에 정의 된 분리 된 선형 모델을위한 것입니다.
  
각 기사 a에 대해 6 차원 기사 피쳐 (상수 1 피쳐 포함)를 얻기 위해 동일한 치수 감소를 수행했습니다. 사용자 기능이있는 외부 제품은 식 6의 공유 기능에 해당하는 zt, ∈ R36으로 표시된 6 × 6 = 36 개의 기능을 제공합니다. (6), 따라서 (zt, a, xt, a)는 하이브리드 선형 모델에서 사용될 수있다. zt, a 기능은 사용자-문서 상호 작용 정보를 포함하고 xt, a는 사용자 정보 만 포함합니다.
  
여기서는 세분화 분석에서 대표적인 것으로 나타난 5 개의 사용자 (및 기사) 그룹을 의도적으로 사용했습니다. [13]. 상대적으로 작은 피처 공간을 사용하는 또 다른 이유는 온라인 서비스에서 대량의 사용자 / 문서 정보를 저장하고 검색하는 데 너무 비싸서 실용적이지 않기 때문입니다.
