...
알고리즘의 클릭률은 클릭 건수를 노출 단계가 실행된 건수의 비율로 정의한다. 무작위로 기록한 사건에 대해 모든 알고리즘의 CTR을 성능 비교 기준으로 삼았다. 비즈니스에 민감한 정보를 보호하기 위해 알고리즘 CTR을 무작위 정책의 것으로 나눠서 알고리즘의 *상대적* CTR만 제시하겠다. 따라서 무작위 정책의 상대적 CTR은 정의에 따라 항상 1이므로 제시하지 않는다. 편의상 "상대 CTR" 대신 "CTR"이라는 용어를 사용한다.  
