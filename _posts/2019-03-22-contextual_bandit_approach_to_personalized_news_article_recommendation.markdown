...
본 논문에서 제안하는 정책 평가자는 알고리즘 3에 나와있다. 이 방법론은 정책 \\(\pi\\)와 평가의 기초인, "양질의" 사건에 대한 원하는 개수 \\(T\\)를 입력으로서 취한다. 그런 다음 기록한 이벤트 스트림을 하나씩 살펴 봅니다. 현재 히스토리 ht-1이 주어지면, 정책 π가 로깅 정책에 의해 선택된 것과 동일한 암 a를 선택하면, 이벤트가 유지되고, 즉, 이력에 추가되고, 업데이트 된 총 지불 Rt. 그렇지 않으면, 정책 π가 로깅 정책에 의해 취해진 것과 다른 팔을 선택하면, 이벤트는 완전히 무시되고 알고리즘은 상태의 다른 변경없이 다음 이벤트로 진행합니다.
