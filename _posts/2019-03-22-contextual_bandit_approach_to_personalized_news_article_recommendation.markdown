...
일반적인 문맥 적 강도 문제의 중요한 특별한 경우는 (i) At 세트가 변경되지 않고 모든 T에 대해 K 암을 포함하는 잘 알려진 K- 무장 적기와 (ii) 사용자 ut (또는 이와 동등하게, context (xt, 1, ..., xt, K))는 모든 t에 대해 동일하다. 팔 세트와 컨텍스트는 매 시도마다 일정하기 때문에 적기 알고리즘과 아무런 차이가 없으므로이 유형의 적기를 상황없는 적기라고 부른다.
