...
파라미터를 다른 슬롯 손잡이와 공유하지 않기 때문에 본 모형은 *배타적*이다. \\(\mathbf{D}_a\\)를 \\(t\\)번째 시도에서의 \\(m \times d\\) 차원인 설계 행렬이라고 정의하자. 행은 \\(m\\) 개의 훈련 입력값 (예: 기사 \\(a\\)에서 이전에 관찰한 문맥)에 해당하고 ba ∈ Rm이 해당 응답 벡터 (예 : 사용자 피드백을 클릭 / 클릭하지 마십시오). 훈련 데이터 (Da, ca)에 융기 회귀 분석을 적용하면 계수의 추정값을 얻을 수 있습니다.
