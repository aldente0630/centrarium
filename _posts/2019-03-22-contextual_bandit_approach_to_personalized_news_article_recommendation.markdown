...
섹션 5.3의 각 경쟁 알고리즘 (임의 및 전능 제외)에는 단일 매개 변수가 필요합니다. ε- 욕심 알고리즘의 경우 ε, UCB 알고리즘의 경우 α. 이러한 데이터를 최적화하기 위해 튜닝 데이터를 사용했습니다. 그림 2는 각 알고리즘의 CTR이 각 매개 변수에 따라 어떻게 변하는 지 보여줍니다. 모든 결과는 단일 실행으로 얻은 것이지만 데이터 세트의 크기와 정리 1의 편견없는 결과를 고려할 때보고 된 수치는 통계적으로 신뢰할 수 있습니다.
