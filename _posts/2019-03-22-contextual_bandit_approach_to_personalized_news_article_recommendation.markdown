...
이 알고리즘은 몇 가지 좋은 성질이 있다. 첫째, 계산 복잡도가 슬롯 손잡이 개수에 선형이며 변수 개수에 최대 세제곱이다. 계산량을 더 줄이기 위해 모든 단계(\\(O(d^2)\\) 시간이 걸리는)에서 \\(\mathbf{A}\_{a_t}\\)를 실시간으로 업데이트하는 대신 주기적으로 \\(\mathbf{Q}_a \overset{\underset{\mathrm{def}}{}}{=} \mathbf{A}^{-1}\_{a_t}\\\) (모든 \\(a\\)에 대하여)를 계산하고 캐시할 수 있다. 둘째, 알고리즘은 동적인 슬롯 손잡이 집합에 대해 잘 작동하며 \\(\mathcal{A}\_{a_t}\\)의 크기가 너무 크지 않은 한 효율적이다. 이는 많은 응용 프로그램에 통용되는 경우이다. 예를 들어, 뉴스 기사 추천에서 편집자는 기사를 풀에 추가 / 제거하고 풀 크기는 본질적으로 일정합니다. 셋째, 본 논문의 초점은 아니지만 [6]의 분석을 다음과 같이 나타낼 수있다. At 세트가 고정되어 있고 K 개의 팔을 포함하고 있다면, 신뢰 구간 (즉, 오른손 (4)의 측면)은 점점 더 많은 데이터로 충분히 빠르게 감소하고 최첨단 결과 [6]와 일치하는 O ~ (√ KdT)의 강한 후회 경계를 증명한다.
