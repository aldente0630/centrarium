...
로깅 정책은 각 슬롯 손잡이를 무작위로 균일하게 선택하기 때문에 본 평가 알고리즘을 거치는 동안 각 사건은 정확히 \\(1 / K\\)의 발생 확률을 독립적으로 유지한다. 이는 보존한 사건이 \\(D\\)에 의해 선택한 것과 동일한 분포를 가짐을 뜻한다. 결과적으로 두 프로세스가 동일하다는 것을 증명할 수 있습니다. 첫 번째는 D의 T 실제 이벤트에 대한 정책을 평가하고, 두 번째는 기록 된 이벤트 스트림에서 정책 평가기를 사용하여 정책을 평가하는 것입니다.
