...
지도 학습 상황에서 진행하는 보다 표준화된 기계 학습과 비교하자면 컨텍스츄얼 밴딧의 평가 방법론은 꽤나 까다롭다. *밴딧 알고리즘* \\(\pi\\), 즉 앞서 발생한 상호 작용을 기반으로 각 시점마다 슬롯 손잡이를 선택하는 규칙(예컨대 위에서 설명한 알고리즘)의 성능을 측정하는 것이 목표이다. 그러나 상호작용적인 특성 때문에 "라이브" 데이터 위에서 알고리즘을 실제로 수행해보는 것 외에 목표를 달성할 방법이 없어보인다. 그러나 서비스에 들어갈 비용을 생각하면 이런 접근법은 사실상 실행 불가능하다. 지금 알고리즘과 완전히 *다른* 예전의 로깅 정책을 사용해 수집한 *오프라인* 데이터만 이용 가능할 수 있다. 로깅 정책에 의해 선택된 슬롯 손잡이에서만 결과 값이 관찰되고 이는 평가 대상 알고리즘 \\(\pi\\)에 의해 선택될 슬롯손잡이의 값과 매우 *다를* 것이기에 로깅 데이터에만 의존하여 \\(\pi\\)를 어떻게 평가할 수 있을지 명확하지 않다. 이 평가 문제는 강화 학습에서의 "오프-폴리시 평가"의 특별한 경우로 볼 수 있다(해당 논문[^4]를 참조하라).
  
한 가지 해결책은 기록된 데이터로부터 적기 프로세스를 모델링하기 위해 시뮬레이터를 구축 한 다음 시뮬레이터로 π를 평가하는 것입니다. 그러나 모델링 단계는 시뮬레이터에 바이어스를 도입 할 것이므로이 시뮬레이터 기반 평가 방법의 신뢰성을 정당화하는 것을 어렵게 만듭니다. 대조적으로, 우리는 구현하기 쉽고, 기록 된 데이터에 근거하고, 편견없는 접근법을 제안합니다.
