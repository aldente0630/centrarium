...
한 가지 해결책은 기록된 데이터로부터 밴딧의 절차를 모형화하기 위해 시뮬레이터를 구축한 다음 시뮬레이터로 \\(\pi\\)를 평가하는 것이다. 그러나 모형 작업은 시뮬레이터에 *편향*을 가져오므로 시뮬레이터 기반 평가 방식의 신뢰성을 증명하기 어렵다. 그보다 구현하기 쉽고 기록된 데이터에 근거하고 *편향 없는* 방법론을 제안한다.
 ...
 **II. "웜 스타트"를 적용한 알고리즘.** - 개인화 서비스를 향한 중간 단계. 아이디어는 전체 트래픽으로 구한, 기사의 맥락 무관한 CTR에 대해 오프라인에서 예측한 사용자 별 조정값을 제공하는 것이다. 오프셋은 신규 콘텐츠에 대한 CTR 추정치의 초기값, 말하자면 "웜 스타트" 역할을 한다. 위에서 구성한 변수 \\(\mathbf{z}\_{t, a}\\)를 이용하여 2008년 9월의 무작위 트래픽 데이터에 대해 선행 논문[^6]에서 연구했던 이중 선형 로지스틱 회귀 모형을 재훈련시켰다. 그러면 선택 기준은 맥락 무관한 CTR 추정치와 사용자 별 CTR 조정을 위한 이중 선형 항의 합계가 된다. 모형 훈련의 경우 \\(\epsilon = 1\\)인 맥락무관한 **\\(\epsilon\\)-탐욕**을 사용하여 CTR을 추정했다.
...
세 번째로, **\\(\epsilon\\)-탐욕** 알고리즘(그림 2의 왼쪽)은 적절한 파라미터를 사용할 때 배포 버킷에서 UCB(그림 2의 오른쪽)와 비슷한 CTR을 달성한다. 따라서 두 가지 유형의 알고리즘 모두 비슷한 정책을 학습하는 걸로 나타났다. 그러나 실제 버킷 시험에서 진행했던 맥락 무관한 알고리즘의 실험 결과[^9]와 일치하게, 학습 버킷의 CTR이 더 낮은 편이다.
...
### 변수 사용에 대하여
  ...
  먼저 기사 추천에 변수를 사용하는 것이 도움이 되는지 조사해보자. 사용자 변수를 고려하여 ε-greedy (seg / disjoint / hybrid) 및 UCB 방법 (ucb (seg) 및 linucb (disjoint / hybrid))이 모두 약 10의 CTR 상승을 달성 할 수 있음이 표 1에서 분명합니다. 기준 ε- 욕심과 비교 한 %.
기능의 효과를 더 잘 시각화하기 위해 그림 3은 기사의 CTR (알고리즘에 의해 선택된 경우)이 기본 CTR (컨텍스트가없는 CTR)과 비교하여 어떻게 향상되었는지를 보여줍니다 .4 여기서 기사의 기본 CTR은 얼마나 흥미로운 지 측정합니다. 임의의 사용자를 대상으로하며 기록 된 이벤트에서 추정되었습니다. 따라서 기사의 해제 및 기본 CTR 비율이 높으면 알고리즘이 관심있는 사용자에게이 기사를 추천한다는 강력한 지표가됩니다. 그림 3 (a)는 ε-greedy 나 ucb가 사용자 정보를 사용하지 않았기 때문에 기사 CTR을 들어 올릴 수 없었 음을 보여줍니다. 대조적으로, 다른 세 가지 도표는 모두 개인화 된 권장 사항을 고려함으로써 분명한 이점을 보여줍니다. 극단적 인 경우 (그림 3 (c)), 기사의 CTR 중 하나가 1.31에서 3.03으로 132 % 개선되었습니다.

  또한 UCB 방법은 ε- 욕심 알고리즘과 비교할 때 배포 버킷에서 더 높은 CTR을 달성했으며 학습 버킷에서 이점이 훨씬 더 크다는 이전의 튜닝 데이터 결과와 일치합니다. 2.2 절에서 언급 한 바와 같이, 탐욕스러운 접근법은 탐색을 위해 무작위로 기사를 선택하기 때문에 지침이 없다. 반대로 신뢰 상한 방법의 탐색은 알고리즘의 CTR 추정치에서 불확실성을 측정하는 신뢰 구간에 의해 효과적으로 유도됩니다. 우리의 실험 결과는 신뢰 수준이 높은 방법의 효과를 암시하며 다른 많은 응용 분야에서도 비슷한 이점이 있다고 생각합니다.
  
### 데이터 크기에 대하여
  
  개인화 된 웹 서비스의 과제 중 하나는 응용 프로그램의 규모입니다. 예를 들어, 우리의 문제에서, 소수의 뉴스 기사는 인간 편집자들이 직접 선택했습니다. 그러나 더 많은 선택을 허용하거나 자동화 된 기사 선택 방법을 사용하여 기사 풀을 결정하려는 경우 대량의 Yahoo! 교통. 따라서 데이터가 드문 경우 알고리즘이 사용자 관심사와 기사 내용 간의 적절한 일치를 신속하게 식별하는 것이 중요합니다. 우리의 실험에서, 우리는 대량의 기사 풀이 있지만 고정 된 양의 트래픽이있는 상황을 모방하기 위해 데이터 크기를 인위적으로 (각각 30 %, 20 %, 10 %, 5 % 및 1 % 수준으로) 줄였습니다.
  
  비교 결과를 더 잘 시각화하기 위해 그림 4의 막대 그래프를 사용하여 모든 데이터 희소성 수준으로 모든 알고리즘의 CTR을 플로팅합니다. 몇 가지 관찰이 순서대로 이루어집니다. 첫째, 모든 데이터 희소성 수준에서 기능이 여전히 유용했습니다. 예를 들어 1 % 수준에서 ucb (1.354)에 비해 배포 버킷 (1.493)에서 linucb (하이브리드)의 CTR이 10.3 % 개선되었습니다.
  
  둘째, UCB 방법은 배포 버킷에서 욕심 많은 방법을 지속적으로 능가했습니다 .5 욕심에 대한 이점은 데이터 크기가 작을 때 훨씬 분명했습니다.
  
  셋째, ucb (seg)와 linucb (disjoint)에 비해 linucb (hybrid)는 데이터 크기가 작을 때 상당한 이점을 보여 주었다. 하이브리드 모델에서는 모든 기사가 일부 기능을 공유하므로 한 기사의 CTR 정보를 다른 기사에게 "전송"할 수 있습니다. 이 장점은 기사 풀이 클 때 특히 유용합니다. 대조적으로, 분리 된 모델에서, 한 기사의 피드백은 다른 기사에 의해 이용되지 않을 수있다; ucb (seg)도 마찬가지입니다. 그림 4 (a)는 전송 학습이 데이터가 희박 할 때 실제로 도움이된다는 것을 보여줍니다.
  
### ucb (seg)와 linucb (disjoint)를 비교하기
  
  그림 4 (a)에서 ucb (seg)와 linucb (disjoint)의 성능이 비슷한 것을 볼 수 있습니다. 우연의 일치가 아니라고 생각합니다. 비 연속 모델의 기능은 실제로 5.2.2 절에 설명 된 5 개의 클러스터에서 사용자의 정규화 된 멤버십 측정임을 기억하십시오. 따라서, 이러한 특징들은 ucb (seg)에 의해 채택 된 사용자 할당 프로세스의 "소프트"버전으로 볼 수있다. 그림 5는 가장 가까운 클러스터, 즉 사용자의 5 가지 비정형 기능 중 가장 큰 구성 요소에 대한 사용자의 상대적 회원 측정에 대한 막대 그래프를 보여줍니다. 대부분의 사용자는 5 개의 클러스터 센터 중 하나에 매우 근접한 것으로 나타났습니다. 최대 약 85 %의 사용자는 0.5보다 높았으며, 그 중 약 40 %는 0.8보다 높았습니다. 따라서 이러한 기능 중 상당수는 지배적 인 구성 요소를 갖기 때문에 기능 벡터가 "하드"버전의 사용자 그룹 할당과 유사합니다. Linucb (분리)를 ucb (세그먼트)와 추가로 구별하려면 주성분 분석에서 찾은 것과 같이 다양한 구성 요소로 더 많은 기능을 추가해야한다고 생각합니다.
  
# 6. 결론
  
이 백서는 뉴스 기사 추천과 같은 개인화 된 웹 기반 서비스에 대한 상황 별 접근 방식을 취합니다. 기록 된 이벤트에서 직접 산적 알고리즘을 평가할 수있는 간단하고 안정적인 방법을 제안하여 종종 문제가되는 시뮬레이터 구축 단계를 피할 수 있습니다. 실제 야후! 프론트 페이지 트래픽에서 상위 신뢰 한계 방법이 일반적으로 단순하지만 유도되지 않은 탐욕스러운 방법보다 성능이 우수함을 발견했습니다. 또한 새로운 알고리즘 LinUCB는 데이터가 드문 경우 이점을 보여 주므로 풀의 내용 수가 많은 경우 개인화 된 웹 서비스에 대한 효과를 제안합니다. 앞으로는 온라인 광고와 같은 다른 유사한 웹 기반 서비스에 대한 산적 접근 방식을 조사하고 알고리즘을 Banditron과 같은 관련 방법과 비교할 계획입니다 [16]. 두 번째 방향은 적기의 공식과 알고리즘을 확장하는 것입니다
"팔"은 복잡한 물체가 아닌 복잡한 물체를 가리킬 수 있습니다.
항목 (예 : 기사) 예를 들어 순위는 팔이 검색된 웹 페이지의 순열에 해당하는 순위입니다. 마지막으로, 사용자 관심사는 시간이 지남에 따라 변하기 때문에 산적 알고리즘에서 시간 정보를 고려하는 것이 흥미 롭습니다.
