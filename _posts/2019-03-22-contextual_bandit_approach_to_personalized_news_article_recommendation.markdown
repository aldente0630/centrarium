...
로깅 정책은 각 슬롯 손잡이를 무작위로 균일하게 선택하기 때문에 정책 평가자를 거치는 동안 각 사건은 정확히 \\(1 / K\\)의 발생 확률을 독립적으로 유지한다. 이는 보존한 사건이 \\(D\\)에 의해 선택된 것과 동일한 분포를 가짐을 뜻한다. 결과적으로 두 프로세스가 동일하다는 것을 증명할 수 있다. 여기서 두 프로세스란 전자는 \\(D\\)의 \\(T\\) 개의 실제 사건에 대해 정책을 평가하는 것을, 후자는 기록된 사건 스트림에서 정책 평가자를 적용하여 정책을 평가하는 것을 뜻한다.
