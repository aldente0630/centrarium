...
두 번째로, 변수 없는 버전의 \\(\epsilon\\)-탐욕과 UCB에 비해 웜 스타트 정보가 사용자 관심사와 기사 컨텐츠 간의 더 나은 대응을 찾는데 큰 도움이 됨을 도표을 통해 알 수 있다. 특히 **\\(\epsilon\\)-탐욕(웜 스타트)**와 **ucb(웜 스타트)**의 경우 맥락 무관한 정책이 달성 가능한 CTR의 최고값을 갖는 **전지적 정책**을 능가했다. 그러나 웜 스타트 정보를 사용하는 두 알고리즘의 성능은 온라인으로 가중치를 학습하는 알고리즘만큼 안정적이지 않았다. “웜 스타트”에 대한 오프라인 모형은 무작위 트래픽 전체에 대해 기사 별로 계산한 CTR을 갖고 훈련시켰으므로 **\\(\epsilon\\)-탐욕(웜 스타트)**의 경우 \\(\epsilon\\)이 1에 가까워야 배포 버킷에서 더 안정적인 성능을 얻을 수 있었다. 또한 학습 버킷에서는 사용자가 처음부터 더욱 흥미로워할 기사를 채택하여 **ucb(웜 스타트)**가 도움이 됐지만 배포에 가장 적합한 온라인 모형을 결정하는데 **ucb(웜 스타트)**는 도움이 되지 않았다. **ucb**는 탐색을 위한 신뢰 구간에 의존하기 때문에 "웜 스타트"를 통해 유입된 초기값 편향를 수정하는 건 어려운 일이다. 대조적으로 모든 온라인 학습 알고리즘은 전지적 정책을 능가했다. 따라서 평가 데이터에 대해 웜 스타트 알고리즘을 시도하지 않았다.
  
세 번째로, **\\(\epsilon\\)-탐욕** 알고리즘(그림 2의 왼쪽)은 적절한 파라미터를 사용할 때 배포 버킷에서 UCB(그림 2의 오른쪽)와 비슷한 CTR을 달성했다. 따라서 두 가지 유형의 알고리즘 모두 비슷한 정책을 학습하는 걸로 나타났다. 그러나 학습 버킷에서 CTR이 더 낮은 것처럼 보이며 실제 버킷 시험에서는 맥락 무관한 알고리즘의 실험 결과와 일치한다.
