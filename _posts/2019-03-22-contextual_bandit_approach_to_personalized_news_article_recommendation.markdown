...
### ucb(세분화)와 linucb(분리)를 비교할 때
  ...
  그림 4 (a)에서 ucb (seg)와 linucb (disjoint)의 성능이 비슷한 것을 볼 수 있습니다. 우연의 일치가 아니라고 생각합니다. 비 연속 모델의 기능은 실제로 5.2.2 절에 설명 된 5 개의 클러스터에서 사용자의 정규화 된 멤버십 측정임을 기억하십시오. 따라서, 이러한 특징들은 ucb (seg)에 의해 채택 된 사용자 할당 프로세스의 "소프트"버전으로 볼 수있다.   
  그림 5는 가장 가까운 클러스터, 즉 사용자의 5 가지 비정형 기능 중 가장 큰 구성 요소에 대한 사용자의 상대적 회원 측정에 대한 막대 그래프를 보여줍니다. 대부분의 사용자는 5 개의 클러스터 센터 중 하나에 매우 근접한 것으로 나타났습니다. 최대 약 85 %의 사용자는 0.5보다 높았으며, 그 중 약 40 %는 0.8보다 높았습니다. 따라서 이러한 기능 중 상당수는 지배적 인 구성 요소를 갖기 때문에 기능 벡터가 "하드"버전의 사용자 그룹 할당과 유사합니다. 
  
![그림5](https://aldente0630.github.io/assets/contextual-bandit_approach_to_personalized_news_article_recommendation6.png) 
**그림 5: 사용자 최대 멤버십 히스토그램.**
  
  Linucb (분리)를 ucb (세그먼트)와 추가로 구별하려면 주성분 분석에서 찾은 것과 같이 다양한 구성 요소로 더 많은 기능을 추가해야한다고 생각합니다.
