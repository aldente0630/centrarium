...
이 장은 K개의 슬롯 손잡이를 갖는 컨텍스츄얼 밴딧 문제를 수식으로 정의하고 이것을 개인화 뉴스 기사 추천 문제에 어떻게 적용할 수 있는지 보인다. 그런 다음 기존 방법과 그 한계점을 논한다.

## 2.1다중 슬롯머신 실험 문제 수식화

개인화 뉴스 기사 추천 문제는 자연스럽게 맥락 상의 정보가 있는 다중 슬롯머신 문제로 모형화할 수 있다. 이전 연구에 따라 이를 *컨텍스츄얼 밴딧*이라고 부를 것이다. 공식적으로 컨텍스츄얼 밴딧 알고리즘 A는 이산 적 재판 t = 1, 2, 3, ...으로 진행된다. 재판에서 :
