...
일반적인 컨텍스츄얼 밴딧 문제에서 특별히 중요한 경우는 (ⅰ) 슬롯 손잡이의 집합 \\(\mathcal{A}_t\\)이 변하지 않으며 모든 \\(t\\)마다 슬롯 손잡이가 \\(K\\)개인 (ⅱ) 사용자 \\(u_t\\)(또는 이와 동치로 맥락 \\(\(\mathbf{x}\_{t, a}\)\\) )가 모든 \\(t\\)마다 동일한, 잘 알려진 *\\(K\\)개의 슬롯 손잡이를 갖는 다중 슬롯머신 실험 문제*이다. 팔 세트와 컨텍스트는 매 시도마다 일정하기 때문에 적기 알고리즘과 아무런 차이가 없으므로이 유형의 적기를 상황없는 적기라고 부른다.
