...
### 데이터 크기에 대하여
  ...
  개인화된 웹 서비스의 과제 중 하나는 응용 프로그램의 규모이다. 예를 들어 본 문제의 경우 소수의 뉴스 기사는 편집자들이 직접 손으로 선택했다. 그러나 더 많은 선택을 허용하거나 자동화 된 기사 선택 방법을 사용하여 기사 풀을 결정하려는 경우 대량의 Yahoo! 교통. 따라서 데이터가 드문 경우 알고리즘이 사용자 관심사와 기사 내용 간의 적절한 일치를 신속하게 식별하는 것이 중요합니다. 우리의 실험에서, 우리는 대량의 기사 풀이 있지만 고정 된 양의 트래픽이있는 상황을 모방하기 위해 데이터 크기를 인위적으로 (각각 30%, 20%, 10%, 5% 및 1% 수준으로) 줄였습니다.
