...
정책 평가자 내의 *모든* 이력 \\(h_T\\)가 현실 세계에서 발생할 확률과 동일하다고 이 정리는 말한다. 알고리즘 3이 반환하는 평균 손익 \\(R_T / T\\)처럼 이력에 관한 다수의 통계량은 알고리즘 \\(\pi\\) 값에 대한 편향되지 않은 추정치이다. 또한 정리에 따르면 길이 \\(T\\)의 표본을 보유하기 위해 \\(KT\\)개의 기록된 사건이 평균적으로 필요하다.
...
모든 역사 ht에 대해 동일한 진술을 증명하고 싶습니다. 데이터는 i.d.이므로 정책의 임의 추출은 전 세계의 임의 추출과 무관하며, 우리는 t 번째 사건에 대한 분포가 각 프로세스마다 동일하다는 것을 역사 ht-1에 따라 조절한다는 것만 증명하면됩니다. 다시 말해, 우리는 다음을 보여 주어야합니다.
  
로깅 정책에서 arm a가 무작위로 균일하게 선택되므로 정책 평가자가 내부 루프를 종료 할 확률은 모든 정책, 모든 기록, 기능 및 모든 암에 대해 동일하므로 확률이있는 마지막 이벤트에서 발생합니다. 마지막 이벤트의 PrD (xt, 1, ..., xt, K, rt, a). 마찬가지로 정책 π가
팔에 대한 분포는 이력 ht-1과 특징 (xt, 1, ..., xt, K)에 따라 독립적이며 팔 a의 확률은 단지 Prπ (ht-1) (a | xt, 1, .. , xt, K).
  
마지막으로, 스트림의 각 이벤트는 정확히 1 / K의 확률로 유지되므로 T 이벤트를 유지하는 데 필요한 예상 수는 정확히 KT입니다.
