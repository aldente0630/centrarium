...
상술한 과정에서 \\(\mathsf{A}\\)의 *총 \\(T\\)번의 시도에 따른 손익*을 \\(\sum_\{t = 1}^T r\_{t, a}\\)로 정의한다. 이와 비슷하게 *총 \\(T\\)번의 시도에 따른 최적 손익의 기대값*을 \\(\mathbf{E}\[\sum_\{t = 1}^T r\_{t, a_t^\*}\]\\)으로 정의한다. 여기서 \\(a_t^\*\\)는 \\(t\\)번째 시도에서 기대 손익이 최대인 슬롯 손잡이이다. 목표는 \\(\mathsf{A}\\)를 잘 설계하여 위의 총 손익의 기대값을 최대로 만드는 것이다. 이와 동치로 최적의 슬롯 손잡이 선택 전략에 대해 리그릿이 최소가 되게 하는 알고리즘을 찾는 것이다. 여기서 알고리즘 A의 T-trial 후회 RA (T)는 다음과 같이 공식적으로 정의된다.
