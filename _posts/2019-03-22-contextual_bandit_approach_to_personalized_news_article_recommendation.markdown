...
알고리즘 1은 입력 파라미터가 오직 \\(\alpha\\)인 **LinUCB** 알고리즘 전반에 대하여 자세하게 설명한다. 식 (1)에 주어진 \\(\alpha\\) 값에 주목하라. 식 (4)의 경우 일부 응용 프로그램에서 보수적으로 큰 값일 수 있으므로 매개 변수를 최적화할 때 전체 손익을 높일 수 있다. 모든 UCB 방법과 마찬가지로 **LinUCB**는 항상 UCB가 가장 높은 슬롯 손잡이를 선택한다(식 (5)와 같다).
