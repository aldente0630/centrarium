...
기사 추천의 관점에서 컨텐츠 풀에 속하는 기사를 하나의 슬롯 손잡이로 볼 수 있다. 제공한 기사를 클릭하면 손익은 1이고 그렇지 않으면 0이라고 하자. 이 손익 정의에 따라 어떤 기사의 손익 기대값은 정확히 *클릭률*(CTR)이며 CTR이 최대인 기사를 선택하는 것과 사용자의 기대 클릭 수를 최대로 만드는 건 동일한 얘기다. 또한 다중 슬롯머신 실험 수식에서 손익의 기대값을 최대로 만드는 것과 같다.
...
맥락 무관한, \\(K\\)개의 슬롯 손잡이를 갖는 다중 슬롯머신 실험의 경우 광범위하게 연구되면서 이해가 깊어졌지만 보다 일반적인 컨텍스츄얼 밴딧 문제는 여전히 난제이다. **EXP4** 알고리즘은 \\(\tilde{O}(\sqrt{T})\\)의 리그릿을 달성하기 위해 지수 가중 기법을 이용하지만 변수 개수가 늘어남에 따라 계산 복잡도가 지수적으로 증가한다. 일반적인 컨텍스츄얼 밴딧 알고리즘 중 하나는 \\(\epsilon\\)을 축소시켜 나가는 **\\(\epsilon\\)-탐욕**과 유사한 유형인 **에폭 탐욕** 알고리즘이다. 해당 알고리즘은 신탁적 옵티마이저가 주어진다는 가정 하에 계산량이 효율적이지만 보장하는 리그릿은 \\(\tilde{O}(T^{2 \over 3})\\) 수준으로 약한 편이다.
  
다중 슬롯머신 실험에 대한 다양한 모형화 가정 아래 리그릿을 좀 더 강력히 보장하는 알고리즘을 설계할 수 있다. Auer는 슬롯 손잡이의 기대 손익이 변수에 선형적이라고 가정하고 본질적으로 UCB 유형의 접근법인 **LinRel** 알고리즘을 소개한다. 그 변형 중 하나는 이전 알고리즘을 상당히 개선한 \\(\tilde{O}(\sqrt{T})\\)의 리그릿을 갖는다.
  
초기 알고리즘에 비해 크게 개선되었습니다 [1]. 마지막으로 Gittins 지수 방법 [15]과 같은 Bayes 규칙에 기반한 또 다른 종류의 적기 알고리즘이 있음을 알 수있다. 적절히 정의 된 사전 분포를 사용하면 베이지안 접근법이 좋은 성능을 발휘할 수 있습니다. 이 방법은 좋은 이전 모델을 얻기 위해 광범위한 오프라인 엔지니어링이 필요하며 근사화 기술과의 결합이 없으면 계산적으로 금지됩니다 [2].
  
# 3. 알고리즘
  
컨텍스트없는 적기 알고리즘에 대한 점근 적 최적 성 및 UCB 방법의 강한 후회를 감안할 때 문맥 적 문제에 대한 유사한 알고리즘을 고안하는 것이 바람직하다. 일부 감안할 때
매개 변수 형태의 보수 함수를 이용하여 추정 된 팔 보수의 UCB를 계산할 수있는 매개 변수의 신뢰 구간을 데이터로부터 추정하기위한 많은 방법이 존재한다. 그러나 이러한 접근법은 일반적으로 비용이 많이 든다.
  
이 연구에서, 우리는 보수 구간이 선형 일 때 닫힌 형태로 신뢰 구간을 효율적으로 계산할 수 있고이 알고리즘을 LinUCB라고 부른다. 설명의 편의를 위해, 우리는 먼저 분리 된 선형 모델에 대한 더 간단한 형태를 기술하고, 3.2 절에서 하이브리드 모델의 일반적인 경우를 고려한다. LinUCB는 개인화 된 뉴스 기사 권장 사항 이외의 애플리케이션에 적용되는 일반적인 문맥 적 산적 알고리즘입니다.
