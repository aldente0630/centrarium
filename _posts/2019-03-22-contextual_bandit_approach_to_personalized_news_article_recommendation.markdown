...
알고리즘 1(또는 해당 논문과 유사한 알고리즘)은 행렬 \\(\mathbf{D}^T_a\mathbf{D}_a + \mathbf{I}_d\\)(또는 \\(\mathbf{D}^T_a\mathbf{D}_a\\))의 역행렬을 계산한다. 여기서 \\(\mathbf{D}_a\\)는 학습 데이터 변수에 해당하는 행을 갖는 디자인 행렬이다. 모든 팔의이 행렬은 고정 된 차원 d × d를 가지며 효율적으로 점차적으로 업데이트 될 수 있습니다. 또한 알고리즘 1의 파라미터가 서로 다르기 때문에 이들의 역수를 쉽게 계산할 수 있습니다. (3) 다른 팔의 훈련 데이터의 영향을받지 않으므로 별도로 계산할 수 있습니다. 지금 우리는 *하이브리드* 모델에 대해 더 흥미로운 사례를 고려합니다. 
