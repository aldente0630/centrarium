...
다중 슬롯머신 실험에 대한 다양한 모형화 가정 아래 리그릿을 좀 더 강력히 보장하는 알고리즘을 설계할 수 있다. 슬롯 손잡이의 기대 손익이 변수에 선형적이라고 가정하면 Auer는 본질적으로 UCB 유형 접근법인 **LinRel** 알고리즘을 설명하고 변종 중 하나에는 O ~ (√T)의 후회가 있음을 보여 주며, 이전 알고리즘 [1].
