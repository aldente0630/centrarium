...
$$R_{\mathsf{A}}(T) \overset{\underset{\mathrm{def}}{}}{=} \mathbf{E}[\sum_{t = 1}^T r_{t, a_t^*}] - \mathbf{E}[\sum_{t = 1}^T r_{t, a_t}] \  (1)$$
...
다중 슬롯머신 실험의 경우 핵심 과제는 탐색과 활용 간의 균형점을 찾는 일이다. 수식 (1)의 리그릿을 최소로 만들고자 알고리즘 \\(\mathsf{A}\\)는 과거 경험을 바탕으로 가장 좋아보이는 슬롯 손잡이를 선택한다. 반면에,이 겉으로보기에 최적 인 팔은 A의 지식에 부정확 함으로 인해 차선책 일 수 있습니다. 이러한 바람직하지 않은 상황을 피하기 위해, A는 그들에 대한 더 많은 정보를 수집하기 위해보기 흉한 무기를 실제로 선택함으로써 탐구해야한다 (c.f., 이전 하위 섹션에서 정의 된 적기 프로세스의 3 단계). 일부 최적의 무기가 선택 될 수 있으므로 탐사는 단기 후회를 증가시킬 수 있습니다. 그러나 무기의 평균 보수 (즉, 탐사)에 대한 정보를 얻는 것은 A의 무기 보수에 대한 추정을 개선 할 수 있으며, 결과적으로 장기적인 후회를 줄일 수 있습니다. 분명히 순수하게 탐색하는 알고리즘이나 순수하게 악용 알고리즘은 일반적으로 가장 잘 작동하지 않으며 좋은 절충이 필요합니다.
   
알고리즘은 일반적으로 가장 잘 작동하며 좋은 절충이 필요합니다. 맥락이없는 K- 무장 적기 기적 문제는 통계 학자들에 의해 오랫동안 연구되어왔다 [9, 24, 26]. 가장 단순하고 가장 간단한 알고리즘 중 하나는 - greedy입니다. 각 시험에서이 알고리즘은 먼저 각 팔 a의 평균 보수 값 μt, a를 추정합니다. 그 다음, 확률 1 -로, 욕심 팔 (즉, 가장 높은 보수 추정치를 갖는 팔)을 선택한다; 확률로 무작위 팔을 선택합니다. 한계 안에, 각 팔은 무한하게 수시로 시험 될 것이다, 그래서 지불 보수 견적 μt, a는 확률 1을 가진 진실한 가치 μa에 수렴한다. 더구나, 적절하게 감퇴해서 (예를 들면, [24]), 단계 당 후회, RA (T) / T는 확률 1로 0으로 수렴합니다.
