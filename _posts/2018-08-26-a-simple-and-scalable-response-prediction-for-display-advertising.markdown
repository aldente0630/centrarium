...
전환 모델을 구축하려면 전환 이벤트가 올바른 클릭 이벤트로 분류되어야합니다. 이는 전환 이벤트가없는 클릭 이벤트에 비해 긍정적 인 PCC 사례를 나타 내기 때문입니다. 전환 이벤트는 클릭 이벤트 후 몇 분, 몇 시간 또는 며칠 후에 발생할 수 있습니다. 클릭 이벤트와 전환 이벤트 속성을 올바르게 일치시킴으로써 수행 할 수있는 마우스 오른쪽 버튼 이벤트에 대한 전환 이벤트의 적절한 속성은 PCC 예측뿐만 아니라 지불 처리에도 필수적입니다.
...
![클릭 전환 기여 시간 지연 분포](https://aldente0630.github.io/assets/a_simple_and_scalable_response_prediction_for_display_advertising1.PNG)
