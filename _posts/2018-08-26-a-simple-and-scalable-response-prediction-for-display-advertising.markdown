...
실제 트래픽 데이터에 대해 대규모 실험 결과, 우리 프레임워크는 디스플레이 광고에 사용하고 있는 최첨단 모형보다 뛰어나다. 이 결과와 제안한 프레임워크의 단순성으로 디스플레이 광고 반응 예측을 위한 표준으로 삼을 수 있을 것이다.

논문의 나머지 부분은 다음과 같이 구성되어있다. 2장은 연관된 연구에 대해 논의한다. 3장에서는 클릭률과 전환율의 변수 차이를
