...
디스플레이 광고 맥락에서 이런 변수를 모두 사용할 수 있는 건 아니다. 일반적으로 광고주와 게시자 정보는 고유 식별자로 표시된다. 각 고유 식별자가 변수로 간주되는 공간에서 차원의 크기는 주요 관심사가 된다. 상호 의존 정보와 이와 유사한 필터 기법이 이 영역에선 래퍼 기반 방법 상에서 자주 사용한다. 차원 감소를 다루기 위한 최신 접근법은 논문[^5]에 나와있다. 해시의 기본 개념은 하위 차원 공간으로 투영하여 변수가 취할 수 있는 값의 수를 줄이는 것이다. 이 접근법은 단순성과 경험적인 효용성으로 인해 대규모 기계 학습의 맥락에서 인기를 얻고 있다.
