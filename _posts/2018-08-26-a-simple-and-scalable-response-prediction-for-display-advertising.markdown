...
일간 광고 노출 약 10억 건을 처리하며 세계에서 가장 큰 광고 거래소 중 하나인 Yahoo! Right Media Exchange(RMX)의 실제 트래픽 기록을 연구를 위해 수집했다. RMX는 광고주와 게시자 사이를 네트워크라고 하는 중개자가 편리하게 연결해주는 *네트워크 속 네트워크* 모델을 따른다. 네트워크는 게시자와 광고주 중 한쪽 또는 양쪽 모두 가질 수 있다. 교환의 모든 주체(네트워크, 게시자와 광고주)는 고유 식별자(ID)를 갖는다. 게시자는 자신의 웹 페이지에 사이트 ID를 표시한다. 또한 동일한 페이지의 다른 부분에 다른 섹션 ID로 태그를 지정할 수 있습니다. 교환 프로그램은 섹션 ID의 의미에 대한 규칙을 시행하지 않지만 게시자는 종종 동일한 페이지의 다른 광고 슬롯에 다른 섹션 ID를 사용합니다. 네트워크와 광고주 또는 게시자 간의 관계는 가격 유형, 예산 및 타겟팅 프로필과 같은 일련의 특성에 의해 지정됩니다.
