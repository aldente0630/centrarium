...
세계에서 가장 큰 광고 거래소 중 하나이며 매일 광고 노출 약 10억 건을 처리하는 Yahoo! Right Media Exchange(RMX)의 실제 트래픽 기록을 연구를 위해 수집했다. RMX는 광고주와 게시자 사이를 네트워크라고 하는 중개자가 편리하게 연결해주는 *네트워크 속 네트워크* 모델을 따른다. 네트워크는 게시자와 광고주 중 한쪽 또는 양쪽 모두 가질 수 있다. 교환의 모든 주체(네트워크, 게시자와 광고주)는 고유 식별자(ID)를 갖는다. 게시자는 웹 페이지에 사이트 ID를 부여한다. 또한 동일한 페이지 속 다른 부분마다 다른 섹션 ID를 태그로 지정할 수 있다. 거래소는 섹션 ID 의미에 대한 규칙을 강제하지 않지만 게시자는 종종 동일한 페이지의 다른 광고 슬롯에 다른 섹션 ID를 사용한다. 네트워크와 광고주 또는 게시자 간의 관계는 가격 유형, 예산 및 타겟팅 프로필과 같은 일련의 특성에 의해 지정된다.

### 3.2 클릭률과 전환률
  
Google은 클릭 및 클릭 후 전환 (PCC) 이벤트의 기본 속성을 파악하는 데 관심이 있습니다. 이 백서의 맥락에서 클릭은 사용자가 마우스 클릭 (또는 이와 동등한 방법)을 통해 광고와 상호 작용할 때 발생하는 이벤트를 나타냅니다. 반면 클릭 후 전환은 사용자가 광고주의 방문 페이지를 방문한 후 취하는 행동으로 해당 광고주가 가치 있다고 간주합니다. 전환 액션의 전형적인 예에는 이메일 목록 구독, 예약 또는 제품 구매가 포함됩니다. 이 두 가지 유형의 이벤트의 속성을 측정하기위한 기본 수량은 각각 클릭률 (CTR)과 전환율 (CVR)입니다.
