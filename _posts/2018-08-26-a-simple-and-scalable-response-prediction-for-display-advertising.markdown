...
우리는 클릭과 클릭 후 전환(PCC) 이벤트의 기본적인 성질을 파악하는데 관심이 있다. 본 논문의 맥락에서 *클릭*은 사용자가 마우스 클릭(또는 이와 동등한 방법)을 통해 광고와 상호 작용할 때 발생하는 이벤트를 말한다. 반면 *클릭 후 전환*은 사용자가 광고주의 랜딩 페이지를 방문한 후 취하는 행동으로 해당 광고주가 가치 있다고 간주합니다. 전환 액션의 전형적인 예에는 이메일 목록 구독, 예약 또는 제품 구매가 포함됩니다. 이 두 가지 유형의 이벤트의 속성을 측정하기위한 기본 수량은 각각 클릭률 (CTR)과 전환율 (CVR)입니다.
