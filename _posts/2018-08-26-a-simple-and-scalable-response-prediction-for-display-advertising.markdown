...
전산 광고에서 응답 예측을 위해 개발한 학습 방법은 사용자 응답에 영향을 줄 수 있는 모든 요인을 변수 또는 공변량으로 명시적으로 포함시켜 회귀 또는 분류 모형을 사용한다. 
  
이러한 모형에 사용한 변수는 다음과 같이 분류할 수 있다.
  
스폰서 검색의 쿼리 또는 콘텐츠 일치의 게시자 페이지와 같은 컨텍스트 기능; [Ciaramita et al. 2008; Hillard et al. 2010], 텍스트 광고는 [Cheng et al. 2012; Liu et al. 2012] 디스플레이 광고; [Cheng and Cantu-Paz 2010]에서 소개 한 사용자 기능. 이력 데이터를 집계하여 생성되고 [Chakrabarti et al. 2008; Hillard et al. 2010].
