...
게시자는 웹 페이지에 사이트 ID를 부여한다. 또한 동일 페이지 속 다른 부분마다 다른 섹션 ID를 태그로 지정할 수 있다. 거래소는 섹션 ID에 의미론적 규칙을 강제하진 않지만 게시자는 종종 동일한 페이지의 다른 광고 슬롯에 다른 섹션 ID를 사용한다. 네트워크와 광고주 또는 게시자 간의 관계는 가격 유형, 예산 및 타겟팅 프로필과 같은 일련의 특성에 의해 지정된다.
