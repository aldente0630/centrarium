...
데이터 과학자만큼이나 작업, 목표와 기술이 다양하기 때문에 데이터 과학 작업 흐름에 여러 형태가 있다. 탐색 작업 또는 임시로 분석하는 연구 과학자는 제품 코드를 작성할 필요가 없다. GitHub의 기계 학습 팀의 데이터 과학자와 같은 데이터 과학자들은 규모에 맞게 실행되어야하는 소프트웨어 제품으로 끝나는 코드를 작성합니다.
